...
from .utils import load_config_from_tango, logger, parse_config_string
...
def get_default_reader(tango_host=''):
    try:
        ...
        logger.info('tango://DefaultSchema: %s' % str(schema))
...
def reader(apiclass='pyhdbpp.abstract.AbstractReader', config=None):
    ...
    """
    logger.info('Creating pyhdbpp.reader(%s,%s)' % (apiclass,config))
