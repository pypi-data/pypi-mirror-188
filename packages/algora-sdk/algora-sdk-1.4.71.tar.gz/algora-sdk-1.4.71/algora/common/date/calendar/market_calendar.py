...
from algora.quant.model.enum import WeekdayCorrection
...
class MarketCalendar:
    ...
    def adjust(self, as_of_date: date, correction: WeekdayCorrection) -> date:
        """
        ...
        Parameters:
            ...
            correction (date): The type of correction to use

        Returns:
            date: The adjusted date
        """
        if correction == correction.UNADJUSTED:
            return as_of_date
        elif correction == correction.FOLLOWING:
            return self._following(as_of_date)
        elif correction == correction.MODIFIED_FOLLOWING:
            return self._modified_following(as_of_date)
        elif correction == correction.PRECEDING:
            return self._preceding(as_of_date)
        elif correction == correction.MODIFIED_PRECEDING:
            return self._modified_preceding(as_of_date)
        else:
            raise Exception(f"WeekdayCorrections '{correction}' not supported")
