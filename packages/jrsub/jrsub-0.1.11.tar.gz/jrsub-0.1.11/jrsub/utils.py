...
def _latin_to_hiragana(latin: str, colons_to_double_vowel: bool = True) -> str:
    return JTran.transliterate_from_latn_to_hrkt(
        latin, colons_to_double_vowel=colons_to_double_vowel
    )


def _latin_to_katakana(latin: str) -> str:
    return JTran.transliterate_from_hira_to_kana(
        JTran.transliterate_from_latn_to_hrkt(latin, False)
    )
...
def _distance(this: str, other: str) -> float:
    ...
    for char in other:
        if char in this_temp:
            ...
            diff_chars += 1
    dist = float(common_chars) / max(len(this), len(other)) - (
        diff_chars + len(this_temp)
    )
