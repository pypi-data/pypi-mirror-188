...
class YarxiDictionary:
        ...
        return list(
            set(
                sum(
                    [
                        tr.translation
                        for tr in self.lookup(lexeme, reading, search_mode, order)
                    ],
                    [],
                )
            )
    ...
    def _translate_by_eid(self, eid: str) -> List[str]:
        def _retrieve_from_references(
            refs: List[YarxiReference], visited_references: List[str]
        ) -> List[str]:
            if not refs:
                return []
            ...
            referenced_entries = [
                (
                    usable_ref.verified,
                    usable_ref.mode,
                    self._get_entry_by_eid(usable_ref.eid),
                )
                for usable_ref in usable_refs
            ]
            ...
            for (
                ...
                reference_mode,
                referenced_entry,
            ) in referenced_entries:
                if referenced_entry.eid in visited_references:
                    continue
                if reference_verified:
                    translations.extend(
                        [
                            " ".join([reference_mode, tr]).strip()
                            for tr in referenced_entry.translation
                        ]
                    )

                usable_refs.extend(
                    [ref for ref in referenced_entry.references if ref.usable]
                )
                visited_references.append(referenced_entry.eid)

            return translations + _retrieve_from_references(
                usable_refs, visited_references
            )

        entry = self._get_entry_by_eid(eid)
        return entry.translation + _retrieve_from_references(
            entry.references, [entry.eid]
        )
    ...
    def save(self, fname: str = "default"):
        if fname == "default":
            ...
            ).parent.joinpath("dictionaries/yarxi.jtdb")
...
class YarxiLoader:
    ...
    def _normalize_kana(self, key: str, mode: str) -> str:
        if mode == "right":
            if self._transliterate_collocations:
                ...
                return _latin_to_hiragana(
                    self._collocations_right[key].replace("wa", "ha").replace(" ", "")
                )
        else:
            if self._transliterate_collocations:
                ...
                return _latin_to_hiragana(
                    self._collocations_left[key].replace("wa", "ha").replace(" ", "")
                )
    ...
    def _in_container(self, container, readings, kanji) -> int:
        res = sorted(
            [
                e
                for e in container
                if set.intersection(set(e.reading), set([cr for cr in readings if cr]))
                == set([cr for cr in readings if cr])
                and any(kj in e.lexeme for kj in kanji)
            ],
            key=lambda x: len(x.reading),
        )
        if res:
    ...
    def _load_kanji_db(self, fname) -> {str: str}:
        ...
        kanji = cursor.execute(
            "SELECT Nomer, Uncd, Kunyomi, Onyomi, Russian, RusNick, UC, UCN from Kanji"
        ).fetchall()
        for (
            eid,
            uncd,
            ...
            rus_nick,
            capital_letters,
            capital_letters_nick,
        ) in kanji:
            res[str(eid)] = self._Kanji(
                kanji=chr(int(uncd)),
                kun=kun,
                on=on,
                rus=self._capitalize(rus, capital_letters),
                rus_nick=self._capitalize(rus_nick, capital_letters_nick).split("*#*")[
                    0
                ],
            )
        conn.close()
    ...
    def _resolve_references(self, show_progress: bool) -> [YarxiReference]:
        for entry in tqdm(
            self._entries,
            desc="[Yarxi] Resolving references".ljust(34),
                ...
                for i in range(0, original_length):
                    if refs[i].eid:
                        ...
                        if (
                            ...
                            and trg.translation
                            and all(
                                tr.startswith(refs[i].mode) for tr in trg.translation
                            )
                        ):
                            refs[i].mode = ""
                    else:
                        fitting = [
                            et
                            for et in self._entries
                            if any(lex in et.lexeme for lex in refs[i].lexeme)
                        ]
                        if len(fitting) > 1:
                            fitting = [
                                f
                                for f in fitting
                                if list(set(f.reading) & set(entry.reading))
                            ]
                        if len(fitting) == 1:
                            ...
                            if (
                                refs[i].mode
                                and fitting[0].translation
                                and all(
                                    tr.startswith(refs[i].mode)
                                    for tr in fitting[0].translation
                                )
                            ):
                                refs[i].mode = ""
                        else:
    ...
    def _load_db(self, fname, show_progress: bool = True) -> List[YarxiEntry]:
        ...
        tango = cursor.execute(
            "SELECT K1, K2, K3, K4, Kana, Reading, Russian, Nomer, Hyphens, UC FROM Tango"
        ).fetchall()[:-1]
        for (
            k1,
            k2,
            ...
            tango,
            desc="[Yarxi] Building word database".ljust(34),
            disable=not show_progress,
        ):
            res.extend(
                self._convert_to_entry_tango(
                    (
                        str(k1),
                        str(k2),
                        str(k3),
    ...
    def _clean_text(self, text: str) -> str:
        ...
        text = text.replace("@\\", "")
        text = re.sub(
            r"\[\\\'\'\\\^(.*)\\\'\'\\\]", lambda m: f"(от «{m.group(1)}»)", text
        )
        text = re.sub(r"(\^[\^|@])", "", text)
        ...
        text = re.sub(
            r"([^\d]),([^=\s])", lambda m: f"{m.group(1)}, {m.group(2)}", text
        )
        text = re.sub(
            r"([^〉]),([^\d\s])", lambda m: f"{m.group(1)}, {m.group(2)}", text
        )
        ...
        if self._highlighting != ("《", "》"):
            text = text.replace("《", self._highlighting[0]).replace(
                "》", self._highlighting[1]
            )
        return text.strip()

    def _convert_to_entry_tango(
        self, info: Tuple[str, str, str, str, str, str, str, str, str, str]
    ) -> List[YarxiEntry]:
        def _resolve_readings(reading: str, variable: bool, hyphens: str) -> List[str]:
            ...
            if variable:
                ...
                if len(split_reading) > 1:
                    base_reading = [r for r in split_reading[1].split("*") if r][
                        -len(full_reading) :
                    ]
                else:
                    base_reading = []
            ...
            return sorted(
                [_latin_to_hiragana(rd.lower()).strip() for rd in res if rd], key=len
            )

        def _resolve_lexemes(lexeme: str) -> (List[str], List[str], bool):
            kanji = [
                ("1", self._get_kanji(k1)),
                ("2", self._get_kanji(k2)),
                ("3", self._get_kanji(k3)),
                ("4", self._get_kanji(k4)),
            ]
            if not lexeme:
                return (
                    [k[1] for k in kanji if k[1]],
                    ["".join([k[1] for k in kanji])],
                    False,
                )
            ...
            if "^" in lexeme:
                lexeme = re.sub(
                    r"\^([\w:]+)@?", lambda x: _latin_to_katakana(x.group(1)), lexeme
                )
                lexeme = re.sub(
                    r"([\w:]*)", lambda x: _latin_to_hiragana(x.group(1)), lexeme
                )
            else:
            ...
            if "[" in lexeme:
                return (
                    kanji_for_entry,
                    [
                        re.sub(r"(\[(.*?)\])", "", lexeme).strip(),
                        re.sub(r"\[(.*?)\]", r"\1", lexeme).strip(),
                    ],
                    False,
                )
            elif "(" in lexeme:
        ...
        def _extract_references(translation: str) -> (List[str], List[str]):
            def _extract_reference(translation: str) -> (str, str):
                ...
                if word_refs:
                    if (
                        re.search(r"^\\?#\\?.*?\\?#\\?\^\^?\d+\\?_?", translation)
                        is not None
                    ):
                        mode = self._clean_text(
                            re.search(
                                r"^(\\?#\\?.*?\\?#\\?)\^\^?\d+\\?_?", translation
                            ).group(1)
                        )
                        ...
                        mode = (
                            "〈~"
                            + self._normalize_kana(
                                re.search(r"^(\*+=?\d+)\^\^?\d+", translation).group(1),
                                "right",
                            )
                            + "〉"
                        )
                        ...
                        mode = (
                            "〈-"
                            + self._normalize_kana(
                                re.search(r"^(\*-\d+)\\?\^\^?\d+", translation).group(
                                    1
                                ),
                                "left",
                            )
                    ...
                    for word_ref in word_refs:
                        res.append(
                            YarxiReference(
                                eid=re.search(r"0*(\d+)", word_ref).group(1), mode=mode
                            )
                        )
                ...
                kanji_refs.extend(
                    re.findall(
                        r"\^0-0*(\d+)-?(?:\\\'\'\\([\w|\s]+)\\\'\')?", translation
                    )
                )
                kanji_refs.extend(
                    re.findall(
                        r"\^2-0*(\d+)-?(?:\\\'\'\\([\w|\s]+)\\\'\')?", translation
                    )
                )
                ...
                if kanji_refs:
                    ...
                    for kanji_ref in kanji_refs:
                        res.append(
                            YarxiReference(
                                lexeme=[
                                    self._get_kanji(kanji_ref[0])
                                    + _latin_to_hiragana(kanji_ref[1].strip())
                                ],
                                eid="",
                                mode=mode,
                            )
            ...
            def _split_translation(translation: str) -> List[str]:
                ...
                numbered_translations = [
                    tr for tr in translation[cnt + 1 :].split("&") if tr
                ]
                if (
                    numbered_translations
                    and len(numbered_translations[0]) == len(translation) - 1
                ):
            ...
            ref_tr = [
                _extract_reference(tr.strip()) for tr in _split_translation(translation)
            ]

            return sum([elem[0] for elem in ref_tr], []), [
                elem[1] for elem in ref_tr if elem[1]
            ]
    ...
    def _extract_compound_values(self, kanji: {str: _Kanji}, show_progress: bool):
        def _split_and_clean_compound_translations(translations: str, rus_nick: str):
            ...
            translations = re.sub(
                r"\\\^\'\'\\([a-z]+)\\\'\'",
                lambda m: f"\\{_latin_to_hiragana(m.group(1))}",
                translations,
            )
            translations = re.sub(r"(\^[78563]\d+)", "", translations)
            translations = re.sub(r"({\^\^[164035iz]\d+})", "", translations)
            translations = re.sub(
                r"\s?{!?=?\d?\$?\+?_?\\?([^}]+?)\.?}", "", translations
            )
            times = 1
            while times:
                translations, times = re.subn(
                    r"[^\^]\^0*(\d{1,4})([^\d])",
                    lambda m: f"{self._get_kanji(m.group(1))}{m.group(2)}",
                    translations,
                )
            translations = re.sub(
                r"t0*(\d+)",
                lambda m: f"теперь {self._get_kanji(m.group(1))}",
                translations,
            )
            ...
            generic_translations = {
                "@9": f"{self._highlighting[0]}тж. счетный суффикс{self._highlighting[1]}",
                "@3": "",
                "@7": "",
                "@6": rus_nick,
                "@4": rus_nick,
                ...
                "@\\0": f"{self._highlighting[0]}употребляется фонетически{self._highlighting[1]}",
                "@\\8": rus_nick
                + f" {self._highlighting[0]}в сочетаниях идиоматичен{self._highlighting[1]}",
                "@\\1": rus_nick
                + f" {self._highlighting[0]}сочетания неупотребительны{self._highlighting[1]}",
                "@\\4": rus_nick,
                "@\\5": rus_nick
                + f" {self._highlighting[0]}встречается в географических названиях{self._highlighting[1]}",
                "@\\2": rus_nick
                + f" {self._highlighting[0]}сочетания малоупотребительны{self._highlighting[1]}",
            }
                ...
                for g_tr in generic_translations.keys():
                    if g_tr in translation:
                        if generic_translations[g_tr]:
                            translation = translation.replace(
                                g_tr, f" {generic_translations[g_tr]} "
                            ).strip()
                        else:
                            translation = translation.replace(g_tr, "")

                final_trs.append(
                    self._clean_text_kanji(re.sub(r"(\(\d*\))", "", translation))
                )

            return list(
                zip(reading_numbers, [fin_tr for fin_tr in final_trs if fin_tr])
            )
        ...
        for kj in tqdm(
            list(kanji.values())[2:],
            desc="[Yarxi] Updating kanji database".ljust(34),
            disable=not show_progress,
        ):
            comp_readings = {
                "0": [
                    _latin_to_hiragana(on) for on in re.split(r"[*,;)(-]", kj.on) if on
                ]
            }
            ...
            if (
                not comp_readings["0"]
                or kj.on.startswith("-")
                or kj.kanji in ["壱", "勺", "玖", "凰", "呶", "夊", "寉", "寔", "螽"]
                or not kj.rus_nick
            ):
                continue
            ...
            if len(comp_translations) > 1:
                comp_translations = _split_and_clean_compound_translations(
                    comp_translations[1], kj.rus_nick
                )
                for tr in comp_translations:
                    if not tr[0]:
                        ...
                        translation = re.sub(
                            r"\'\'(.*)\'\'",
                            lambda match: f"«{_latin_to_hiragana(match.group(1))}»",
                            translation,
                        )
                        already_there = self._in_container(
                            extension, comp_readings["0"], [kj.kanji]
                        )
                        if already_there == -1:
                            extension.append(
                                YarxiEntry(
                                    reading=[cr for cr in comp_readings["0"] if cr],
                                    lexeme=[kj.kanji],
                        ...
                        for tr_r in tr[0]:
                            already_there = self._in_container(
                                extension, comp_readings[tr_r], [kj.kanji]
                            )
                            if already_there == -1:
                                extension.append(
                                    YarxiEntry(
                                        reading=[
                                            cr for cr in comp_readings[tr_r] if cr
                                        ],
                                        lexeme=[kj.kanji],
                ...
                if re.search(r"\$\\60*(\d+)", kj.rus) is not None:
                    ...
                    single_comp_translation = [
                        self._clean_text_kanji(
                            re.search(r"=\\([а-я].+)", kj.rus).group(1)
                        )
                    ]
                elif (
                    any(special in kj.rus for special in ["@\\5", "@\\8"])
                    or re.search(r"^([^а-яА-Я]*?\d+)\*", kj.kun) is None
                ):
                    single_comp_translation = [
                        re.sub(r"^~", "", tr[1]).strip()
                        for tr in _split_and_clean_compound_translations(
                            kj.rus, kj.rus_nick
                        )
                        if tr[1]
                    ]
                elif len(
                    [reading for reading in re.split(r"[^a-zA-Z]", kj.kun) if reading]
                ) < len(kj.rus.split("/")):
                    start_id = len(
                        [
                            reading
                            for reading in re.split(r"[^a-zA-Z]", kj.kun)
                            if reading
                        ]
                    )
                    single_comp_translation = [
                        re.sub(r"^~", "", tr[1]).strip()
                        for tr in _split_and_clean_compound_translations(
                            "".join(kj.rus.split("/")[start_id:]), kj.rus_nick
                        )
                        if tr[1]
                    ]
                    ...
                    start_id = len(
                        [
                            reading
                            for reading in re.split(r"[^a-z]", kj.kun.split("|")[0])
                            if reading
                        ]
                    )
                    single_comp_translation = [
                    ...
                    single_comp_translation = [
                        n for n in kj.rus_nick.split("#")[0].split("*") if n
                    ]
                ...
                if already_there == -1:
                    extension.append(
                        YarxiEntry(
                            ...
                            lexeme=[kj.kanji],
                            translation=[
                                self._in_compounds_pref + tr
                                for tr in single_comp_translation
                            ],
                            eid=str(self._get_next_eid()),
                            references=[],
                            kanji=[kj.kanji],
        ...
        for ext in tqdm(
            extension,
            desc="[Yarxi] Expanding word database".ljust(34),
            disable=not show_progress,
        ):
            already_there = self._in_container(self._entries, ext.reading, ext.lexeme)
            if already_there != -1:
                self._entries[already_there].translation = list(
                    set(self._entries[already_there].translation + ext.translation)
                )
                self._entries[already_there].lexeme = list(
                    set(self._entries[already_there].lexeme + ext.lexeme)
                )
                self._entries[already_there].kanji = list(
                    set(self._entries[already_there].kanji + ext.kanji)
                )
                ...
                ext.translation = [
                    ...
                    + [
                        e.rus_nick
                        for e in self._kanji_db.values()
                        if e.kanji in ext.lexeme and e.rus_nick
                    ][0]
                ]
                self._entries.append(ext)
    ...
    def rescan(
        self,
        fname: str = "../dictionaries/source/yarxi_3.02.2021.db",
