...
class WarodaiDictionary:
    ...
    def lookup(
        ...
        lexeme: str,
        reading: str = "",
        search_mode: SearchMode = SearchMode.consecutive,
        order: int = 1,
    ) -> List[SearchResult]:
        if order < 1:
        ...
        if search_mode != SearchMode.shallow_only and not suitable_entries:
            ...
            if lex_kanji:
                    ...
                    if _is_kanji(char):
                        res.extend(
                            [
                                ...
                                if any(lex for lex in entry.lexeme if char in lex)
                            ]
                        )
                ...
                suitable_entries = [
                    self._get_entry_by_eid(e[1])
                    for e in weighted_res
                    if e[0] >= orders[min(len(orders) - 1, order - 1)]
                ]
            else:
                suitable_entries = [
                    e
                    for e in self._entries
                    if lexeme in e.lexeme or lexeme in e.reading
                ]
    ...
    def _translate_by_eid(self, eid: str) -> List[str]:
        def _retrieve_from_references(
            refs: [WarodaiReference], visited_references: List[str]
        ) -> List[str]:
            if not refs:
                return []
            ...
            for ref in refs:
                ...
                if ref.prefix:
                    ...
                    temp_meanings = [
                        re.sub(r"(〈.+?[〉＿]\s)", "", meaning)
                        for meaning in temp_meanings
                        if meaning.startswith(ref.prefix)
                    ]
                ...
                if ref.mode:
                    temp_meanings = [
                        ref.mode + " " + meaning for meaning in temp_meanings
                    ]

                if ref.body:
                    temp_meanings = [
                        ref.body + " " + meaning for meaning in temp_meanings
                    ]
        ...
        return sum(list(entry.translation.values()), []) + [
            m
            for m in _retrieve_from_references(
                sum(list(entry.references.values()), []), [entry.eid]
            )
            if m
        ]
    ...
    def lookup_translations_only(self, lexeme: str, reading: str = "") -> List[str]:
        return list(
            dict.fromkeys(
                sum([tr.translation for tr in self.lookup(lexeme, reading)], [])
            )
        )
    ...
    def save(self, fname: str = "default"):
        if fname == "default":
            ...
            ).parent.joinpath("dictionaries/warodai.jtdb")
...
class WarodaiLoader:
    ...
    _normalizer: {str: str} = {
        ...
        "滴ru": "shitataru 【滴る】",
        "汽車ga": "kisha 【汽車】 ga",
        "煙wo吐kinagara停車場wo出ta": "kemuri 【煙】 wo hakinagara 【吐きながら】 teishajou 【停車場】 wo deta 【出た】",
    ...
    def rescan(
        self,
        fname: str = "../dictionaries/source/warodai_22.03.2020.txt",
        show_progress: bool = True,
    ) -> WarodaiDictionary:
        self._entries, temp_entries = self._load_db(fname, show_progress)

        self._max_eid = max(
            [WarodaiEid(e.eid) for e in self._entries if e.eid.startswith("009")]
        )

        self._entries.append(
            WarodaiEntry(
                eid=self._max_eid.inc(),
                reading=["つく"],
                lexeme=["尽く"],
                translation={},
                references={
        ...
        self._entries[self._get_entry_index_by_eid("003-19-23")].references["1"][
            0
        ].eid = str(self._max_eid)
        self._entries.append(
            WarodaiEntry(
                eid=self._max_eid.inc(),
                reading=["ざいく"],
    ...
    def _extend_database(
        self, temp_entries: [WarodaiEntry], show_progress: bool = True
    ):
        entries_to_add = []
        custom_reading = {
            "尼さん": ["あまさん"],
        ...
        for entry in temp_entries:
            likely_duplicate = [
                ...
                if sorted(e.lexeme) == sorted(entry.lexeme)
                and sorted(e.reading) == sorted(entry.reading)
            ]
            if likely_duplicate:
                dup_id = entries_to_add.index(likely_duplicate[0])
                for tr_number in entry.translation:
                    entries_to_add[dup_id].translation.setdefault(tr_number, []).extend(
                        entry.translation[tr_number]
                    )
                for ref_number in entry.references:
                    entries_to_add[dup_id].references.setdefault(ref_number, []).extend(
                        entry.references[ref_number]
                    )
            else:
                entries_to_add.append(entry)
                entries_to_add[-1].eid = self._max_eid.inc()
                if not entries_to_add[-1].reading:
                    entries_to_add[-1].reading = custom_reading[
                        entries_to_add[-1].lexeme[0]
                    ]
    ...
    def _resolve_references(self, show_progress: bool = True):
        ...
        for i, entry in tqdm(
            ...
            for ref_id in entry.references:
                for j, ref in enumerate(entry.references[ref_id]):
                    ...
                    if self._entries[i].references[ref_id][j].mode:
                        self._entries[i].references[ref_id][
                            j
                        ].mode = f"{self._highlighting[0]}{self._entries[i].references[ref_id][j].mode}{self._highlighting[1]}"
                    if targ_entry is None:
                        ...
                        for m_n in ref.meaning_number:
                            if m_n not in list(targ_entry.translation.keys()) + list(
                                targ_entry.references.keys()
                            ):
                                self._entries[i].references[ref_id][
                                    j
                                ].meaning_number.remove(m_n)
                    ...
                    if ref.prefix:
                        if not any(
                            ref.prefix in tr
                            for tr in "; ".join(
                                sum(list(targ_entry.translation.values()), [])
                            ).split("; ")
                        ):
                            self._entries[i].references[ref_id][j].usable = False
            ...
            if (
                all(
                    not ref.usable
                    for ref in sum(list(self._entries[i].references.values()), [])
                )
                and not self._entries[i].translation
            ):
    ...
    def _convert_to_entry(
        self, string: str
    ) -> (List[WarodaiEntry], List[WarodaiEntry]):
        def _resolve_lexemes_and_readings(string: str) -> (List[str], List[str]):
            s_string = string.split("\n")
            rl = re.search(
                r"^(?P<reading>.*?)…?\s?(?:【(?P<lexeme>.*?)】)?\(", s_string[0]
            )
        ...
        ) -> ({str: List[WarodaiReference]}, List[str]):
            res = {}
                ...
                if "a href" in translations[i]:
                    ...
                    while True:
                        ...
                        if decomposed_tr is None:
                            translations[i] = re.sub(
                                r"; $", "", translations[i].replace(f"{body}{mode}", "")
                            )
                        ...
                        ref = WarodaiReference(
                            eid=decomposed_tr.group("eid"),
                            mode=re.sub(
                                r",?\s?см.", "", decomposed_tr.group("mode")[1:-1]
                            ),
                            body=body.strip(),
                            usable=True,
                        )
                        if decomposed_tr.group("meaning_number") is not None:
                            ref.meaning_number.extend(
                                decomposed_tr.group("meaning_number")
                                .strip()
                                .split(", ")
                            )
                        else:
                        ...
                        res.setdefault(
                            re.search(
                                r"^〔(\d+)〕", decomposed_tr.group("left_part")
                            ).group(1),
                            [],
                        ).append(ref)

                        translations[i] = translations[i].replace(
                            decomposed_tr.group("to_remove"), ""
                        )
        ...
        def _normalize_translations(translations: List[str]) -> {int: List[str]}:
            def _change_highlighting(translation: str) -> str:
                if self._highlighting != ("《", "》"):
                    return translation.replace("《", self._highlighting[0]).replace(
                        "》", self._highlighting[1]
                    )
                return translation
            ...
            values = [
                _change_highlighting(re.search(r"〔\d+〕(.+)", item).group(1))
        ...
        def _extract_translations(translations: List[str]) -> List[str]:
            ...
            for tr in translations:
                ...
                tr_temp = (
                    ...
                    .replace(
                        "(<i>как много в Китае людей по фамилии Чжан</i> 張 <i>и Ли</i> 李)",
                        "",
                    )
                    .replace(
                        "(<i>слово-каламбур, основанное на том, что</i> ななつ 七つ <i>синонимично слову</i> しち 七, "
                ...
                tr_temp = re.sub(
                    r"(?P<to_remove>\(<i>(?P<mode>[^)]+?)</i> (?P<lexeme>[^<a-zа-я0-9(]+?)\) )",
                    "",
                    tr_temp,
                )
                tr_temp = re.sub(
                    r"\(\<i\>[а-я\s]+\.?\<\/i\> [^)]+\) \(\<i\>[а-я\s]+\.?\<\/i\> [^)]+\)",
                    "",
                    tr_temp,
                )
                ...
                if (
                    not tr_temp
                    or any(_is_kanji(char) for char in tr_temp)
                    and any(_is_kanji(char) for char in re.sub(r"(【.+?】)", "", tr_temp))
                    or "на своем месте по алфавиту" in tr_temp
                ):
                    continue
                if not any(_is_hira_or_kata(char) for char in tr_temp):
                    cleaned_translations.append(tr)

            not_to_confuse_expressions = [
                r"(\(<i>не смешивать с.+?\)\s?)",
                r"(<i>не смешивать .+?</i> <a href=.+?>.+?</a>)",
                r"(\d\) <i>не смешивать с.+)",
            ]
            for exp in not_to_confuse_expressions:
                cleaned_translations = [
                    re.sub(exp, "", ct) for ct in cleaned_translations
                ]
            ...
            cleaned_translations = [
                ...
                .replace("◇", "")
                .replace(".:", ".")
                .replace('<a href="#004-81-10">二</a>', "二")
                .strip()
                for tr in cleaned_translations
            ]
            cleaned_translations = [
                re.sub(r"^(\d+\)):", r"\1", tr) for tr in cleaned_translations
            ]

            cleaned_translations = [
                tr.replace(" <i>и</i>", ",")
                .replace(" <i>а тж.</i>", "")
                .replace("<i>и ср.</i>", "<i>ср.</i>")
                .replace("</a> и <a", "</a>, <a")
                .replace(" <i>(зима)</i>", "")
                .replace(" <i>(весна)</i>", "")
                .replace(" <i>(лето)</i>", "")
            ...
            cleaned_translations = [
                re.sub(r"(^\d+\.):(\s.+)", r"\1\2", ct) for ct in cleaned_translations
            ]
                    ...
                    while True:
                        ...
                        if to_remove is not None:
                            ...
                            cleaned_translations[i] = cleaned_translations[i].replace(
                                to_remove.group("to_remove"), ""
                            )
                        else:
                            if mode:
                                cleaned_translations[i] = cleaned_translations[
                                    i
                                ].replace(f"{mode}</i> <i>и т. п.</i>", "")
                                if "<i>" in mode:
                                    cleaned_translations[i] = cleaned_translations[
                                        i
                                    ].replace(f"{mode}</i> ", "")
            ...
            cleaned_translations = [
                re.sub(r"^[.;]$", "", ct) for ct in cleaned_translations
            ]
            cleaned_translations = [
                re.sub(r"^\d+\)\s?.$", "", ct) for ct in cleaned_translations
            ]
            cleaned_translations = [
                ct.replace(" )", ")") for ct in cleaned_translations if ct
            ]

            pointed_numbers = any(
                re.search(r"^\d\.", ct) is not None for ct in cleaned_translations
            )
            bracketed_numbers = any(
                re.search(r"^\d\)", ct) is not None for ct in cleaned_translations
            )
            ...
            if (
                len(cleaned_translations) > 1
                and re.search(r"^\d[.)]", cleaned_translations[0]) is None
                and (bracketed_numbers or pointed_numbers)
            ):
                for i, _ in enumerate(cleaned_translations[1:]):
                    ...
                    if num is not None:
                        cleaned_translations[
                            i + 1
                        ] = f"{num.group(1)} {cleaned_translations[0]}{num.group(2)}"
            ...
            if bracketed_numbers or pointed_numbers:
                ...
                for i, _ in enumerate(cleaned_translations):
                    ...
                    if num is not None:
                        ...
                        cleaned_translations[i][0] in ["～", "｜", "＝", "…", "[", "＋"]
                        or _is_kanji(cleaned_translations[i][0])
                        or _is_hira_or_kata(cleaned_translations[i][0])
                        or cleaned_translations[i].startswith("<i>")
                        or re.search(r"^[а-ж]\)\s", cleaned_translations[i]) is not None
                        or re.search(r"^[а-я]+", cleaned_translations[i]) is not None
                    ):
                        cleaned_translations[
                            cur_id
                        ] = f'{cleaned_translations[cur_id]}{" " if cleaned_translations[cur_id][-1] in [";", ")", "."] else "; "}{cleaned_translations[i]}'
                        cleaned_translations[i] = ""
            ...
            if pointed_numbers and bracketed_numbers:
                ...
                for i, _ in enumerate(cleaned_translations):
                    if re.search(r"^\d\.$", cleaned_translations[i]) is not None:
                        ...
                        cur_aff = re.search(
                            p_num_expression, cleaned_translations[i]
                        ).group(1)
                        add_meaning = re.search(
                            p_num_expression, cleaned_translations[i]
                        ).group(2)[1:]
                        aff_id = i
                    elif (
                        re.search(b_num_expression, cleaned_translations[i]) is not None
                    ):
                        if not add_meaning:
                            cleaned_translations[
                                i
                            ] = f'{cur_aff}{"" if cleaned_translations[i][0].isnumeric() else " "}{cleaned_translations[i]} '
                        else:
                            tmp1 = re.search(
                                b_num_expression, cleaned_translations[i]
                            ).group(1)
                            tmp2 = re.search(
                                b_num_expression, cleaned_translations[i]
                            ).group(2)
                            cleaned_translations[
                                i
                            ] = f'{cur_aff}{"" if cleaned_translations[i][0].isnumeric() else " "}{tmp1} {add_meaning}{tmp2} '
                        cleaned_translations[aff_id] = ""
                cleaned_translations = [ct for ct in cleaned_translations if ct]

            if (
                not pointed_numbers
                and not bracketed_numbers
                and len(cleaned_translations) > 1
            ):
                if re.search(r"^<i>[^</i>]+</i>$", cleaned_translations[0]) is not None:
                    cleaned_translations = [
                        f"{cleaned_translations[0]} {tr}"
                        for tr in cleaned_translations[1:]
                    ]

            cleaned_translations = [
                re.sub(r"(\s[а-ж\d]+\)\s)", " ", tr)
                for tr in cleaned_translations
                if tr
            ]
            ...
            if pointed_numbers or bracketed_numbers:
                ...
                for i, _ in enumerate(cleaned_translations):
                    tr_idx = re.search(
                        r"^(?P<number>\d+)[\d).]+\s(?P<value>.*)",
                        cleaned_translations[i],
                    )
                    if tr_idx is not None:
                        cleaned_translations[
                            i
                        ] = f'〔{tr_idx.group("number")}〕{tr_idx.group("value")}'
                        cur_val = i
                    elif i > 0:
                        if cleaned_translations[i][0] in ["～", "＊", "＝", "＋"]:
                            cleaned_translations[cur_val] += (
                                " " + cleaned_translations[i]
                            )
                            cleaned_translations[i] = ""
                        else:
                            alpha_pref = re.search(
                                r"^(?P<a_p>[а-ж]\))(?P<rest>\s.+)",
                                cleaned_translations[i],
                            )
                            if alpha_pref is not None:
                                cleaned_translations[cur_val] += alpha_pref.group(
                                    "rest"
                                )
                                cleaned_translations[i] = ""
            else:
            ...
            cleaned_translations = [
                re.sub(
                    r"(?:<i>)(?P<op_pr>\(?)(?P<body>.+?)(?P<punct>[:;]?)(?P<cl_pr>\)?)(?:<\/i>)",
                    lambda m: f'《{m.group("op_pr")}{m.group("body")}{m.group("cl_pr")}》',
                    tr,
                )
                for tr in cleaned_translations
            ]
            ...
            if any("》" in t for t in cleaned_translations):
                quotes = [
                    (re.search(r"《([^》]+)》$", t), cleaned_translations.index(t))
                ...
                for i in quotes:
                    cleaned_translations[i] = re.sub(
                        r"\.》$", "》", cleaned_translations[i]
                    )

            cleaned_translations = [
                re.sub(r"(\(《[^\(\)]+》\))", lambda m: m.group(1)[1:-1], ct)
                for ct in cleaned_translations
            ]
            cleaned_translations = [
                re.sub(r"(《\([^\(\)]+\)》)", lambda m: f"《{m.group(1)[2:-2]}》", ct)
                for ct in cleaned_translations
            ]
            cleaned_translations = [
                ct.replace(", ср.》", "》") for ct in cleaned_translations
            ]
            cleaned_translations = [
                re.sub(r"《«([^《«»》]+)»》", r"《\1》", ct) for ct in cleaned_translations
            ]
            cleaned_translations = [
                ct.replace(".》 《", ". ") for ct in cleaned_translations
            ]
            cleaned_translations = [
                ct.replace("》; 《", "; ") for ct in cleaned_translations
            ]
            ...
            for i, _ in enumerate(cleaned_translations):
                ...
                for j, kana_part in enumerate(
                    sorted([re.split(r"([…～＝｜＋＊])", k) for k in old_kp], reverse=True)
                ):
                    kana_part = [part for part in kana_part[1:] if part]
                    kp_bullets = [
                        (self._bullet_normalizer[part], part)
                        for part in kana_part
                        if kana_part.index(part) % 2 == 0
                    ]
                    kp_collocs = [
                        (self._normalize_kana(part), part)
                        for part in kana_part
                        if kana_part.index(part) % 2 == 1
                    ]
                    for kp1, kp2 in list(zip(kp_bullets, kp_collocs)):
                        cleaned_translations[i] = cleaned_translations[i].replace(
                            f"{kp1[1]}{kp2[1]}", f"〈{kp1[0]}{kp2[0]}〉"
                        )
                if old_kp:
                    cleaned_translations[i] = cleaned_translations[i].replace(
                        "〈-", "…〈-"
                    ...
                    cleaned_translations[i] = re.sub(
                        r"〈\|([^〈]+?)〉～?", r"＿≪-\1≫", cleaned_translations[i]
                    )
                    cleaned_translations[i] = cleaned_translations[i].replace(
                        "〉〈~", "〉＿〈~"
                    )
        ...
        global_quasi_reference = re.search(
            r"^<i>(?P<mode>[^)(]+?)<\/i> (?P<lexeme>[^<a-zA-Zа-яА-Я0-9(～]+?)$",
            string.split("\n")[1],
        )
        if global_quasi_reference is not None:
            mode = ""
            if global_quasi_reference.group("mode") in [
                "уст.",
                "неправ.",
                "кн.",
                "редко",
        ...
        if translations:
            ...
            if global_quasi_reference is not None and mode and mode != "ignore":
                if translations:
                    temp_translations = [
                        re.sub(r"〔\d+〕", "", tr) for tr in translations
                    ]
                    temp_res.append(
                        WarodaiEntry(
                            lexeme=re.sub(
                                r"/.+?/", "", global_quasi_reference.group("lexeme")
                            ).split(", "),
                            eid="||",
                            reading=_resolve_ref_reading(
                                readings, global_quasi_reference.group("lexeme")
                            ),
                            translation={
                                "1": [f"{mode}{tr}" for tr in temp_translations]
                            },
                            references={},
                        )
                    )
            ...
            if referenced_translations:
                for r, t in referenced_translations:
                        ...
                        if part.group("mode") in ["тк.", "тж."]:
                            ...
                            temp_res.append(
                                WarodaiEntry(
                                    ...
                                    translation={
                                        "1": [
                                            f'《{part.group("mode").replace("тк.", "").strip()}》 '
                                            + temp_tr
                                        ]
                                    },
                                    references={},
                    ...
                    if "тк." in [p.group("mode") for p in parts]:
                        ...
                        for part in parts:
                            translations[t_id] = translations[t_id].replace(
                                f"({part.group('to_remove')}) ", ""
                            )

            quasi_references = [
                re.search(
                    r"(?P<to_remove>\(《(?P<mode>[^)》]+?)》 (?P<lexeme>[^<a-zа-я0-9(]+?)\) )",
                    tr,
                )
                for tr in translations
            ]
            ...
            if referenced_translations:
                for r, t in referenced_translations:
                    if (
                        all(
                            new_lex in lexemes
                            for new_lex in r.group("lexeme").split(", ")
                        )
                        and "тк." not in r.group("mode")
                        or r.group("mode")
                        in [
                            "о своем тк.",
                            "о себе тк.",
                        ...
                        if r.group("mode") in [
                            ...
                            translations[translations.index(t)] = t.replace(
                                r.group("lexeme"),
                                self._normalize_kana(r.group("lexeme")),
                            )
                        continue
                    if r.group("mode") not in [
                        "вм.",
                        "сокр.",
                        "от",
                        "от первой буквы слова",
                    ] and "форм" not in r.group("mode"):
                        ...
                        if special_reading is not None:
                            ref_reading = [
                                JTran.transliterate_from_kana_to_hira(
                                    special_reading.group(1)
                                )
                            ]
                        else:
                            ref_reading = _resolve_ref_reading(
                                readings, r.group("lexeme")
                            )
                        temp_res.append(
                            WarodaiEntry(
                                lexeme=re.sub(r"/.+?/", "", r.group("lexeme")).split(
                                    ", "
                                ),
                                eid="|",
                                reading=ref_reading,
                                translation={
                                    "1": [
                                        re.sub(
                                            r"〔\d+〕",
                                            "",
                                            t.replace(r.group("to_remove"), mode),
                                        )
                                    ]
                                },
                                references={},
                        ...
                        if "тк." in r.group("mode"):
                            ...
                            translations[translations.index(t)] = t.replace(
                                r.group("to_remove"), replace_with
                            )
                    else:
                        t_id = translations.index(t)
                        translations[t_id] = re.sub(
                            r"(/[^\s]+?/)", "", translations[t_id]
                        )
                        translations[t_id] = t.replace(
                            r.group("lexeme"),
                            " + ".join(
                                [
                                    self._normalize_kana(lx)
                                    for lx in r.group("lexeme").split(", ")
                                ]
                            ),
                        )
        ...
        if references or translations:
            res.append(
                WarodaiEntry(
                    ...
                    reading=readings,
                    lexeme=lexemes,
                    translation=_normalize_translations(translations),
    ...
    def _load_db(
        self, fname: str, show_progress: bool = True
    ) -> (List[WarodaiEntry], List[WarodaiEntry]):
        with open(fname, encoding="utf-16le") as f:
            contents = f.read()

            to_replace = [
                ("<i>искажённое</i>", "<i>искаж.</i>"),
                ("[…", "…["),
                ("…[を, …に]", "…[を(に)]"),
                ...
                (
                    "…を記録に残す, …の記録をとる <i>см. выше</i> ～する;",
                    "を記録に残す, の記録をとる <i>см. выше</i> する",
                ),
                ("(<i>в конце предл. после</i> かも)", "…かも～ <i>в конце предл.</i>"),
                (
                    "(<i>как 2-й компонент сложн. сл.</i> ざいく) [мелкие] изделия, поделки;",
                    "ざいく",
                ),
                ...
                ("ぽたり落ちる", "～落ちる"),
                ("ぽたり滴る", "～滴る"),
                ("汽車がぽっぽと煙を吐きながら停車場を出た", "＝汽車が～煙を吐きながら停車場を出た"),
                ...
                ("むっくり肥えた", "～肥えた"),
                ("むにゃむにゃ言う", "～言う"),
                ("…しようという気がむらむら[と]起る", "…しようという気が～[と]起る"),
                ...
                ("めりめりいう", "～いう"),
                ("めりめり音がして倒れる", "～音がして倒れる"),
                ("もぐもぐ[と]云う, 口をもぐもぐさせる", "～[と]云う, ＝口を～させる"),
                ("もぐもぐ嚙む", "～嚙む"),
                ("もぐもぐ[と]動く", "～[と]動く"),
                ("勿体をつける", "～をつける"),
                ("物心が付く", "～が付く"),
                ...
                ("◇夜目遠目/トウメ/傘の中/ウチ/", "夜目遠目傘の中"),
                ("よろしきを得る", "～を得る"),
                ("寛大よろしきを得る", "＝寛大～を得る"),
                ("経営よろしきを得る", "＝経営～を得る"),
                ("李下に冠/カンムリ/を正さず, 李下の冠", "李下に冠を正さず, 李下の冠"),
                ("俚耳に入り易/イリヤス/い", "～に入り易/イリヤス/い"),
                ("俚耳に入りにくい", "～に入りにくい"),
                ("槍を(太刀を)りゅうりゅうとしごく", "＝槍(太刀)を～しごく"),
                ("両手ききの人", "～の人"),
                ("両天秤をかける", "～をかける"),
                ("釐亳も", "～も"),
                ("釐毫も違わない", "～も違わない"),
            ...
            for src, trg in tqdm(
                to_replace,
                desc="[Warodai] Preprocessing raw source".ljust(34),
                disable=not show_progress,
            ):
            ...
            for c in tqdm(
                s_contents,
                desc="[Warodai] Building word database".ljust(34),
                disable=not show_progress,
            ):
