...
class smp_reservation_compute(models.Model):
  ...
  startTime = fields.Datetime(string=_("Start"),readonly=True)
  endTime = fields.Datetime(string=_("End"),readonly=True)
  effectiveStartTime = fields.Datetime(string=_("Effective Start"),readonly=True)
  effectiveEndTime = fields.Datetime(string=_("Effective End"),readonly=True)
  duration = fields.Float(string=_("Duration"))
  effectiveDuration = fields.Float(string=_("Effective Duration"))
  lifetimeDurationMins = fields.Float(string=_("Lifetime mins"),_compute="_get_lifetimeDurationMins")
  ...
  related_company_object = fields.Many2one('res.partner', string=_("Company (Relation)"), compute="_set_company_object")
  credits = fields.Float(_("Credits"))
  price = fields.Float(_("Price"))
  cs_group = fields.Char(_("Group (app id)"))
  cs_group_id = fields.Many2one('smp.sm_group',_("Group"),compute="_set_cs_group_id")
  ...
  creation_date_app = fields.Datetime(string=_("Creation Date"),readonly=True)
  lastmodified_date_app = fields.Datetime(string=_("LastModified Date"),readonly=True)
  ...
  def _get_lifetimeDurationMins(self):
    for record in self:
      record.lifetimeDurationMins = (record.effectiveEndTime - record.creation_date_app).total_seconds()/60
      ...
      if 'active_ids' in self.env.context:
        ...
        if computes.exists():
          for compute in computes:
            compute._fetch_update_reservation_data(forced_minimal_update=False)
  ...
  def fetch_force_update_reservation_data_from_action(self):
    if self.env.context:
      if 'active_ids' in self.env.context:
        ...
        if computes.exists():
          for compute in computes:
            compute._fetch_update_reservation_data(forced_minimal_update=True)
  ...
  def _fetch_update_reservation_data(self,forced_minimal_update=False):
    if self.startTime and self.endTime:
      self.env["smp.sm_reservation_wizard"].compute_reservations(
        parent=self, from_q=self.startTime, till_q=self.endTime,forced_minimal_update=forced_minimal_update,update_self=True)
