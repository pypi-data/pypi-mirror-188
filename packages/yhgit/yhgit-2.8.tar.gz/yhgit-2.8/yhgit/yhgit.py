...
def commit_branch(module_list, module_f_path, msg):
    ...
    """
    commit_result = []
    bar = Bar('commit...', fill="*", max=len(module_list))
...
def pull_branch(module_list, module_f_path):
    ...
    index = 0
    pull_result = []
    bar = Bar('pull...', fill="*", max=len(module_list))
    for module in module_list:
        bar.next()
        filename = module.module
        module_path = module_f_path + filename + "/"
...
def push_branch(module_list, module_f_path):
    ...
    index = 0
    pull_result = []
    bar = Bar('push...', fill="*", max=len(module_list))
    for module in module_list:
        bar.next()
        filename = module.module
        module_path = module_f_path + filename + "/"
...
def merge_branch(module_list, convert_list, module_f_path, from_branch):
    ...
    index = 0
    pull_result = []
    bar = Bar('merge...', fill="*", max=len(module_list))
...
def release_branch(module_list, include_list, tag_path, c_path, f_branch):
    ...
    index = 0
    merge_result = []
    bar = Bar('release...', fill="*", max=len(module_list))
    for a in module_list:
        bar.next()
        if not (a.branch and len(a.branch) > 0) and not (a.tag and len(a.tag) > 0):
            a.branch = f_branch
        if a.branch and len(a.branch) > 0:
            if (len(include_list) > 0 and a.module in include_list) or len(include_list) == 0:
...
class yhgit:
    ...
    def status(self):
        ...
        status_result = []
        bar = Bar('status...', fill="*", max=len(module_list))
        for module in module_list:
            bar.next()
            result = 0
            branchname = ''
