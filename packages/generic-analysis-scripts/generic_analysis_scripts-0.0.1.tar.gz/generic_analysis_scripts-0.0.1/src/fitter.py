...
class zfitter:
    ...
    def _set_pdf_pars(self, res):
        ...
        for par in l_par:
            ...
            par.set_value(val)
    #------------------------------
    def fit(self, ntries=None, pval_threshold = 0.05):
        '''
        ...
        if self._pdf.is_extended:
            nll = zfit.loss.ExtendedUnbinnedNLL(model=self._pdf, data=self._data_zf)
        else:
            nll = zfit.loss.UnbinnedNLL(model=self._pdf, data=self._data_zf)
