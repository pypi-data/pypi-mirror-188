...
class FixedAsset(Asset):
    ...
    def __init__(self, account,debit,date,purchase_price, taxes, transportation_costs, installation_costs, professional_services_costs,life_span,location):
        ...
        if self.debit !=self.purchase_price + self.taxes + self.transportation_costs + self.installation_costs + self.professional_services_costs:
            raise ValueError("debit must be the sum of purchase_price、taxes、transportation_costs、installation_costs and professional_services_costs")
        if life_span < 1:
    ...
    def depreciate(self, rate):
        ...
        if self.age < self.life_span:
            ...
            for equity in FixedAsset.accounts:
                if equity.account == self.account:
                    equity.debit -= self.debit
            self.age += 1
        else:
...
class RealState(FixedAsset):
    pass
def main():
    a12=FixedAsset("zhang1",11500,"2022-02-21",11500,0,0,0,0,0,"beijing")
    print(a12.debit)
    print(a12.location)
    print(a12.likely_economic_benefit)
    a22=FixedAsset("zhang2",11500,"2022-02-21",11500,0,0,0,0,0,"beijing")
    a22.depreciate(0.33)
    a22.depreciate(0.33)
    a22.depreciate(0.33)
