...
class Equity(FinancialInstrument):
    accounts = []
    def __init__(self, account, credit,date,parties, consideration, value):
        super().__init__(self,parties, consideration, value)
        self.credit=credit
    ...
    @classmethod
    def sum(cls):
        data = [[asset.account, asset.date, asset.value] for asset in Equity.accounts]
        df = pd.DataFrame(data, columns=['账户类别', '日期', '贷方金额'])
        return df

def main():
    a=Equity("jack",200,"2025-01-01",None,None,22000)
    b=Equity("jack1",200,"2025-01-01",None,None,20000)
