...
exec("def dummy_Handler(data = None): return None", handler.__dict__)
...
class EurekaTestModel(MLSGenericModel, MLSTrainableCustom):
    ...
    def __init__(
        ...
        handler_func_nm: str = "dummy_Handler",
    ):
        ...
        if handler_code:
            ...
            assert isinstance(handler_func_nm, str), "`handler_func_nm`는 string 타입이어야 합니다."
        else:
            handler_code = "def dummy_Handler(data = None): return None"

        handler_code = LogicChecker(handler_code)
        handler = imp.new_module("Handler")
        exec(handler_code, handler.__dict__)
        sys.modules["Handler"] = handler
        module = __import__("Handler")
        self.handler_func = getattr(module, handler_func_nm)
        self.handler_code = handler_code
        self.handler_func_nm = handler_func_nm
    ...
    def _postprocess(
        ...
        try:
            ...
            raise MLSModelError(f"EurekaModel: Logic Converter 후처리에 실패했습니다. {e}")
