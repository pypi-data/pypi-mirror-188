...
def write_crl_der(crl: crypto.CRL, filepath: str, overwrite: bool = False):
    """
    Write the Certificate Revocation List in DER format into the provided filepath.

    If the file already exists, a RuntimeError is raised except if the overwrite parameter is set to True
    ...
    with open(filepath, 'wb') as f:
        f.write(crypto.dump_crl(crypto.FILETYPE_ASN1, crl))
...
def dt_to_asn1(dt: datetime.datetime) -> str:
    """
    Convert the datetime to a ASN.1 TIME string.
    """
