...
from .models import CA, Config, ConfigType, Certificate, CRL, database, CertificateRevokedReason, DATABASE_FILENAME
from .utils import ask_pem_format, ask_private_key_passphrase, get_csr_san, generate_random_serial_number, \
     ...
     ask_password, ask_date_in_future, nb_seconds_to_date, print_tabulated, dt_to_asn1, write_crl_der, \
     get_csr_cn, write_cert_pem, write_private_key_pem, write_p12, ask_cn_and_san
...
class CAManager:
    ...
    def create_vault(self, vault_password: Optional[str] = None) -> None:
        ...
        with database:
            ...
            Config.create(
                name='crl_last_update',
                type=ConfigType.EPOCH,
                value=0
            )
    ...
    def _revoke(self, certificate: Certificate, revoke_reason: CertificateRevokedReason,
        ...
        certificate.revoked_timestamp = datetime.datetime.now()
        certificate.revoked_reason = revoke_reason.value.encode('utf8')
        certificate.revoked_comment = revoke_comment

        certificate.save()

        print('The certificate was successfully revoked.')

    @database.connection_context()
    def crl_and_index(self):
        now = datetime.datetime.now()

        indexfile_content = ''

        crl = crypto.CRL()
        crl.set_lastUpdate(dt_to_asn1(self._get_config('crl_last_update')).encode('utf8'))

        for c in Certificate.select():
            # index
            if c.is_revoked:
                index_line = 'R'
            elif c.not_after <= now:
                index_line = 'E'
            else:
                index_line = 'V'

            index_line += f'\t{dt_to_asn1(c.not_after)}\t'

            if c.is_revoked:
                index_line += dt_to_asn1(c.revoked_timestamp)

            index_line += f'\t{c.serial.replace(":", "")}\tunknown\t'

            c.load_cert()
            cert_subject = ''.join('/{:s}={:s}'.format(name.decode(), value.decode()) for name, value in
                                   c.cert_obj.get_subject().get_components())

            index_line += f'{cert_subject}\n'
            indexfile_content += index_line

            # CRL
            if c.is_revoked:
                revoked = crypto.Revoked()
                revoked.set_serial(c.serial.replace(':', '').encode('utf8'))
                revoked.set_rev_date(dt_to_asn1(c.revoked_timestamp).encode('utf8'))

                if c.revoked_reason and c.revoked_reason != CertificateRevokedReason.UNSPECIFIED:
                    revoked.set_reason(c.revoked_reason.encode('utf8'))
                else:
        ...
        # Sign the CRL
        crl.sign(self.ca.cert_obj, self._get_ca_key(), b'sha256')

        # Write the CRL as DER in the configured output file
        crl_output_dir = self._get_config('crl_output_dir')

        output_path = os.path.join(crl_output_dir, 'crl.crl')
        write_crl_der(crl, output_path, overwrite=True)

        # Write the index file
        output_path = os.path.join(crl_output_dir, 'index.txt')
        with open(output_path, 'w+') as index_file:
            index_file.write(indexfile_content)

        # Update the last update CRL timestamp
        self._set_config('crl_last_update', int(datetime.datetime.now().timestamp()))

        print(f'The CRL and the index have been successfully generated and written to "{crl_output_dir}".')
    ...
    def _get_config(self, config_name: str):
        ...
        if c.type == ConfigType.INT:
            ...
            return datetime.datetime.utcfromtimestamp(int(c.value))
        else:
