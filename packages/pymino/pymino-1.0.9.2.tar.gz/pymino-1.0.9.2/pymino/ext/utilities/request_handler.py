...
class RequestHandler:
    ...
    def print_response(self, response: HTTPResponse) -> None:
        ...
        """
        colors = {"GET": Fore.GREEN, "POST": Fore.YELLOW, "DELETE": Fore.RED}
        debug = {False: None, True: f"{colors[response.request.method]}{Style.BRIGHT}"}
        if debug.get(self.bot.debug):
            print(
                f"{debug[True]}{response.request.method} {response.url} {response.status_code}{Style.RESET_ALL}"
            )
