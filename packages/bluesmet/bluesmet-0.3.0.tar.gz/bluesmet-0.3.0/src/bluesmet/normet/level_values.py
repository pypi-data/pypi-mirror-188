...
class LevelValues:
    ...
    def get_level_values(self,date: datetime, url, values: dict):
        ...
        if not os.path.exists(date_cache_path):
            ...
            self.__download(url, requested_values, new_values, level_name, ilon, ilat, date_cache_path)
        else:
            missing = {}
            with nc.Dataset(date_cache_path,"r") as fcache:
                for key in new_values.keys():
                    if key not in fcache.variables:
                        ...
                        new_values[key] = fcache.variables[key][:]
            if len(missing) > 0:
                self.__append(url, missing, level_name, ilon, ilat, date_cache_path)
                new_values.update(missing)

        LevelValues.__concatenate_arrays(values, new_values)

    def __download(self, url, requested_values, new_values, level_name, ilon, ilat, date_cache_path):
    ...
    def __append(self, url, missing, level_name, ilon, ilat, date_cache_path):
        with nc.Dataset(url,"r") as f0:
