...
class TimeValues:
    ...
    def get_values(self,date: datetime, url, values):
        ...
        if not os.path.exists(date_cache_path):
            ...
            self.__download(url, requested_values, new_values, ilon, ilat, date_cache_path)
        else:
            missing = {}
            ...
            if len(missing) > 0:
                self.__append(url, missing, ilon, ilat, date_cache_path)
                new_values.update(missing)

        TimeValues.__concatenate_arrays(values, new_values)

    def __download(self, url, requested_values, new_values, ilon, ilat, date_cache_path):
