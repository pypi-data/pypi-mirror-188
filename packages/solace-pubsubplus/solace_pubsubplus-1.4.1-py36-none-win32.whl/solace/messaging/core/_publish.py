...
class _SerializedSolacePublisher(_BasicSolacePublisher, _AsyncSolacePublisher):
    ...
    def start(self):
        self._send_executor = \
            _ThreadingUtil.create_serialized_dispatcher(f"send_executor_thread-{str(hex(id(self._messaging_service)))}")
    ...
    def may_block(self):
        ...
        if self._messaging_service.session.transport_state is not _SolaceTransportState.DOWN:
            self._messaging_service.session.can_send_received.clear()
    ...
    def _on_can_write(self):
        ...
        self._messaging_service.session.can_send_received.set()
    ...
    def is_writable(self) -> bool:
        ...
        return self._messaging_service.session.can_send_received.is_set()

    def wait_for_writable(self):
        # blocks calling thread for writable signal if one is needed
        self._messaging_service.session.can_send_received.wait()
...
class _SolacePublish:  # pylint: disable=missing-class-docstring, missing-function-docstring
    ...
    def publish(self, solace_message: _SolaceMessage, topic: 'Topic') -> int:
        ...
        publish_message_status_code = solace_message.send_msg(self._messaging_service.session)
        if logger.isEnabledFor(logging.DEBUG):  # pragma: no cover # Ignored due to log level
    ...
    def publish_request(self, reply_msg_p, solace_message, topic,
        ...
        # """
        self.__set_topic(solace_message, topic)
        return solace_message.send_request(self._messaging_service.session, reply_msg_p, reply_timeout)
