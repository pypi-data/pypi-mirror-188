...
import logging
from abc import ABC
from solace.messaging.receiver.inbound_message import InboundMessage, ReplicationGroupMessageId
...
class ReplayStrategy(ABC):
    """
        ...
        - :py:class:`AllMessagesReplay <solace.messaging.config.replay_strategy.AllMessagesReplay>`: Will replay all the messages
          available in the replay log.
        - :py:class:`TimeBasedReplay <solace.messaging.config.replay_strategy.TimeBasedReplay>`: Will
          replay the messages from a specified date and time.
        - :py:class:`ReplicationGroupMessageIdReplay <solace.messaging.config.replay_strategy.ReplicationGroupMessageIdReplay>`: Will
          replay messages after a specified Replication Group Message Id.
    ...
    def all_messages() -> 'AllMessagesReplay':
        ...
        Returns:
            A new instance of the class :py:class:`AllMessagesReplay<solace.messaging.config.replay_strategy.AllMessagesReplay>`.
        """
    ...
    def time_based(replay_date: datetime.datetime) -> 'TimeBasedReplay':
        ...
        An example building the :py:class:`TimeBasedReplay<solace.messaging.config.replay_strategy.TimeBasedReplay>` strategy using a datetime object::
    ...
    def replication_group_message_id_based(replication_group_message_id: 'ReplicationGroupMessageId') \
        ...
        There are two ways to acquire :py:func:`ReplicationGroupMessageId <solace.messaging.receiver.inbound_message.InboundMessage.get_replication_group_message_id>`
	for this method:
        ...
        - Using a Replication Group Message ID created from the factory method :py:func:`ReplicationGroupMessageId.of() <solace.messaging.receiver.inbound_message.ReplicationGroupMessageId.of>`
...
class TimeBasedReplay(ReplayStrategy):
    ...
    def get_replay_date(self) -> datetime.datetime:
        """
        Returns:
            The date used to construct this :py:class:`TimeBasedReplay<solace.messaging.config.replay_strategy.TimeBasedReplay>` object.
        """
