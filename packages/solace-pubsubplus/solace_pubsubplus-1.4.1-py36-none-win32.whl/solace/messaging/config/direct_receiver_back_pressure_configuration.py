...
class DirectReceiverBackPressureConfiguration(ABC):
    ...
    def on_back_pressure_elastic(self) -> 'DirectReceiverBackPressureConfiguration':
        ...
        On memory exhaustion receiver behaviour is undefined. Elastic, essentially no, back-pressure is an ideal strategy
    ...
    def on_back_pressure_drop_latest(self, buffer_capacity: int) -> 'DirectReceiverBackPressureConfiguration':
        """
        Configures the publisher with capacity-bounded, buffered back-pressure. If the buffer is full and is still receiving 
        messages, the incoming message will be discarded. The maximum capacity of the buffer is given by the parameter of 
        this method, ``buffer_capacity``.
    ...
    def on_back_pressure_drop_oldest(self, buffer_capacity: int) -> 'DirectReceiverBackPressureConfiguration':
        """
        Configures the publisher with capacity-bounded, buffered back-pressure. If the buffer is full and is still receiving 
        messages, the oldest message in the buffer is discarded. The maximum capacity of the buffer is given by the parameter 
        of this method, ``buffer_capacity``.
