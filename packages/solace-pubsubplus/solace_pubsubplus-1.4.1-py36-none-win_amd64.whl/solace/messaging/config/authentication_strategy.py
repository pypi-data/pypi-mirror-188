...
class ClientCertificateAuthentication(AuthenticationStrategy):
    ...
    def with_user_name(self, username: str) -> 'ClientCertificateAuthentication':
        ...
        Returns:
            ClientCertificateAuthentication: The instance of the object with the username configured. This object can be used for
            method chaining.
        """
...
class OAuth2(AuthenticationStrategy):
    ...
    def __init__(self, oidc_id_token: str=None, access_token: str=None, issuer_identifier: str=None):
        ...
        if issuer_identifier is not None:
            self._authentication_configuration[authentication_properties.SCHEME_OAUTH2_ISSUER_IDENTIFIER] = issuer_identifier
            if logger.isEnabledFor(logging.DEBUG):  # pragma: no cover
                logger.debug('Added issuer identifier to authentication configuration dictionary.')


    @staticmethod
    def of(access_token: str, oidc_id_token: str, issuer_identifier: str = None) -> 'OAuth2':
        """
        ...
        if oidc_id_token is not None:
            if not is_type_matches(actual=oidc_id_token, expected_type=str, raise_exception=False) \
                ...
                logger.error(f"Encountered error while instantiating OAuth2 authentication strategy: " \
                             f"{exception_message}")
        ...
        if oidc_id_token is None and access_token is None:
            ...
            logger.error("Encountered error while instantiating OAuth2 authentication strategy: {exception_message}")
            raise IllegalArgumentError(exception_message)
...
class Kerberos(ABC):
    ...
    To implement Kerberos authentication for clients connecting to a Solace PubSub+ event broker, the following
