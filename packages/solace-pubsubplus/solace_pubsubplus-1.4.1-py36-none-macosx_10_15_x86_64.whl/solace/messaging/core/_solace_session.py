...
class _SolaceSession(_SolacePublisherEventEmitter, _SolacePublisherAcknowledgementEmitter,
    ...
    def emit_receiver_event(self, event: '_SolaceReceiverEvent', error_info: dict):
        handlers = []
        with self._mutex:
    ...
    def disconnect(self) -> (int, Union[dict, None]):
        ...
        self._emit_transport_event(
            ...
            _SolaceTransportEventInfo(self._host, event_message[CCSMP_INFO_CONTENTS], event_message))
        return return_code, error_info
    ...
    def __do_connect(self) -> int:
        ...
        if return_code == SOLCLIENT_OK:
            ...
            error = self.__handle_exception_message(return_code=return_code, caller_desc='do_connect')
    ...
    def __handle_exception_message(self, return_code, caller_desc='SessionConnect'):  # pylint: disable=no-self-use
        ...
        if core_sub_code == CCSMP_SUBCODE_UNTRUSTED_CERTIFICATE:
            ...
            return PubSubPlusCoreClientError(message=f'{UNTRUSTED_CERTIFICATE_MESSAGE} {core_exception_msg}',
                                            sub_code=info_sub_code)
            ...
            return PubSubPlusCoreClientError(message=f'{FAILED_TO_LOADING_CERTIFICATE_AND_KEY} {core_exception_msg}',
                                            sub_code=info_sub_code)
        elif core_sub_code == CCSMP_SUB_CODE_LOGIN_FAILURE:
            self.adapter.warning('%s %s', BAD_CREDENTIALS, core_exception_msg)
            return AuthenticationError(message=f'{BAD_CREDENTIALS} {core_exception_msg}')
        self.adapter.warning('%s', core_exception_msg)
        return PubSubPlusCoreClientError(message=core_exception_msg, sub_code=info_sub_code)
    ...
    def __service_event_callback_routine \
        ...
        if event == SOLCLIENT_SESSION_EVENT_UP_NOTICE:
            ...
            self._emit_transport_event(
                ...
                _SolaceTransportEventInfo(self._host, message, error_info))
        ...
        # TODO handle SOLCLIENT_SESSION_EVENT_RX_MSG_TOO_BIG_ERROR
            ...
            #TODO this should be handled in the publisher not here
            if error_info[CCSMP_SUB_CODE] != CCSMP_SUB_CODE_OK:
