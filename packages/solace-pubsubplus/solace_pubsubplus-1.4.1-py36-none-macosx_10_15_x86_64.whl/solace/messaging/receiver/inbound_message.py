...
class InboundMessage(Message):
    ...
    class MessageDiscardNotification(ABC):
        ...
        def has_internal_discard_indication(self) -> bool:
            """
            Retrieves the internal discard indication. A receiving client can use a message discard indication method or
            function to query whether Python API has for any reason discarded any messages previous to the current 
            received message.
    ...
    def __str__(self):
        ...
        if return_code == SOLCLIENT_OK:
            try:
                ...
                return repr(self)
        elif return_code == SOLCLIENT_FAIL:
            return repr(self)
