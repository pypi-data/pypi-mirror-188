...
class OutboundMessageBuilder(PropertyBasedConfiguration):
    ...
    def with_sender_id(self, sender_id: Union[str, None]) -> 'OutboundMessageBuilder':
        """
        Sets the sender ID for a message from a string, or deletes it if None is passed. Note:
        if :py:const:`GENERATE_SENDER_ID<solace.messaging.config.solace_properties.service_properties.GENERATE_SENDER_ID>`
