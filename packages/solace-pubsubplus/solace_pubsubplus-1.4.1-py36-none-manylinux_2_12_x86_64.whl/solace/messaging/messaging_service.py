...
from solace.messaging.builder._impl._transactional_message_publisher_builder import _TransactionalMessagePublisherBuilder
...
from solace.messaging.config._sol_constants import SOLCLIENT_SESSION_PROP_DEFAULT_COMPRESSION_LEVEL, \
    ...
    SOLCLIENT_SUB_CODE_OK
from solace.messaging.config._solace_message_constants import CCSMP_SUB_CODE, CCSMP_INFO_SUB_CODE, TRANSACTIONAL_MESSAGE_SERVICE_NOT_ATTEMPTED_TO_CONNECT
...
from solace.messaging.config.solace_properties.transactional_service_properties import TRANSACTIONAL_SERVICE_REQUEST_TIMEOUT
...
from solace.messaging.errors.pubsubplus_client_error import InvalidDataTypeError, \
    ...
    UnknownTransactionStateError, TransactionRollbackError, TransactionError
...
__all__ = ['MessagingServiceClientBuilder', 'ServiceEvent', 'MessagingService', 'ServiceInterruptionListener',
           'ReconnectionListener', 'ReconnectionAttemptListener', 'RequestReplyMessagingService', 'TransactionalMessagingService',
...
class ListenerThread(_SolaceThread):  # pylint: disable=missing-class-docstring, missing-function-docstring
    ...
    def __init__(self,
                 owner_info: str, logger: 'logging.Logger', listener_queue: '_PubSubPlusQueue', *args, **kwargs):
        super().__init__(owner_info, logger, *args, **kwargs)
    ...
    def run(self):
        ...
        while self._is_running.is_set():
            try:
                ...
                self.adapter.debug(f"Listener queue was shutdown with the following messge: {str(e)}. Executing remaining listener callbacks now.")
                break
    ...
    def commit(self):
        """
          Commits all messages based on this following:
            - For all messages published to the broker within given transaction, 'commit' allows PubSub+
...
class TransactionalMessagingService(TransactionalSupport, Connectable, ABC):
    """
      An interface for transactional (non-XA, broker transacted session) use case of a PubSub+ event broker.
      Transactions are feature of the PubSub+ event broker, client just enables broker-side transactional support.
    """
...
class _TransactionalMessagingService(TransactionalMessagingService):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, config: dict, messaging_service: 'MessagingService'):
        ...
        self._transactedSession_p = c_void_p(None)
    ...
    def _connect_job(self) -> TransactionalMessagingService: # pylint: disable=protected-access
        ...
        ret = solace.CORE_LIB.solClient_session_createTransactedSession(
            ...
            byref(self._transactedSession_p),
            None)
        if ret in [SOLCLIENT_OK]:
            buff_size = 255
            session_name_buffer = create_string_buffer(buff_size)
            solace.CORE_LIB.solClient_transactedSession_getSessionName(self._transactedSession_p,
                                                                       session_name_buffer,
                                                                       buff_size)
            ...
            self._service_down_id = \
                self._messaging_service._transport_event_emitter.register_transport_event_handler(_SolaceTransportEvent.TRANSPORT_DOWN,
                                                                           self._service_down_callback)
    ...
    def _disconnect_job(self):  # pylint: disable=protected-access
        ...
        ret = solace.CORE_LIB.solClient_transactedSession_destroy(byref(self._transactedSession_p))
        ...
        self._shutdown(_SolaceTransportEventInfo("", "", event_message))
        if ret in [SOLCLIENT_OK]:
            return ret
    ...
    def _commit_job(self):
        ...
        if self._commit_lock.acquire(blocking=False):
            try:
                ret = solace.CORE_LIB.solClient_transactedSession_commit(self._transactedSession_p)
            finally:
    ...
    def _rollback_job(self):
        ...
        if self._commit_lock.acquire(blocking=False):
            try:
                    ret = solace.CORE_LIB.solClient_transactedSession_rollback(self._transactedSession_p)
            finally:
                self._commit_lock.release()
        else:
            raise TransactionError(TRANSACTIIONAL_OPERATION_UNDERWAY)
...
class TransactionalMessagingServiceBuilder(PropertyBasedConfiguration):
    """
        An interface to create transactional (non-XA) messaging service with a PubSub+ event broker.
        Transactions are feature of the PubSub+ event broker, client just enables broker-side
...
class _BasicMessagingService(MessagingService):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, **kwargs):
        ...
        self._transport_event_handlers = dict()
    ...
    def _on_service_down(self, ctx=None):
    ...
    def remove_service_interruption_listener(self, listener: ServiceInterruptionListener) -> bool:
        ...
        if isinstance(listener, ServiceInterruptionListener):
            try:
                result = self._service_interruption_listeners.remove(listener.on_service_interrupted)
    ...
    def _transport_up_event_handler(self, event_info: '_SolaceTransportEventInfo'):
        with self._mutex:
    ...
    def __put_service_event(self, host, listener, message, exception):
        ...
        try:
            ...
            logger.info(f"While trying to buffer transport events, the buffer was shutdown. " \
                        f"The following event was dropped: {obj}. " \
                        f"The following listener was dropped: {listener}.")
