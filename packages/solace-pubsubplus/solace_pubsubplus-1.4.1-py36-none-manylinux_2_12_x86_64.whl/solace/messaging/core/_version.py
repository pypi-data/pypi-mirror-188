...
def __load_version_file_info(path):
...
def solclient_get_version(core_lib) -> SolClientVersionInfo:
...
def solclient_get_version_info(core_lib) -> dict:
...
def solclient_set_version(core_lib, info:SolClientVersionInfo) -> int:
    return core_lib._solClient_version_set(byref(info)) # pylint: disable=protected-access

def solclient_set_version_from_info(core_lib, info:dict) -> int:
