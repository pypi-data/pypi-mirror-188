...
from solace.messaging.core._receive import _event_callback_func_type, \
    ...
    SolClientFlowCreateRxCallbackFuncInfo, \
    _flow_msg_callback_func_type, \
    solClient_flow_eventCallbackInfo_pt, \
    SolClientFlowEventCallbackInfo, \
    _event_callback_func_type
...
from solace.messaging.receiver._inbound_message_utility import acknowledge_message, pause, resume, \
    ...
    topic_endpoint_subscribe, topic_endpoint_unsubscribe, flow_destination, SolClientFlowRxMsgDispatchFuncInfo
...
from solace.messaging.utils._solace_utilities import get_last_error_info, is_type_matches, is_not_negative, \
    convert_ms_to_seconds, COMPLETED_FUTURE, _ThreadingUtil, _Released, _PubSubPlusQueue, QueueShutdown, \
...
class PersistentStateChangeListenerThread(_SolaceThread) \
    ...
    def __init__(self, owner_info: str, logger: 'logging.Logger', state_change_queue: Queue,
    ...
    def run(self):
        ...
        while self._is_running.is_set():
            ...
            try:
                ...
                self.adapter.debug(f"Persistent receiver state change listener queue was shutdown with the following message: {str(e)}")
                break
...
class PersistentMessageReceiverThread(_SolaceThread):  # pylint: disable=missing-class-docstring
    ...
    def __init__(self, owner_info: str, logger: 'logging.Logger', persistent_message_receiver,
...
class _PersistentMessageReceiver(_MessageReceiver, PersistentMessageReceiver) \
    ...
    def __init__(self, builder: '_PersistentMessageReceiverBuilder') \
        ...
        self._end_point_props = {k: v for (k, v) in end_point_props.items()}
        ...
        self._persistent_state_change_listener_finalizer = weakref.finalize(self, persistent_state_change_listener_cleanup,
                                                                            self._persistent_state_change_listener_holder)

        self._persistent_message_receiver_thread_finalizer = weakref.finalize(self, persistent_message_receiver_thread_cleanup,
        ...
        if self._missing_resource_strategy and \
            ...
            if return_code != SOLCLIENT_OK:
                ...
                raise PubSubPlusClientError(f"{self._queue_name}creation failed with the"
                                            f" following sub code {error_info[CCSMP_SUB_CODE]}({error_info[CCSMP_INFO_SUB_CODE]}) ")
    ...
    def __endpoint_topic_subscribe(self, topic):
        return_code = topic_endpoint_subscribe(self._end_point_arr, self._messaging_service.session.session_pointer, topic)
        if return_code != SOLCLIENT_OK:
            ...
            if self._message_receiver_thread is None:
                self._message_receiver_thread = \
                    PersistentMessageReceiverThread(self._id_info, logger, self, _receiver_thread_msg_queue_pop,
                                                    self._messaging_service, self._auto_ack, self.stop_event, _receiver_thread_on_join)
