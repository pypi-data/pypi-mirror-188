...
class SearchTask(object):
    ...
    def put_result(self, result):
        ...
        while max_tries > 0:
            try:
                RESULTS_QUEUES[os.getppid()].put(result,
...
class FileSearcher(SearcherBase):
    ...
    def _get_results(self, results, expected=None, event=None):
        _queue = RESULTS_QUEUES[os.getpid()]
        while True:
            if not _queue.empty():
    ...
    def run(self):
        ...
        jobs = []
        RESULTS_QUEUES[os.getpid()] = multiprocessing.Queue()
        with multiprocessing.Manager() as m:
            lock = m.Lock()
            event = threading.Event()
            event.clear()
            t = threading.Thread(target=self._get_results,
                                 args=[results], kwargs={'event': event})
            ...
            with multiprocessing.Pool(processes=self.num_parallel_tasks) as p:
                ...
                event.set()
                t.join()
                self._get_results(results, expected=self.stats['results'])

        del RESULTS_QUEUES[os.getpid()]
