            ...
            if 0 in doc_diff["Statement"] and len(doc_diff["Statement"]) == 1:
                if (
                    ...
                    for i, change, line in sorted(line_changes):
                        ...
                        diff_lines.append(
                            f" [{line_color}][bold]{change}[/bold] ({i}) {line}"
                        )
...
def do_full_setup(
    iam, user_name, account, region, setup_name, ongoing_name, manual_final_setup, yes
) -> bool:
...
def do_just_update_policies(iam, account, setup_name, ongoing_name, yes) -> bool:
    ...
    if not setup_arn:
        ...
        print(
            "Use `--setup-policy` to specify a different name for the existing setup policy."
        )
        print()
        return False
    ...
    if not ongoing_arn:
        ...
        print(
            "Use `--ongoing-policy` to specify a different name for the existing ongoing policy."
        )
        print()
        return False
