...
def get_wikipedia_page_titles(categories: list = ["Climate_change"], corpus: str = properties.CORPUS):
    wikipedia_pages_df = {}

    for category in categories:
        try:
            all_unique_pages = [Path(x).name.replace(".parquet", "") for x in
                                glob(os.path.join(corpus, category, "*", "*.parquet"))]
            all_unique_pages = list(set(all_unique_pages))

            wikipedia_pages_df[category] = all_unique_pages
            logging.info(f"There are {len(all_unique_pages)} wikipedia pages for the category {category}")
        except:
            logging.error(f"Make sure that the corpus parquet files are located here {os.path.join(corpus, category)}")
    return wikipedia_pages_df


def get_wikipedia_page_data(wikipedia_page: str, category: str = "Climate_change",
                            corpus: str = properties.CORPUS) -> pd.DataFrame:
    def to_tuple(arr):
