...
class Xform(object):
	...
	def aimObjectAtPoint(objects, target_pos, aim_vect=(1,0,0), up_vect=(0,1,0)):
		...
		for obj in Iter.makeList(objects):
	...
	def snap3PointsTo3Points(vertices):
		...
		dp = Math.dotProduct(axis1, axis2, 1)
		dp = Math.clamp(-1.0, 1.0, dp)
		angle = math.acos(dp)
		crossProduct = Math.crossProduct(axis1, axis2, 1, 1)
		...
		rotation = Math.xyzRotation(angle, crossProduct)
		...
		dp = Math.dotProduct(axis3, axis2, 0)
		...
		dp = Math.dotProduct(axis4, axis2, 0)
		...
		dp = Math.dotProduct(axis3, axis4, 1)
		dp = Math.clamp(-1.0, 1.0, dp)
		angle = math.acos(dp)

		# reverse the angle if the cross product is in the -ve axis direction
		crossProduct = Math.crossProduct(axis3, axis4, 1, 1)
		dp = Math.dotProduct(crossProduct, axis2, 0)
		if dp < 0:
			angle = -angle

		rotation = Math.xyzRotation(angle, axis2)
	...
	def getMatchingVerts(cls, meshA, meshB, worldSpace=False):
		...
		return [i for h in matching 
			for i in zip(Iter.indices(hashA, h), Iter.indices(hashB, h))]
	...
	def orderByDistance(objects, point=[0, 0, 0], reverse=False):
		...
		for obj in pm.ls(objects, flatten=1):
			...
			bb_dist = Math.getDistBetweenTwoPoints(point, bb_pos)
