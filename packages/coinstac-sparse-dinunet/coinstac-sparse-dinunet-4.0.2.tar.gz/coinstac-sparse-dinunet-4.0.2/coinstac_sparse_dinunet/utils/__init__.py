...
def save_time_logs_remote(cache, log_dir):
    with open(log_dir + _os.sep + f"timelogs_remote.json", 'w') as fp:
        log = {}.fromkeys(cache.keys())
        for k in log:
            try:
                log[k] = _copy.deepcopy(cache[k])
...
def save_time_logs(cache, log_dir):
    with open(log_dir + _os.sep + f"timelogs.json", 'w') as fp:
        log = {}.fromkeys(cache.keys())
        for k in log:
            try:
                log[k] = _copy.deepcopy(cache[k])
...
def save_epoch_logs(cache, log_dir):
    with open(log_dir + _os.sep + f"epochlogs.json", 'w') as fp:
        log = {}.fromkeys(cache.keys())
        for k in log:
            try:
                log[k] = _copy.deepcopy(cache[k])
