...
class COINNLocal:
    ...
    def __init__(self, cache: dict = None, input: dict = None, state: dict = None,
        ...
        self._args["sparse_training"]=self.input.get('sparse_training')
        self.cache["sparse_training"]= self.input.get('sparse_training')
    ...
    def compute(self, mp_pool,
        ...
        """Computation begins..."""
        # self.cache["timelogsDirectory"] = self.state['transferDirectory']
        ...
        if self.out['phase'] == Phase.COMPUTATION:
            ...
            #_utils.save_epoch_logs(epochlogsdict,self.state['transferDirectory'])
            with open(self.cache['logger_directory'] + _sep +  'epoch_logs.json', 'a+') as file:
                file.write("\n" + str(epochlogsdict))
