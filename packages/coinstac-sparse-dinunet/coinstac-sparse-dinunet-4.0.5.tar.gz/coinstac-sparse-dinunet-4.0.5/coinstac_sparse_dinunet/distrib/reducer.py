...
class COINNReducer:
    ...
    def reduce(self):
        ...
        _shutil.copy(self.timelogs, f"{self.state['outputDirectory']}{_sep}timelogsremote.json")
        _shutil.copy(self.timelogs, f"{self.state['transferDirectory']}{_sep}timelogsremote.json")
                #Write into logger
        with open(self.cache['timelogsDirectory'] + '/time_logs_remote.json', 'a+') as file:
            file.write("\n" + str(self.timelogs))
