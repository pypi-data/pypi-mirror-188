...
class COINNLearner:
    def __init__(self, trainer=None, mp_pool=None, **kw):
        ...
        self.timeLogsDirectory = _os.path.join(
            self.state['transferDirectory'],
            self.cache['task_id']
        )
        self.timelogs={}
        _os.makedirs(self.timeLogsDirectory, exist_ok=True)
    ...
    def to_reduce(self):
        ...
        #Write into logger
        with open(self.cache['log_dir'] + _sep + 'time_logs.json', 'a+') as file:
            file.write("\n" + str(self.timelogs))
        
        #_utils.save_time_logs(self.timelogs,self.state['transferDirectory'])
