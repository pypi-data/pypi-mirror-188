...
(defn get (& args)           ;; Safely get item from any collection.
...
(defn select-strs (coll strs)  ;; Returns key-value pairs from a dict
 (when (and (dict? coll) (or (list? strs) (tuple? strs) (set? strs)))
  (->> coll
       (.items)
       (filter (fn (keyword-pair)
                 (let ((key __) keyword-pair) (contains? strs key))))
       dict)))
