...
def formula_apply(S, N, func: Callable[..., Any], raw: bool = False):
    s_len = len(S)
...
def AVEDEV(S, N):
    ...
    :return:
    """
    return formula_apply(S, N, lambda R, W: (np.abs(R - R.mean())).mean(), raw=True)
...
def FORCAST(S, N):
    ...
    :return:
    """
    return formula_apply(S, N, lambda R, W: np.polyval(np.polyfit(range(W), R, deg=1), W - 1) if W > 1 else np.nan,
                         raw=True)
    ...
    if isinstance(N, (int, float)):  # 常量
        ...
        for i in range(s_len):
            if (not np.isnan(N[i])) and N[i] <= i + 1:
                ...
                pos = i + 1
                t = S[pos - window:pos]
                d = pd.Series(t).rolling(window).apply(
