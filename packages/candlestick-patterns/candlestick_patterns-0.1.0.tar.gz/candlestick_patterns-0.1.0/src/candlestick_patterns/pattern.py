...
def doji(df):
    df['Doji'] = np.where(np.multiply((df.High - df.Low), 0.05)
                          > np.absolute(df.Open-df.Close), True, False)
    return df


def gravestone_doji(df):
    df['Gravestone Doji'] = np.where((np.multiply((df.High - df.Low), 0.05) > np.absolute(df.Open-df.Close)) &
                                     (df.Open < (df.Low + np.multiply((df.High - df.Low), 0.05))), True, False)
    return df


def dragonfly_doji(df):
    df['Dragonfly Doji'] = np.where((np.multiply((df.High - df.Low), 0.05) > np.absolute(df.Open-df.Close)) &
...
def long_legged_doji(df):
    ...
    var_df['isupper'] = np.where((((var_df.High - var_df.l10) > var_df.Open) & (
        (var_df.High - var_df.l30) < var_df.Open)), True, False)
    var_df['islower'] = np.where((((var_df.Low + var_df.l10) < var_df.Open)
                                 & ((var_df.Low + var_df.l30) > var_df.Open)), True, False)

    df['Long-Legged Doji'] = np.where(var_df.Doji &
                                      (var_df.isupper | var_df.islower), True, False)
    del var_df
...
def inverted_hammer(df):
    df['Inverted Hammer'] = np.where(((((df.High - df.Close) > (df.Close - df.Low) * 2) & (df.Close > df.Open)) |
...
def spinning_top(df):
    df['Spinning Top'] = np.where(((df.Close >= (df.Low + ((df.High - df.Low) / 3))) &
...
def marubozo(df):
    df['Bullish Marubozo'] = np.where(((df.Open < df.Close) & (
        df.Open == df.Low) & (df.Close == df.High)), True, False)
    df['Bearish Marubozo'] = np.where(((df.Open > df.Close) & (
        df.Open == df.High) & (df.Close == df.Low)), True, False)
    return df
...
def harami(df):
    df['Bullish Harami'] = np.where(((df.High <= df.Open.shift(1)) &
                                     (df.Low >= df.Close.shift(1)) & (df.Close > df.Open)), True, False)
    df['Bearish Harami'] = np.where(((df.High <= df.Close.shift(1)) &
                                     (df.Low >= df.Open.shift(1)) & (df.Close < df.Open)), True, False)
    return df


def dark_cloud_cover(df):
    df['Dark Cloud Cover'] = np.where(((df.Close.shift(1) > df.Open.shift(1)) &
...
def piercing_pattern(df):
    df['Piercing Pattern'] = np.where(((df.Close.shift(1) < df.Open.shift(1)) &
                                       (((df.Close.shift(1) + df.Open.shift(1)) / 2) < df.Close) & (df.Open < df.Close) &
                                       (df.Open < df.Close.shift(1)) & (df.Close < df.Open.shift(1)) &
                                       ((df.Open - df.Close) / (0.001 + (df.High - df.Low)) < 0.6)), True, False)
    return df
