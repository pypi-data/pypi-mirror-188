...
EncryptionMissing, ProtocolError = Exception, Exception
...
class Client(object):
    ...
    def _notification(self):
        ...
        """
        request = ClientRequest(self, notify=True, req_id=None)
        self._requests.append(request)
        return request
    ...
    def _call_batch(self, requests):
        ...
        if len(ids) == 0:
            notify = True
        response_text = self._send_and_receive(message, batch=True, notify=notify)
...
class BatchResponses(object):
    ...
    def get(self, req_id):
        ...
        if not responses or len(responses) == 0:
            raise KeyError('Job "%s" does not exist or has already be retrieved.' % req_id)
...
class ClientRequest(object):
    ...
    def __call__(self, *args, **kwargs):
        if not (len(args) == 0 or len(kwargs) == 0):
            raise ValueError(
                "JSON spec allows positional arguments OR " + "keyword arguments, not both."
            )
        params = list(args)
        if len(kwargs) > 0:
            params = kwargs
    ...
    def _request(self):
        request = {"jsonrpc": "2.0", "method": self._namespace}
        if self._params:
