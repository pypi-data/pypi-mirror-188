...
def simulateNIR_calibrationTransfer(nSample = 100, nComp = 3,shifts = 0.01, refType = 1, noise = 0.0, seeds = 1):
    """
    ...
    np.random.seed(seeds)
    conc = np.random.random((nSample,nComp))
    mu1 = np.random.random(nComp)*1500+1000
    sigma1 = np.random.random(nComp)*100+100
    spcBase1 = [norm.pdf(wv, mu1[i],sigma1[i]) for i in range(nComp)]
    ...
    spcBase2 = [norm.pdf(wv, mu2[i],sigma2[i]) for i in range(nComp)]
