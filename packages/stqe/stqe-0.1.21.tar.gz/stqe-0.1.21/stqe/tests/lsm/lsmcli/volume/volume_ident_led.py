...
def volume_ident_led_success():
    ...
    for config in yield_lsm_config():
        ...
        _, disks = atomic_run("Listing local disks before changing ident LED.",
        ...
        _, disks = atomic_run("Listing local disks after changing ident LED.",
                                command=lsm.local_disk_list,
                                return_output=True,
                                script=True,
                                errors=errors)
        ...
        if data_before != data_after:
            atomic_run("Enabling ident LED to return it to previous state with protocol %s." % config['protocol'],
                       ...
                       vol=vol,
                       errors=errors)
            _, disks = atomic_run("Listing local disks after returning ident LED state.",
                                    command=lsm.local_disk_list,
                                    return_output=True,
                                    script=True,
                                    errors=errors)
