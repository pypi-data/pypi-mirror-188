...
def access_groups_success():
    ...
    for config in yield_lsm_config():
        ...
        atomic_run("Using LSM to list ACCESS_GROUPS with protocol %s" % config['protocol'],
                   command=lsm.list,
...
def access_groups_fail():
    ...
    for config in yield_lsm_config():
        lsm = LibStorageMgmt(disable_check=True, **config)
        for argument in arguments:
            atomic_run("Trying to fail using LSM to list ACCESS_GROUPS with protocol %s" % config['protocol'],
                       errors=errors,
                       command=lsm.list,
                       lsm_type="ACCESS_GROUPS",
                       success=False,
