...
class FeatureServingEndpointClient:
    def __init__(self, get_host_creds, fs_client):
        ...
        self._dummy_training_df = self._create_dummy_train_df()
    ...
    def create_feature_serving_endpoint(
        ...
        workload_size: str = FEATURE_SERVING_DEFAULT_WORKLOAD_SIZE,
        scale_to_zero_enabled: bool = FEATURE_SERVING_DEFAULT_SCALE_TO_ZERO,
    ) -> FeatureServingEndpoint:
        self._validate_endpoint_name(endpoint_name)
        self._verify_endpoint_not_exists(endpoint_name)
        model_name = self._get_model_name(endpoint_name)
        model_version = self._create_model(model_name, feature_lookups)

        request_body = self._get_create_endpoint_request_body(
    ...
    @check_enable_flag
    def get_feature_serving_endpoint(self, endpoint_name) -> FeatureServingEndpoint:
        self._validate_endpoint_name(endpoint_name)
        result = self._call_api(f"/{endpoint_name}", method="GET", json_body={})
        return self._convert_to_feature_serving_endpoint(result)

    @check_enable_flag
    def delete_feature_serving_endpoint(self, endpoint_name) -> None:
        self._validate_endpoint_name(endpoint_name)
    ...
    def _create_model(self, model_name, feature_lookups) -> ModelVersion:
        ...
        self._log_model_func(
            ...
            use_alpha_lookup_client=True,
        )
        return self._register_model(model_name)
    ...
    def _validate_endpoint_name(self, endpoint_name):
        if not self._endpoint_name_matcher.match(endpoint_name):
            raise ValueError(
    ...
    def _get_create_endpoint_request_body(
        self,
        endpoint_name,
    ...
    def _create_dummy_train_df(self):
