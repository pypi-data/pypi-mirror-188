...
#elif ndim_macro==3
	madd_kvv(-eta0,w_unit,eta,e_unit); // Alternatively, use cross products
	const Scalar norm2_e = norm2_v(e_unit);
	if(norm2_e>0){mul_kV(Scalar(1)/sqrt(norm2_e),e_unit);} 
	//Better than eta1=scal_vv(eta,e_unit) if norm2_e is small
	const Scalar eta1 = sqrt(max(Scalar(0),norm2_v(eta)-eta0*eta0)); 
#endif
