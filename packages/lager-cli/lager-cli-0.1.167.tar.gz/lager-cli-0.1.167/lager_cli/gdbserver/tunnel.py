...
async def serve_tunnel(host, port, connection_params, name, debug=True, rtt=None, rtt_port=None, *, task_status=trio.TASK_STATUS_IGNORED):
    """
    ...
    async with trio.open_nursery() as nursery:
        ...
        if rtt:
            rtt_handler = functools.partial(cloud_connection_handler, rtt, debug=debug)
            rtt_serve_listeners = functools.partial(trio.serve_tcp, rtt_handler, rtt_port, host=host)
            await nursery.start(rtt_serve_listeners)
            click.echo(f'Serving RTT on {host}:{rtt_port}')
