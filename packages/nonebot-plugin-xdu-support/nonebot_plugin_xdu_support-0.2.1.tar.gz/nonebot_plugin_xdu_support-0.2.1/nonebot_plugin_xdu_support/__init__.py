...
import os
from .model import check, cron_check, get_sport_record, get_timetable, get_whole_day_course, get_next_course, get_question, get_teaching_buildings, get_classroom, get_idle_classroom, des_encrypt, des_descrypt
...
idle_classroom_query = on_command("空闲教室", priority=5, block=True)
...
async def _(state: T_State, time_selector: str = ArgStr("time_selector")):
    message = ""
    ...
    try:
        ...
        rooms = state["rooms"]
        build = state["build"]
        result = get_idle_classroom(ses, rooms, time_)
        message += f"{time_} {build}的空闲教室如下\n****************\n"
        for lesson, idle_rooms in result.items():
            message += f"第{lesson}节课：\n"
            for idle_room in idle_rooms:
                message += f"{idle_room},"
            message += f"\n"
