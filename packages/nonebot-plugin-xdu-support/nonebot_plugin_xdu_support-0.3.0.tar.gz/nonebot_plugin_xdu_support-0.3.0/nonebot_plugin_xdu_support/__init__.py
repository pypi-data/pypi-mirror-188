...
async def _():
    if not SK:
        await aed_search.finish("无法使用，未在配置文件中填写SK")
    await asyncio.sleep(0.5)
...
async def _(state:T_State, num:str = ArgStr("num")):
    ...
    if max_num == 2:
        ...
        url = get_url_marker(info["lat"], info["lng"], SK, appname, info["loc"] + "_" + info["description"])

    message = MessageSegment.share(url, info["loc"] + "_" + info["description"])
