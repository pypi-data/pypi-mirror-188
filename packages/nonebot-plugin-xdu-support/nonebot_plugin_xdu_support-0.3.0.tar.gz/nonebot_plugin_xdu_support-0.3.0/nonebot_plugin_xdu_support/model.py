...
def get_min_distance_aed(now_lat:str = "108.8326315482191", now_lng:str = "34.12501587001219") -> (Dict, float, List):
    response = requests.get(
        "https://gis.xidian.edu.cn/openmap/mapi/place/v1/list?",
        params={
            "category":"AED",
            "location": f"{now_lat},{now_lng}"
        }).json()
    aed_infos = []
    min_dist = np.inf
    aed_min_dist = {}
    for aed in response["result"]:
        if aed["campus"] == "南校区":
            info = {
                "id": aed["id"],
                "campus": "南校区",
                "loc": aed["name"].replace("(南校区）", "").replace("AED", ""),
            ...
            info = {
                "id": aed["id"],
                "campus": "北校区",
                "loc": aed["name"].replace("(北校区）", "").replace("AED", ""),
                "description": aed["description"].replace("\n",'').replace("位置：",""),
                "distance": aed["distance"],
                "lat": aed["location"]["x"],
                "lng": aed["location"]["y"]
        ...
        if min_dist > info["distance"]:
            ...
            aed_min_dist = info
        if info["id"] == 282418 or info["id"] == 282419 or info["id"] == 282414 or info["id"] == 282417 or info[
            "id"] == 282343 or info["id"] == 282365 or info["id"] == 282351:
...
def get_url_routeplan(now_lat:str, now_lng:str, tar_lat:str, tar_lng:str, SK:str, appname:str) -> str:
    base_url = "https://apis.map.qq.com"
    ori_url = f"/uri/v1/routeplan?type=drive&fromcoord={now_lat},{now_lng}&tocoord={tar_lat},{tar_lng}&policy=0&referer={appname}"
    signed_url = ori_url + get_signed_url(ori_url+SK)
    url = base_url + signed_url
    return url

def get_url_marker(lat:str, lng:str, SK:str, appname:str, address:str) -> str:
    base_url = "https://apis.map.qq.com"
    ori_url = f"/uri/v1/marker?marker=coord:{lat},{lng};title:AED;addr:{address}&referer={appname}"
    signed_url = ori_url + get_signed_url(ori_url + SK)
    url = base_url + signed_url
    return url
