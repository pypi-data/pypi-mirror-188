...
import os
from .model import check, corn_check, get_sport_record, get_timetable, get_whole_day_course, get_next_course, get_question, des_encrypt, des_descrypt
...
# 定时每10分钟查一次
@scheduler.scheduled_job("corn", minute="*/10", hour="7-22", month="2-7,9-12")
async def run_every_10_minutes():
    ...
    if flag:
        for user in users:
            ...
            flag, res = corn_check(ses, user[1])
            if not flag:
                await asyncio.sleep(1)
...
@scheduler.scheduled_job("corn", hour="8", month="2-7,9-12")
async def run_at_8():
    bot = nonebot.get_bot()
    path = Path(XDU_SUPPORT_PATH, "Ehall.txt")
    flag, users = read_data(path)
...
@scheduler.scheduled_job("corn", minute="55", hour="9", month="2-7,9-12")
async def run_at_8():
    bot = nonebot.get_bot()
    path = Path(XDU_SUPPORT_PATH, "Ehall.txt")
    flag, users = read_data(path)
...
@scheduler.scheduled_job("corn", minute="30", hour="13", month="2-7,9-12")
async def run_at_8():
    bot = nonebot.get_bot()
    path = Path(XDU_SUPPORT_PATH, "Ehall.txt")
    flag, users = read_data(path)
...
@scheduler.scheduled_job("corn", minute="25", hour="15", month="2-7,9-12")
async def run_at_8():
    bot = nonebot.get_bot()
    path = Path(XDU_SUPPORT_PATH, "Ehall.txt")
    flag, users = read_data(path)
...
@scheduler.scheduled_job("corn", minute="30", hour="18", month="2-7,9-12")
async def run_at_8():
    bot = nonebot.get_bot()
    path = Path(XDU_SUPPORT_PATH, "Ehall.txt")
    flag, users = read_data(path)
...
@scheduler.scheduled_job("corn", hour="22", month="2-7,9-12")
async def run_at_8():
    bot = nonebot.get_bot()
    path = Path(XDU_SUPPORT_PATH, "Ehall.txt")
    flag, users = read_data(path)
