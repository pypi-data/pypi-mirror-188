...
def save_file(content, file_base_name, extension, dest_dir="."):
    ...
    :rtype: str
    """
    full_file_path = get_unused_name(file_base_name, extension, dest_dir)
    with open(full_file_path, 'wb') as f:
        f.write(content)
    return full_file_path


def get_unused_name(basename, extension="", dirname="."):
    """
    This function returns a file/folder path in the destination dir (defaulted to ".") as base_name + extension,
    but adds a counter if needed.

    :param basename: the future file/folder name without extension
    :type basename: str
    :param extension: the file name extension (for example: 'xlsx'),
                        leave blank for no extension, or for folder names
                        Default: ""
    :type extension: str
    :param dirname: the directory folder path for the new file/folder
                        Default: current directory (".")
    :type dirname: str

    :return: full nonexistent file/folder path
    :rtype: str
    """
    separator = '.' if extension else ''
    full_path = os.path.join(dirname,
                             f"{basename}{separator}{extension}")
    counter = 0
    while os.path.exists(full_path):
        counter += 1
        full_path = os.path.join(dirname,
                                 f"{basename}-{counter}{separator}{extension}")
    return full_path
