...
class WidowxEnv(gym.Env):
    ...
    def __init__(
        ...
        self.pybullet_action_min = np.array([-0.05, -0.025, -0.025, -0.025,-0.025, -0.05, 0]) * self.pybullet_action_coeff # old values
        self.pybullet_action_max = np.array([0.05, 0.025, 0.025, 0.025, 0.025, 0.05, 0.025]) * self.pybullet_action_coeff # old values
        ...
        if self.joint_limits == "small":
            ...
            self.joint_min = np.array([-2.6, -1.5, -1.5, -1.7,-1.7, -2.6, 0.0])
            self.joint_max = np.array([2.6, 0.8, 1.5, 1.7,1.7, 2.6, 0.0])
        elif self.joint_limits == "large":
            self.joint_min = np.array([-3.2, -3.2, -3.2, -3.2,-3.2, -3.2, -3.2])
            self.joint_max = np.array([3.2, 3.2, 3.2, 3.2,3.2, 3.2, 3.2])
        ...
        if self.obs_type == 1:
            ...
            self.obs_space_low = np.float32(
                np.concatenate(([-1.0]*7, self.joint_min), axis=0))
            self.obs_space_high = np.float32(
                np.concatenate(([1.0]*7, self.joint_max), axis=0))
    ...
    def _get_joint_info(self):
        ...
        info = p.getJointStates(self.arm, range(7))
        pos, vel, rf, t = [], [], [], []
