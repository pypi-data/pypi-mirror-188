...
class _LookaheadIterator:
    """A LookaheadIterator provides the ability to put back previously
    returned tokens.

    Conceptual view of LookaheadIterator:
...
class _CriteriaTargetIterator(_LookaheadIterator):
    """The purpose of this iterator is to record the criteria
    fields that we were unable to parse.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.__stored_fields = []

    def forward_past(self, field_name: str) -> None:
        """Forward past the specified field, which must exist.
        """
        try:
            while True:
                field = next(self)
    ...
    def next_value(self, attr: str) -> str:
        """Helper method to return the next field which holds the 'value'
        for attr
        """
        try:
            return next(self)
...
class ChainRule:        # pylint: disable=too-many-instance-attributes
    ...
    def parsing_failed(self) -> bool:
        ...
        """
        return (self.__iptables_line is not None) and not self.__parsed
    ...
    def has_match(self, match: Match) -> bool:
        ...
        of the specified match. An object of :class:`MatchNone` can be
    ...
    def targets_chain(self, chain) -> bool:
    ...
    def get_target_chain(self):
        """Returns the (non-builtin) :class:`Chain` object that is
    ...
    def go_to(self, *, chain) -> 'ChainRule':
    ...
    @classmethod
    def __parse_target_name(cls, field_iter: _LookaheadIterator):
    ...
    def create_from_existing(cls,   # pylint: disable=too-many-locals
                        iptables_output_line: str, pft) -> 'ChainRule':
        ...
        try:
            ...
            if field_iter.peek() == '[goto]':
                _ = next(field_iter)
                uses_goto = True
            match_list = parse_criteria(field_iter)
            new_field_iter = _CriteriaTargetIterator(field_iter, 3)
            target = parse_target(target_name, new_field_iter, uses_goto)
        ...
        if isinstance(target, (ChainTarget, UnparsedTarget)):
            ...
            if isinstance(target, UnparsedTarget):
                parsed = False
        rule._set_iptables_line(iptables_output_line, parsed)
        return rule
