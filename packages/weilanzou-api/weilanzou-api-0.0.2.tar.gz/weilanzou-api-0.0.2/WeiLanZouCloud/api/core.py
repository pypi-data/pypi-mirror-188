...
from WeiLanZouCloud.api.models import FileList, FolderList
from WeiLanZouCloud.api.types import *
from WeiLanZouCloud.api.utils import *
...
class LanZouCloud(object):
    ...
    ua = UserAgent()
    random_ua = ua.random
    lanzou_icon_prefix = "https://image.woozooo.com/image/ico/"
    default_icon = "https://cdn.jsdelivr.net/gh/Fuukei/Public_Repository@latest/vision/basic/favicon.ico"
    ...
    def get_folder_info_by_url(self, share_url, dir_pwd='', mark_time=0, is_phone_ua=False) -> FolderDetail:
        """获取文件夹里所有文件的信息"""
        ...
        try:
            if is_phone_ua:
                ...
                html = self._get(share_url, headers=phone_headers).text
            else:
        ...
        try:
            ...
            # Acloudtwei修复k的获取，现在lanzou的k获取变量前面多了一个 '_'
            k = re.findall(r"var [_|0-9a-z]{6} = '([0-9a-z]{15,})';", html)[0]
            uid = (re.findall(r"'uid':'[0-9].*'", html)[0].split(':')[-1]).replace("'", '')
            # 文件夹的信息
            folder_id = re.findall(r"'fid':'?(\d+)'?,", html)[0]
            folder_name = re.findall(r"var.+?='(.+?)';\n.+document.title", html) or \
                          re.findall(r'<div class="user-title">(.+?)</div>', html) or re.findall(
                r'<title>(.+?)</title>', html)
        ...
        while True:
            ...
            try:
                ...
                post_data = {'lx': lx, 'pg': page, 'k': k, 't': t, 'fid': folder_id, 'pwd': dir_pwd, 'uid': uid,
                             'rep': 0, 'up': 1}
            ...
            if resp['zt'] == 1:  # 成功获取一页文件信息
                if mark_time != 0:
                    for f in resp["text"]:
                        if time.strftime('%Y-%m-%d', time.localtime(time.time())) == time_format(
                                f["time"]):
                            files.append(FileInFolder(
                                ...
                                icon=self.lanzou_icon_prefix + f["ico"] if f["p_ico"] != 0 else self.default_icon
                            ))
                        else:
                            break
                    break
