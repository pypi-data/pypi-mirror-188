...
"""
    ...
    def __init__(
        ...
        self.notes = []
        self.num = 0
        if type(notes) in (list, np.array, np.ndarray, Change):
            typeOfNotes = ""
            # Check that all notes are JazzNote type or all are JazzNoteStr
            if type(notes) in (str, JazzNote):
                notes = [notes]
            for note in notes:
            ...
            self.notes = Change.makeFromRingNumber(ringNumber=ringNumber).notes
        elif noteset != None:
            self.notes = Change.makeFromSet(notes=noteset).notes
        else:
            raise ValueError(
        ...
        if byAnotherName == True:
            for i in self.notes:
                for note in notesToFind:
                    if i.semitonesFromOne(octaveLimit=1) == note.semitonesFromOne(
                        octaveLimit=1
                    ):
                        _foundNotes += 1
        if not byAnotherName:
    ...
    def makeFromChangeNumber(
        ...
        return Change.makeFromSet(_st, straighten=False)
    ...
    def getPolygon(
        ...
        if returnFCircle == True:
            ...
            try:
                _polygonCentre = polylabel([_poly], precision=1, with_distance=True)
                            ...
                            if _chordSymbol.index("sus") != 0:
                                ...
                                if _baseExtension == "6/9":
                                    ...
                                    _extensionQualities[-1].replace(
                                        "6/9",
                                        makeTextMulticoloured(
                                            "6", Key(rootKey, JazzNote("6")).getASCII(), outlineColourTag = outlineColourTag,
                                        )
                                        + "/"
                                        + makeTextMulticoloured(
                                            "9", Key(rootKey, JazzNote("9")).getASCII(), outlineColourTag = outlineColourTag,
                                        ),
                                    )
                                    _extensionQualities[-1] = _makeCiphered(
                                        _extensionQualities[-1],
                        ...
                        if (
                            ...
                            _extensionQualities[
                                ...
                                makeTextMulticoloured(
                                    _makeStyled("ma"),
                                    [
                                        Key(rootKey, JazzNote(n)).getASCII()
                                        for n in ("3", "7")
        ...
        for way in ways:
            ...
            if Change.isValidWay(way):  # Is a way of note set
                ...
                if way == "Area":
                    ...
                    from .Book import Book
                    try:
                        Book.theBook.sequencePrimes[0]
                    except Exception:
                        Book.makePrimes = 1
                        Book.theBook = Book()
                    try:
                        _notesByWay = str(len(self)) + "-"+ str(1 + Book.theBook.sequencePrimesByLength[len(self)].index(self.forceChangeIntoAllowedNotes(allowedNotes=Change.allowedNoteNamesAllFlats).getPrimeForm()))
                    except ValueError as e:
                        input("a):\n{}:\n{}\n{}".format(
                            e, Book.theBook.sequencePrimesByLength[len(self)], self.forceChangeIntoAllowedNotes(allowedNotes=Change.allowedNoteNamesAllFlats)))
