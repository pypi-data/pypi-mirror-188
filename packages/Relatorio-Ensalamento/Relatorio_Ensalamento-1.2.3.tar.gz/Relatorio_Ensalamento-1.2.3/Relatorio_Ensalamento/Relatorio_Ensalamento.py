...
import time
from Relatorio_Ensalamento.Sugestao import Sugestao
import pandas
...
def gerarRelatorio(file_name_turma_disciplina,file_name_asc_xml,file_json_sigla,file_name_capacidade,file_json_turmasfic,contador, file_json_escolas, file_json_tipos,f):
    siglas = dict(file_json_sigla)
    ...
    for i in array_turmas_disciplinas:
        ...
        chave_dia = f"{periodo},{nome_turma},{cr_disciplina},{hora_inicio}".replace(' ', '').replace('2022/2','').replace('2023/1','').upper()
    ...
    nome_arquivo = f'./Relatorios_Gerados/Relatorio_Ensalamento_' + str(dt_now.strftime("%y%m%d_%H%M")) + '.xlsx'
    ...
    df_capacidade = pandas.read_excel(file_name_capacidade, sheet_name="BASE DE CAPACIDADES CWB", header=2,usecols=range(5,22))
...
def main(arquivo_carga_horaria,arquivo_capacidade, arquivo_xml):
    ...
    utc_now = pytz.utc.localize(datetime.datetime.utcnow())
    dt_now = utc_now.astimezone(pytz.timezone("America/Sao_Paulo"))
    f = open(f"./Log_gerados/Log_Turmas  {str(dt_now.strftime('%y-%m-%d %H-%M'))}.txt", "w+", encoding="utf8")
    ...
    nome_arquivo = gerarRelatorio(file_name_turma_disciplina,file_name_asc_xml,file_json_sigla,file_name_capacidade,file_json_turmasfic,contador, file_json_escolas, file_json_tipos,f)
    return  nome_arquivo
def gerarExcel(file_carga_horaria,file_capacidade,file_xml):

        # le os valores vindo da tela
        arquivo_carga_horaria = file_carga_horaria
        arquivo_capacidade = file_capacidade
        arquivo_xml = file_xml


        nome_arquivo = main(arquivo_carga_horaria, arquivo_capacidade, arquivo_xml)
