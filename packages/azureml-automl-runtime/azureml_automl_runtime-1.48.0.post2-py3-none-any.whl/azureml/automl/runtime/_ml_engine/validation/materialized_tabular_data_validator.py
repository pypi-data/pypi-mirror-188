...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import (AllFeaturesAreExcluded, AllTargetsNan,
                                                                              ...
                                                                              InsufficientSamplesForMinorityClass)
...
class MaterializedTabularDataValidator(AbstractTabularDataValidator):
    ...
    def validate_tabular_data(self, tabular_data: TabularData) -> None:
        ...
        elif self._task_type == Tasks.CLASSIFICATION:
            self._check_data_for_classification(tabular_data)
    ...
    def _check_data_for_classification(self, tabular_data):
        """Validate data for classification tasks."""
        # check if the number of samples for the minority class is at least as much as the request n_cv folds
        if self._n_cv_folds:
            labels, counts = np.unique(tabular_data.y, return_counts=True)
            if min(counts) < self._n_cv_folds:
                logger.error(f"Number of samples for the minority class label ({min(counts)}) is fewer than "
                             f"n_cv_folds ({self._n_cv_folds})")
                minority_class = labels[np.argmin(counts)]
                raise DataException(azureml_error=AzureMLError.create(
                    InsufficientSamplesForMinorityClass, class_label=minority_class, count=min(counts),
                    n_cv=self._n_cv_folds
                ))
