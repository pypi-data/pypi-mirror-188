# Copyright Louis Paternault 2019-2022
#
...
VERSION = "pypdf2" + PyPDF2.__version__
...
def _metadata2dict(pdf):
    ...
    if raw:
        return {
            key: raw[f"/{key.capitalize()}"]
            for key in METADATA_KEYS
            if f"/{key.capitalize()}" in raw
        }
    return {}
...
class PDFPage(AbstractPDFPage):
    ...
    def merge_translated_page(self, page, x, y):
        self._page.mergeTranslatedPage(page.pypdf2_page, x, y)
