...
if not cache_back.cached_objects:
    ...
    #for i in cache_back.cached_objects.keys():
        #return_table.append(i)
    ...
    for line in loc:
        ...
        if not_yet_observed:
            ...
            if f"add_to_cache({var_name}," in temp_line:
                line = '# ' + line
                line = line + f'''
                # actually, have some cache_back.rewrite_assignment stuff later
                cacheback_cur.execute({sql_query})
                {var_name} = pd.DataFrame(cacheback_cur.fetchall(), columns = [desc[0] for desc in cacheback_cur.description])
                '''
