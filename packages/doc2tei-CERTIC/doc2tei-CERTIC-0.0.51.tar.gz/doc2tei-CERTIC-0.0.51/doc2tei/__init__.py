...
def _log_xml_fatal(saxon_output: bytes, logger: logging.Logger) -> bool:
    has_fatal = False
...
def _call_saxon(
    ...
    if _write_binary_to_file(destination_path, out, logger):
        ...
        if _log_xml_fatal(out, logger):
            return False, "Not OK"
        _log_xml_error(out, logger)
        return True, "OK"
    return False, "Not OK"
...
def _process_doc(
    ...
    # Normalize, step 1
    success, output = _call_saxon(
        xsl_path="xsl-02-normalisation/normalizea.xsl",
    ...
    # Normalize step 2
    success, output = _call_saxon(
        xsl_path="xsl-02-normalisation/normalizeb.xsl",
    ...
    # Normalize, step 3
    success, output = _call_saxon(
        xsl_path="xsl-02-normalisation/normalizec.xsl",
    ...
    #
    # TRANSFORMATIONS XSL : 4 group (sp)
    #
    success, output = _call_saxon(
        xsl_path="xsl-04-group/sp.xsl",
    ...
    #
    # TRANSFORMATIONS XSL : 4 group (figure)
    #
    success, output = _call_saxon(
        xsl_path="xsl-04-group/figure.xsl",
        source_path=f"{doc_file_no_extension}_04a_sp.xml",
        destination_path=f"{doc_file_no_extension}_04abis_fig.xml",
        logger=logger,
        transient_files=transient_files,
    )
    if not success:
    ...
    #
    # TRANSFORMATIONS XSL : 4 group (floatingText)
    #
    success, output = _call_saxon(
        xsl_path="xsl-04-group/floatingText.xsl",
        source_path=f"{doc_file_no_extension}_04abis_fig.xml",
        destination_path=f"{doc_file_no_extension}_04b_floatingText.xml",
        logger=logger,
        transient_files=transient_files,
    )
    if not success:
    ...
    #
    # TRANSFORMATIONS XSL : 4 group (floatingText hierarchise)
    #
    success, output = _call_saxon(
        xsl_path="xsl-04-group/hierarchize.xsl",
        source_path=f"{doc_file_no_extension}_04b_floatingText.xml",
        destination_path=f"{doc_file_no_extension}_04c_floatingText.xml",
        logger=logger,
        transient_files=transient_files,
    )
    if not success:
    ...
    success, output = _call_saxon(
        ...
        source_path=f"{doc_file_no_extension}_04c_floatingText.xml",
    ...
    #
    success, output = _call_saxon(
        xsl_path="xsl-07-totei-modules/totei.xsl",
        source_path=f"{doc_file_no_extension}_06_hierarchize.xml",
        destination_path=f"{doc_file_no_extension}_07_tei.xml",
        logger=logger,
        transient_files=transient_files,
    )
    if not success:
    ...
    #
    # TRANSFORMATIONS XSL : 8 control styles and id
    #
    success, output = _call_saxon(
        xsl_path="xsl-05-control/control-styles.xsl",
        source_path=f"{doc_file_no_extension}_07_tei.xml",
        destination_path=f"{doc_file_no_extension}_08_tei_plus.xml",
        logger=logger,
        transient_files=transient_files,
    )
    if not success:
    ...
    #
    # TRANSFORMATIONS XSL : 9 suppression des erreurs
    #
    success, output = _call_saxon(
        xsl_path="xsl-05-control/control-errors.xsl",
        source_path=f"{doc_file_no_extension}_08_tei_plus.xml",
        destination_path=f"{doc_file_no_extension}_09_tei_metopes.xml",
        logger=logger,
        transient_files=transient_files,
    )
    if not success:
    ...
    if validation_schema == "metopes":
        ...
        return_code, out, err = _cli_exec(
            [
                ...
                doc_file_no_extension + "_09_tei_metopes.xml",
            ],
            logger,
        )
        transient_files.append(doc_file_no_extension + "_09_tei_metopes.xml")
        if return_code != 0:
            logger.error(
                "Validate {} {}".format(
                    os.path.basename(doc_file_no_extension + "_09_tei_metopes.xml"),
            ...
            logger.info(
                "Validate {}".format(
                    os.path.basename(
                        ...
                        + "_09_tei_metopes.xml validates {}".format(validation_schema)
                    )
                )
            )
    elif validation_schema == "openedition":
        #
        # TRANSFORMATIONS XSL : 10 to OpenEdition
        #
        success, output = _call_saxon(
            xsl_path="xsl-08-toOpenedition/toOpenedition.xsl",
            source_path=f"{doc_file_no_extension}_09_tei_metopes.xml",
            destination_path=f"{doc_file_no_extension}_10_tei_openedition.xml",
            logger=logger,
            transient_files=transient_files,
        )
        if not success:
        ...
        return_code, out, err = _cli_exec(
            [
                ...
                doc_file_no_extension + "_10_tei_openedition.xml",
                #     "--noout",
            ],
            logger,
        )
        if return_code != 0:
            logger.error(
                "Validate {} {}".format(
                    os.path.basename(doc_file_no_extension + "_10_tei_openedition.xml"),
                    _union_java_output(out, err),
                )
            )
        else:
            logger.info(
                "Validate {}".format(
                    os.path.basename(
                        doc_file_no_extension
                        + "_10_tei_openedition.xml validates {}".format(
                            validation_schema
                        )
                    )
                )
    ...
    if validation_schema == "metopes":
        shutil.copy(
            os.path.join(working_dir, doc_file_no_extension + "_09_tei_metopes.xml"),
            final_file_destination,
        )
    if validation_schema == "openedition":
        shutil.copy(
            os.path.join(
                working_dir, doc_file_no_extension + "_10_tei_openedition.xml"
            ),
            final_file_destination,
        )
