        ...
        <!-- titre traduit -->
            ...
            <xsl:for-each-group select="//text:p[starts-with(@text:style-name,'TEI_author') or starts-with(@text:style-name,'TEI_editor')]" group-starting-with="text:p[starts-with(@text:style-name,'TEI_author:') or starts-with(@text:style-name,'TEI_editor:')]">
                ...
                <xsl:element name="{$context}">
                    ...
                    <xsl:if test="$source='Metopes' and current-group()/.[@text:style-name='TEI_authority_biography']">
...
<xsl:template match="text:p[@text:style-name='TEI_authority_biography']" mode="bio">
    <p>
        <xsl:apply-templates/>
    </p>
</xsl:template>
...
<!--            <xsl:comment>author and editor mode review</xsl:comment>-->
            <docAuthor>
    ...
    <xsl:choose>
        <xsl:when test="$first">
            <xsl:variable name="surname">
                <xsl:choose>
                    <xsl:when test="ends-with(substring-before($list, ', '),' ')">
                        <xsl:value-of select="tokenize(substring-before(substring-before($list, ', '),' '),' ')[last()]"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="tokenize(substring-before($list, ', '),' ')[last()]"/>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <item>
                <persName>
                    <xsl:if test="substring-before($first,$surname)!=''">
                        <forename>
                            <xsl:value-of select="substring-before($first,$surname)"/>
                        </forename>
                    </xsl:if>
                    <surname>
        ...
        <xsl:otherwise>
            ...
            <xsl:comment>surname=<xsl:value-of select="$surname"/></xsl:comment>
            <item>
                <persName>
                    <xsl:if test="substring-before($first,$surname)!=''">
                        <forename>
