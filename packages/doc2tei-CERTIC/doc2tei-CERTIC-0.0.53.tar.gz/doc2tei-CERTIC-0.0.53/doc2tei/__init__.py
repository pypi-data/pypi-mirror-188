...
def _process_doc(
    ...
    #
    success, output = _call_saxon(
        xsl_path="xsl-03-enrich/enrich-lang.xsl",
    ...
    #
    # TRANSFORMATIONS XSL : 4 group (d:floatingText)
    #
    success, output = _call_saxon(
        xsl_path="xsl-04-group/floatingText.xsl",
        source_path=f"{doc_file_no_extension}_04c_fig.xml",
        destination_path=f"{doc_file_no_extension}_04d_floatingText.xml",
        logger=logger,
        transient_files=transient_files,
    )
    if not success:
    ...
    #
    # TRANSFORMATIONS XSL : 4 group (dbis:floatingText hierarchise)
    #
    success, output = _call_saxon(
        xsl_path="xsl-04-group/hierarchize.xsl",
        source_path=f"{doc_file_no_extension}_04d_floatingText.xml",
        destination_path=f"{doc_file_no_extension}_04dbis_floatingText.xml",
        logger=logger,
        transient_files=transient_files,
    )
    if not success:
    ...
    success, output = _call_saxon(
        ...
        source_path=f"{doc_file_no_extension}_04dbis_floatingText.xml",
