...
<xsl:template name="keyWordsListPC">
    ...
    <xsl:choose>
        ...
        <xsl:otherwise>
            <xsl:variable name="surname"><xsl:value-of select="normalize-space($list)"/></xsl:variable>
            <xsl:variable name="persname"><xsl:value-of select="substring-before($list,$surname)"/></xsl:variable>
            <item>
                <persName>
                    <xsl:if test="$persname!=''">
                        <persname>
