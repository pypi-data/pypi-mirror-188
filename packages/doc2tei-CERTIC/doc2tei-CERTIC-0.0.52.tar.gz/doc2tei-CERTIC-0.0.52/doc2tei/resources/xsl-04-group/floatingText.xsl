...
<xsl:template match="element()">
    <xsl:if test="not(preceding-sibling::start) or not(following-sibling::end) or local-name(preceding::*[local-name()='end' or local-name()='start'][1])='end' or local-name()='start' or local-name()='end'">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()"/>
        </xsl:copy>
    </xsl:if>
...
<xsl:template match="start">
    <xsl:for-each-group select="following-sibling::*" group-ending-with="end">
