...
if __name__ == '__main__':
    # debug
    import sqlalchemy
    conn_str = f'postgresql+psycopg2://liran:123456@10.204.14.29:5433/dvdrental'
    eng = sqlalchemy.create_engine(conn_str)

    source_df = pd.read_sql_table("staff_source", eng, schema="public")
    target_df = pd.read_sql_table("staff_scd", eng, schema="public")
    columns = ["first_name", "last_name"]

    df = scd2(src=source_df, tgt=target_df, cols_to_track=columns)

    df.to_sql("staff_scd", eng, schema="public", if_exists="replace", index=False)
