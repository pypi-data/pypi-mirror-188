...
#	Copyright (c) 2014, 2016, 2019, 2020 R.Dreas Nielsen
...
#	2021-08-16	Modified to seek(0) in the output buffer before truncating it.  RDN.
#	2022-02-01	Corrected setting of error_type.  Modified to use argparse instead of optparse.
#				Replaced "string.letters", which no longer exists.  RDN.
#	2022-02-02	Cleanup.  RDN
#	2023-01-29	Modified to allow column letters, as well as numbers, to be used for
#				the 'row_headers' specification.  RDN.
#=====================================================================================
...
CONFIG_KEYWORDS = """Configuration keywords and usage:
    ...
    column_header_label_#: The header text (label) for one of the column_group_count
                           ...
                           used directly.  There should be exactly column_group_count
...
class RowHeaderGenerator(object):
	...
	that was at the left (usually) of the crosstabbed data and is to be retained in the normalized 
	...
	def __init__(self, row_header_column_number, config_data):
		...
		if ROW_HEADERS_ROW in config_data:
...
class HeaderAsColumnGenerator(object):
...
class DataColumnGenerator(object):
...
def clparser():
	...
	vers_msg = "%s, %s" % (_version, _vdate)
	desc_msg = "Convert a crosstabbed CSV data table to a normalized data table (%s)." % vers_msg
	parser = ArgumentParser(description=desc_msg)
	...
	parser.add_argument("-d", "--displayspecs", action="store_true", dest="showspecs",
						default=False,
	...
	parser.add_argument("-n", "--number_rows", action="store", dest="rowseq",
						default=None,
						help="The column header for a column of sequential numbers for the output data rows.")
	parser.add_argument("-o", "--outputheaders",
	...
	parser.add_argument("-s", "--specname", action="store", dest="specname",
	...
	parser.add_argument("-v", "--version", action="version", version=vers_msg)
...
def read_config(config_file_name, config_section):
	...
	try:
		...
		config_vals[ROW_HEADERS] = list(set(columnspecs(config.get(config_section, ROW_HEADERS))))
	except:
...
def header_as_column_count(cfgdata):
	...
	while not done:
		if "header_as_column_" + str(colno+1) in cfgdata:
			colno = colno + 1
		else:
			done = True
	return colno
...
def un_xtab(inputfilename, outputfilename, configfilename, configsectionname, char_encoding, seq_name):
	...
	if len(cols) % grpsize != 0:
		raise UnXtabError(_FATAL, True, "The number of columns (%d) is not a multiple of the group size (%d)." % (len(cols), grpsize))
	grps = int(len(cols) / grpsize)			# Number of groups
	...
	if sys.version_info < (3,):
		...
		outf = UnicodeWriter(open(outputfilename, "w", newline = ''), dialect, char_encoding)
...
def main():
	...
	if not ("inputfile" in opts and "outputfile" in opts):
		bad_argument_error("Two command-line arguments, the names of the input and output CSV files, must be provided.")
	input_csv_file = opts.inputfile
	output_csv_file = opts.outputfile
