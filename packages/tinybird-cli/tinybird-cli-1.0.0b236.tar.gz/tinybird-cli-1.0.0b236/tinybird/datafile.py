...
async def check_pipe(
    ...
    token_for_top_requests: Optional[str] = None
):
    ...
    headers = {'Authorization': f'Bearer {token_for_top_requests}'} if token_for_top_requests else {'Authorization': f'Bearer {token}'}
    sql_for_coverage = f"""
            ...
            if check and not populate:
                if not is_materialized:
                    await check_pipe(
                        ...
                        token_for_top_requests=await get_token_from_main_branch(tb_client) if tests_last_requests_from_main else None)
                else:
...
def format_include(file_parts, doc, unroll_includes=False):
    if unroll_includes:
        return
...
def format_datasource(filename, unroll_includes=False):
    doc = parse_datasource(filename)
    file_parts = []
...
def format_node(file_parts, node, includes, line_length=None, unroll_includes=False):
...
def format_pipe(filename, line_length, unroll_includes=False):
    doc = parse_pipe(filename)
...
async def wait_job(tb_client, job_id, job_url, label, timeout=None):
    with click.progressbar(label=f"{label} ", length=100, show_eta=False, show_percent=True, fill_char=click.style("█", fg="green")) as progress_bar:
