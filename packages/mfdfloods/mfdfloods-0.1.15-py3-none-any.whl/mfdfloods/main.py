...
class MFD (matrix.Matrix):
    ...
    def __init__(self, dtm_path: str, manning_path: str, nodata: float = -999, mute: bool = True):
        ...
        matrix.Matrix.__init__(self, gtif.as_array(self.dtm_ds))
        ...
        self.mannings = geotransform.GeoTransformFit(
            self.mannings,
            self.mannings_gt,
            self.dtm_gt
        )
    ...
    def drainpaths (self, src: tuple[float, float], hydrogram_curve: list[tuple[float, float]]):
        ...
        try:
            ...
            hyd = hydrogram.gen_hydrogram(hydrogram_curve)
            break_flow = 0
            while break_flow == 0:
                break_flow = next(hyd)
            last_flood = None
