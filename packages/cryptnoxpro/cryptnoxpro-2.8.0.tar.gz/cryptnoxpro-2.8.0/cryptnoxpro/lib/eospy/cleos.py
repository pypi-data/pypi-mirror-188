...
from .exceptions import EOSMsigInvalidProposal
...
class Cleos:
    ...
    # transactions
    #####
    def push_transaction(self, transaction, key, broadcast=True, compression='none', timeout=30, sigtype='K1', pin=""):
        ...
        if isinstance(key, str): # account creation tx
            signatures = [ EOSKey(key).sign(digest) ]
        else:  # standard tx, sign the transaction with the card (key is a card object)
            signatures = [ EOSKey("").sign_with_cryptnox(bytes.fromhex(digest), key, sigtype, pin) ]
        # build final trx
        final_trx = {
    ...
    def transaction_hash(self, transaction):
        chain_info, lib_info = self.get_chain_lib_info()
        trx = Transaction(transaction, chain_info, lib_info)
        return sig_digest(trx.encode(), chain_info['chain_id'])

    def push(self, transaction, public_key, signature, key_type="K1", compression='none'):
        chain_info, lib_info = self.get_chain_lib_info()
        trx = Transaction(transaction, chain_info, lib_info)
        digest = sig_digest(trx.encode(), chain_info['chain_id'])
        signature_encoded = EOSKey("").build_signature(bytes.fromhex(digest), public_key,
                                                       signature, key_type)
        final_trx = {
            'compression': compression,
            'transaction': trx.__dict__,
            'signatures': [signature_encoded]
        }
