...
class BusinessLogic:
        ...
        :param sql_params: (dict) параметры для источника данных SQL.
            Поля, передаваемые в словарь:
                "server" - хост, который может быть задан в виде IP-адреса сервера (например, "10.18.0.132"),
                    либо в виде имени сервера (например, "polyapi.database1.ru");
                    опционально также может быть задан порт подключения, в таком случае он должен идти после указания
                    хоста через двоеточие (например, "10.18.0.132:5433", "polyapi.database1.ru:5433");
    ...
    def parse_subtotal_idxs_col_rows(self):
        response: dict = self.execute_olap_command(command_name='view', state='get')
        command_body: dict = response['queries'][0]['command']

        top_rows = command_body['top']
        left_data_rows = command_body['left']

        top_subtotal_col_idxs = set()
        for top_row in top_rows:
            for col_idx, cell in enumerate(top_row):
                if cell['type'] == self.SUBTOTAL_ANALYTICS_TYPE:
                    top_subtotal_col_idxs.add(col_idx)

        left_data_subtotal_row_idxs = set()
        for row_idx, left_data_row in enumerate(left_data_rows):
            for cell in left_data_row:
                if cell['type'] == self.SUBTOTAL_ANALYTICS_TYPE:
                    left_data_subtotal_row_idxs.add(row_idx)

        return top_subtotal_col_idxs, left_data_subtotal_row_idxs
