...
class CreateProject(TemplateCommand):
    ...
    def handle(self, **options):
        ...
        if not options["template"]:
            options["template"] = tenants_path
        template_path = os.path.join(
            os.path.join(tenants_path, "project_template"), options["template"]
        )
...
Next steps:
    ...
    5. tenant = create_public_tenant("localhost", "yourdomain@example.com", "password")
