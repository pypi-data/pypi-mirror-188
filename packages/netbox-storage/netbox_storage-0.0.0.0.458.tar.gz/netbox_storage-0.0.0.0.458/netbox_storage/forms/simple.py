...
class LVMSimpleForm(NetBoxModelForm):
    ...
    def clean(self):
        super().clean()
        drive = Drive(cluster=self.cluster, virtual_machine=self.virtual_machine, size=self.size)
        drive.save()
        partition = Partition(drive=drive, device=drive.device_name, size=self.size, type="Linux LVM")
        partition.save()
        vg = VolumeGroup(vg_name=self.vg_name)
        PhysicalVolume(partition=partition, pv_name=drive.device_name, vg=vg)
        LogicalVolume(vg=vg, size=self.size, path=self.path, fs=self.fs)
