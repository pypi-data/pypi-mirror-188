...
from netbox_storage.models import Drive, Partition, PhysicalVolume, LogicalVolume, VolumeGroup


class LVMModel:
    def __init__(self, lv_name, path, fs, vg_name, pv_list, partition_list, drive_list):
...
class RelatedDrives(PluginTemplateExtension):
    ...
    def full_width_page(self):
        ...
        for lv in lvs:
            vg = VolumeGroup.objects.filter(lv.vg).first()
            lvm_list.append(LVMModel(lv.lv_name, lv.path, lv.fs, vg.name))
            # physical_volumes = PhysicalVolume.objects.all()
            # physical_volume_list = []
            # for physical_volume in physical_volumes:
            #     if physical_volume.vg == vg:
            #         physical_volume_list.append(physical_volume.pv_name)
