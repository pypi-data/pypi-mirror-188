...
from .serialize import frame_path, monkeypatch_queryset_repr, KoloJSONEncoder
...
class KoloProfiler:
                ...
                if frame_filter(frame, event, arg):
                    ...
                    if frame_data:  # pragma: no branch
                        ...
                        self.frames_of_interest.append(
                            json.dumps(frame_data, skipkeys=True, cls=KoloJSONEncoder)
                        )
    ...
    def save_request_in_db(self, frames=None) -> None:
        if self.rust_profiler:
            self.rust_profiler.save_request_in_db()  # type: ignore
            return
...
class Enabled:
    ...
    def __enter__(self) -> None:
        ...
        self._monkeypatch = monkeypatch_queryset_repr()
        self._monkeypatch.__enter__()
    ...
    def __exit__(self, *exc) -> None:
        ...
        if self._monkeypatch is not None:
            self._monkeypatch.__exit__(*exc)
