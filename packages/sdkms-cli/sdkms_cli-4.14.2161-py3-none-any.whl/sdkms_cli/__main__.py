...
def audit_log(**kwargs):
    ...
    if not kwargs['sysadmin']:
        ...
        f = open(filename, 'w')
        f.write('Severity,Type,Groups,IP-Address,Message,Actor,Time,Response\n')
    else:
        ...
        f = open(filename, 'w')
        f.write('Severity,Type,IP-Address,Message,Actor,Time,Response\n')
    ...
    while True:
        ...
        if kwargs['limit']:
            url = '/sys/v1/logs?size=' + str(min(1000,limit)) + '&from=' + str(start) + '&range_from=' + str(start_time) + '&range_to=' + str(end_time)
        else:
            url = '/sys/v1/logs?size=1000&from=' + str(start) + '&range_from=' + str(start_time) + '&range_to=' + str(end_time)
        ...
        for log in response['hits']:
            log_data = log['_source']['severity'] + ',' + \
                       log['_source']['action_type'] + ','
            ...
            log_data = log_data + group_list + ',' + ip_data + ',' + '\"' + log['_source']['message'] + '\",' if not kwargs['sysadmin'] else log_data + ',' + ip_data + ',' + '\"' + log['_source']['message'] + '\",'
            ...
            log_data = log_data + actor + ',' + log['_source']['time'] + '\",'

            if log['_source']['response_time'] == None:
               log_data = log_data + "None" + "\n"
            else:
               rtimeMS = log['_source']['response_time']["secs"] * 1000 + \
	                 log['_source']['response_time']["nanos"] / 1000000
               log_data = log_data + str(rtimeMS) + "ms" + "\n"

            f.write(log_data)

        start = start + len(response['hits'])
        if kwargs['limit']:
...
'''.format(VERSION)
    ...
    parser_audit_log.add_argument("--start-time", help="start time in yyyy-mm-dd format", required=True)
    parser_audit_log.add_argument("--end-time", help="end time in yyyy-mm-dd format", required=True)
    parser_audit_log.add_argument("--limit", help="Number of audit logs to return. If not provided all audit-logs between given time-range will be returned")
