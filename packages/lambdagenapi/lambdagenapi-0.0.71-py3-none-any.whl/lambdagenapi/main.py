...
class login:
    def login(mail, password):
        ...
        default_persona = response_content["account"]["defaultPersona"]
        s.headers.update({'authorization': token})
        print(default_persona)
        print(token)

class update_persona:
    def update_persona(updatedFields):
        headers = {"authorization": "Bearer " + token}
        form_data = {'updatedFields': json.dumps(updatedFields)}
        try:
            r = s.patch('https://api.community.lambdageneration.com/api/personas/' + default_persona,headers=headers, json=form_data)
            r.raise_for_status()
            return r.content
...
class post:
    def post_message(text, hive):
        ...
        headers = {'Content-Type': multipart_data.content_type, "Authorization": "Bearer " + token}
