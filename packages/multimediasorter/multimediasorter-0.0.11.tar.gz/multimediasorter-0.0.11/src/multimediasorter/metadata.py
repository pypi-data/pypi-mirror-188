...
class TMDB(MovieMetadataApi):
    ...
    async def _match_movie(self, movie_data: dict, search_term, search_year: int = None):
        ...
        def parse_release_year(movie):
            try:
                return int(movie.get('release_date', '0000-00-00').split('-')[0])
        ...
        if search_year:
            new_result_list = list(
                filter(
                    lambda res: parse_release_year(res) == search_year, result_list
                )
            )
        elif len(result_list) > 1:
            raise MetadataQueryError(f"{search_term}: too many results ({len(result_list)}), we cant' be sure")

        for movie in new_result_list:
            # search_terms = self.clean_search_term(search_term).lower().split()
            # result_terms = movie.get("title").lower().split()
