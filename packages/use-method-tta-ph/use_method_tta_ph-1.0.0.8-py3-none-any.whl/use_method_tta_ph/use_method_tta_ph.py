...
class S3_wavfile_check:
    ...
    def s3_wavfile_playtime_check(self, wav_path, error_list=[]):
        ...
        with op(f"s3://{self.__bucket}/{wav_path}", "rb", transport_params={"client": self.__s3_resource})as wf:
            end_flag = True
...
class Ph_use_method:
    ...
    def get_bbox_IoU(self, pred_bbox, gt_bbox, decimal_point=2):
        ...
        pred_poly = box(pred_bbox_point.x1, pred_bbox_point.y1,
                        pred_bbox_point.x2, pred_bbox_point.y2)
        gt_poly = box(gt_bbox_point.x1, gt_bbox_point.y1,
                      gt_bbox_point.x2, gt_bbox_point.y2)

        iou = round(pred_poly.intersection(gt_poly).area /
                    pred_poly.union(gt_poly).area, decimal_point)
