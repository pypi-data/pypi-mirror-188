...
from brevettiai.platform.models import Dataset, CamelModel
...
class Application(CamelModel):
    ...
    def related_ids(self):
        return {self.id, *self.model_ids, *self.starred_model_ids, *self.training_dataset_ids, *self.test_dataset_ids}

    @validator("training_dataset_ids", "test_dataset_ids")
    def validate_test_and_training_dataset_ids(cls, value, values, field):
        if field.name == "training_dataset_ids" and not set(value).isdisjoint(values.get("test_dataset_ids", [])):
            raise ValueError("Training and test data must not overlap")
        if field.name == "test_dataset_ids" and not set(value).isdisjoint(values.get("training_dataset_ids", [])):
            raise ValueError("Training and test data must not overlap")
        return value

    def add_training_dataset(self, uuid):
        if id in self.training_dataset_ids:
    ...
    def add_test_dataset(self, uuid):
        if id in self.test_dataset_ids:
            raise ValueError("Dataset already in training datasets")
        # Assign to trigger validation
