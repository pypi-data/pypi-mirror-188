...
class PoolBuilder:
    ...
    def add_captcha(
        self,
        frequency: toloka.pool.QualityControl.CaptchaFrequency = toloka.pool.QualityControl.CaptchaFrequency.LOW,
        history_size: int = 10,
        min_completed_count: int = 5,
        ...
        self.pool.set_captcha_frequency(frequency)
        self.pool.quality_control.add_action(
            collector=toloka.collectors.Captcha(history_size=history_size),
            conditions=[
                toloka.conditions.StoredResultsCount >= min_completed_count,
                toloka.conditions.SuccessRate <= min_success_rate,
            ],
            action=toloka.actions.RestrictionV2(
                private_comment='Captcha', duration=duration, duration_unit=duration_unit, scope=scope
            ),
        )
        return self
...
def create_pool_params(config: CrowdConfig):
    ...
    ).add_captcha().add_skipped_assignments().add_control_rules(config.control_params).add_vacation(
