...
class plot_optuna_mod_performances():
    ...
    def plot_it(self, d_in, auto_set_lims_percentage =0.5, ylim_left=None, ylim_right=None):
        if ylim_left is None:
            x_in = 1-d_in['auc']
            self.ax1_set = self.get_y_lim_set(x_in, percent_best=auto_set_lims_percentage)

        if ylim_right is None:
            x_in = d_in['touch_count_errors_per_touch']
            self.ax2_set = self.get_y_lim_set(x_in, percent_best=auto_set_lims_percentage)
