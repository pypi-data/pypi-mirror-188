...
class APITestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._saved_checks = kaninipycs._checks
    ...
    def tearDown(self):
        ...
        kaninipycs._checks = self._saved_checks
    ...
    def test_register_physical_check(self):
        ...
        kaninipycs.register_check(check_dummy, ['Z001'])

        self.assertTrue(check_dummy in kaninipycs._checks['physical_line'])
        codes, args = kaninipycs._checks['physical_line'][check_dummy]
        self.assertTrue('Z001' in codes)
        self.assertEqual(args, ['physical_line', 'line_number'])

        options = kaninipycs.StyleGuide().options
    ...
    def test_register_logical_check(self):
        def check_dummy(logical_line, tokens):
            raise NotImplementedError
        kaninipycs.register_check(check_dummy, ['Z401'])

        self.assertTrue(check_dummy in kaninipycs._checks['logical_line'])
        codes, args = kaninipycs._checks['logical_line'][check_dummy]
        self.assertTrue('Z401' in codes)
        self.assertEqual(args, ['logical_line', 'tokens'])

        kaninipycs.register_check(check_dummy, [])
        kaninipycs.register_check(check_dummy, ['Z402', 'Z403'])
        codes, args = kaninipycs._checks['logical_line'][check_dummy]
    ...
    def test_register_ast_check(self):
        kaninipycs.register_check(DummyChecker, ['Z701'])

        self.assertTrue(DummyChecker in kaninipycs._checks['tree'])
        codes, args = kaninipycs._checks['tree'][DummyChecker]
    ...
    def test_register_invalid_check(self):
        ...
        kaninipycs.register_check(InvalidChecker, ['Z741'])
        kaninipycs.register_check(check_dummy, ['Z441'])

        for checkers in kaninipycs._checks.values():
            self.assertTrue(DummyChecker not in checkers)
            self.assertTrue(check_dummy not in checkers)

        self.assertRaises(TypeError, kaninipycs.register_check)

    def test_styleguide(self):
        report = kaninipycs.StyleGuide().check_files()
        self.assertEqual(report.total_errors, 0)
        self.assertFalse(sys.stdout)
        self.assertFalse(sys.stderr)
        self.reset()

        report = kaninipycs.StyleGuide().check_files(['missing-file'])
        ...
        report = kaninipycs.StyleGuide().check_files([E11])
        stdout = sys.stdout.getvalue().splitlines()
        self.assertEqual(len(stdout), report.total_errors)
        ...
        report = kaninipycs.StyleGuide(paths=[E11]).check_files()
        stdout = sys.stdout.getvalue().splitlines()
        self.assertEqual(len(stdout), report.total_errors)
        self.assertEqual(report.total_errors, 24)
        self.assertFalse(sys.stderr)
    ...
    def test_styleguide_options(self):
        ...
        pep8style = kaninipycs.StyleGuide(paths=[E11])

        # Check style's attributes
        self.assertEqual(pep8style.checker_class, kaninipycs.Checker)
    ...
    def test_styleguide_ignore_code(self):
        def parse_argv(argstring):
            ...
            try:
                return kaninipycs.StyleGuide(parse_argv=True)
            finally:
        ...
        pep8style = kaninipycs.StyleGuide(select='E', paths=[E11])
        self.assertFalse(pep8style.ignore_code('E112'))
        self.assertTrue(pep8style.ignore_code('W191'))
        self.assertFalse(pep8style.ignore_code('E241'))

        pep8style = kaninipycs.StyleGuide(select='W', paths=[E11])
        self.assertTrue(pep8style.ignore_code('E112'))
        self.assertFalse(pep8style.ignore_code('W191'))
        self.assertTrue(pep8style.ignore_code('E241'))

        pep8style = kaninipycs.StyleGuide(select=('F401',), paths=[E11])
    ...
    def test_styleguide_checks(self):
        ...
        options = kaninipycs.StyleGuide().options
        functions = [func for _, func, _ in options.logical_checks]
        self.assertIn(kaninipycs.indentation, functions)
        options = kaninipycs.StyleGuide(select=['E']).options
        functions = [func for _, func, _ in options.logical_checks]
        self.assertIn(kaninipycs.indentation, functions)
        options = kaninipycs.StyleGuide(ignore=['W']).options
        functions = [func for _, func, _ in options.logical_checks]
        self.assertIn(kaninipycs.indentation, functions)
        options = kaninipycs.StyleGuide(ignore=['E12']).options
        functions = [func for _, func, _ in options.logical_checks]
        self.assertIn(kaninipycs.indentation, functions)

        # Do not run E11 checks
        options = kaninipycs.StyleGuide(select=['W']).options
        functions = [func for _, func, _ in options.logical_checks]
        self.assertNotIn(kaninipycs.indentation, functions)
        options = kaninipycs.StyleGuide(ignore=['E']).options
        functions = [func for _, func, _ in options.logical_checks]
        self.assertNotIn(kaninipycs.indentation, functions)
        options = kaninipycs.StyleGuide(ignore=['E11']).options
        functions = [func for _, func, _ in options.logical_checks]
        self.assertNotIn(kaninipycs.indentation, functions)

    def test_styleguide_init_report(self):
        style = kaninipycs.StyleGuide(paths=[E11])

        standard_report = kaninipycs.StandardReport

        self.assertEqual(style.options.reporter, standard_report)
        self.assertEqual(type(style.options.report), standard_report)

        class MinorityReport(kaninipycs.BaseReport):
            pass
        ...
        style = kaninipycs.StyleGuide(paths=[E11], reporter=MinorityReport)
    ...
    def test_check_nullbytes(self):
        ...
        pep8style = kaninipycs.StyleGuide()
