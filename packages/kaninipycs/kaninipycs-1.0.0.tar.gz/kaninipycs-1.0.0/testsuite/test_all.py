...
class PycodestyleTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self._style = kaninipycs.StyleGuide(
    ...
    def test_doctest(self):
        ...
        fail_d, done_d = doctest.testmod(
            kaninipycs, verbose=False, report=False
        )
    ...
    def test_own_dog_food(self):
        files = [kaninipycs.__file__.rstrip('oc'), __file__.rstrip('oc'),
                 os.path.join(ROOT_DIR, 'setup.py')]
        report = self._style.init_report(kaninipycs.StandardReport)
