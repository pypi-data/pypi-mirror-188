...
from .distribution import UNIVARIATE_DEFAULT_THRESHOLD, validate as validate_distribution
...
def _validate_fertiliser_value(country: dict, list_key: str, threshold: float, groups: dict):
    def exec(values: tuple):
        ...
        valid, outliers, min, max = validate_distribution(
            country.get('@id'), input_id, [input_value], threshold, get_post_func=get_post, get_prior_func=get_prior
        )
        return valid or {
            'level': 'warning',
...
def validate_fertiliser_value(
    ...
    try:
        ...
        return _filter_list_errors(map(
            _validate_fertiliser_value(country, list_key, threshold, groups), enumerate(cycle.get(list_key, []))
