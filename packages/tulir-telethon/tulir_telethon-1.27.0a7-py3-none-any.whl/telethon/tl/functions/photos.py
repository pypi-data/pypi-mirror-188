...
class UploadContactProfilePhotoRequest(TLRequest):
    CONSTRUCTOR_ID = 0xb91a83bf
    SUBCLASS_OF_ID = 0xc292bd24

    def __init__(self, user_id: 'TypeInputUser', suggest: Optional[bool]=None, save: Optional[bool]=None, file: Optional['TypeInputFile']=None, video: Optional['TypeInputFile']=None, video_start_ts: Optional[float]=None):
        """
        :returns photos.Photo: Instance of Photo.
        """
        self.user_id = user_id
    ...
    def from_reader(cls, reader):
        ...
        return cls(user_id=_user_id, suggest=_suggest, save=_save, file=_file, video=_video, video_start_ts=_video_start_ts)
...
class UploadProfilePhotoRequest(TLRequest):
    ...
    def __init__(self, fallback: Optional[bool]=None, file: Optional['TypeInputFile']=None, video: Optional['TypeInputFile']=None, video_start_ts: Optional[float]=None):
        """
        :returns photos.Photo: Instance of Photo.
        """
        self.fallback = fallback
    ...
    def _bytes(self):
        return b''.join((
            b'i\x0f\xf3\x89',
            struct.pack('<I', (0 if self.fallback is None or self.fallback is False else 8) | (0 if self.file is None or self.file is False else 1) | (0 if self.video is None or self.video is False else 2) | (0 if self.video_start_ts is None or self.video_start_ts is False else 4)),
            b'' if self.file is None or self.file is False else (self.file._bytes()),
            b'' if self.video is None or self.video is False else (self.video._bytes()),
            b'' if self.video_start_ts is None or self.video_start_ts is False else (struct.pack('<d', self.video_start_ts)),
        ))
    ...
    def from_reader(cls, reader):
        ...
        if flags & 4:
            ...
            _video_start_ts = None
        return cls(fallback=_fallback, file=_file, video=_video, video_start_ts=_video_start_ts)
