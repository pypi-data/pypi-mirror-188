...
class Moral_HazardEnv(gym.Env):
    ...
    def step(self, action):
        if action == 0: # 努力
            private_profit = np.clip(np.random.normal(0.5, 0.1),0.01,0.99)# 私人收益在 0.5 左右波动
            project_success =  np.clip(np.random.normal(0.8, 0.1),0.01,0.99) # 项目成功概率在 0.8 左右波动
        else: # 卸责
            private_profit = np.clip(np.random.normal(0.7, 0.1),0.01,0.99)# 私人收益在 0.7 左右波动
            project_success = np.clip(np.random.normal(0.6, 0.1),0.01,0.99) # 项目成功概率在 0.6 左右波动
        reward = (private_profit + project_success)/2 # 计算总回报
        done = False # 环境不结束
        info = {}
        truncated = False
        terminated = False
...
if __name__ == '__main__':
    a=Moral_HazardEnv()
    print(a.observation_space.sample())
