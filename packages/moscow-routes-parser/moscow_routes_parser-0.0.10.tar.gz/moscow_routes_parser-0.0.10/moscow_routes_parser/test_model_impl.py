...
from moscow_routes_parser.model_impl import Stop_impl, Stop_builder_impl, Timetable_stop_time_t_mos_ru, \
    Timetable_stop_t_mos_ru, Timetable_t_mos_ru, Timetable_stop_builder_t_mos_ru, Timetable_builder_t_mos_ru
...
class TestStop_impl(TestCase):
    def test_stop(self):
        ...
        id_stop = 100
        coords = [10.2, 13.3]
        stop = Stop_impl(id_stop, name, coords)

        self.assertEqual(stop.get_name(), name)
        self.assertEqual(stop.get_id(), id_stop)
...
class TestStop_builder_impl(TestCase):
    def test_builder(self):
        name = "Stop"
        id_stop = 100
        stop = Stop_builder_impl().set_name(name).set_id(id_stop).build()
    ...
    def test_builder_reverse(self):
        name = "StopStop"
        id_stop = 200
        stop = Stop_builder_impl().set_id(id_stop).set_name(name).build()
...
class TestTimetable_t_mos_ru(TestCase):
    ...
    def test_timetable_t_mos_ru(self):
        ...
        stop = Timetable_stop_t_mos_ru('ул. Иванова', [20, 30], times)
        stops = [
            stop
        ]
