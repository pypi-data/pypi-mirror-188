...
class API(object):
            ...
            try:
                if self._response.status_code == status.HTTP_200_OK:
                    ...
                    if status.is_server_error(self._response.status_code):
                        self._response['error'] = f'ERP Core Server Error: HTTP Error Code {self._response.status_code}'
                    elif status.is_client_error(self._response.status_code):
                        self._response['error'] = f'ERP Sync Service Server Error: HTTP Error Code {self._response.status_code}: {self.get_full_url()}'
            except ValueError as e:
                self._response['error'] = self._response.text

        except Exception as e:
            self._response['error'] = f'ERP Sync Error: {e}'
        finally:
    ...
    def is_identity_config_set(self):
        if self.get_client_id() and self.get_client_secret() and self.get_auth_token_url() and self.get_auth_scope():
            return True
        else:
            return False
