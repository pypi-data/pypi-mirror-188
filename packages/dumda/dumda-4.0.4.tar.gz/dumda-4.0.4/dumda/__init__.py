...
def random_city(country=None, populus=False, returnobj=False, pop=__AVERAGE_POPULATION):
    """
    ...
    :param populus: optional populus specifer, if marked will only
...
def random_cities(n=10, country=None, populus=False, returnobj=False, pop=__AVERAGE_POPULATION):
    """
    ...
    if n > len(CITIES):
        n = len(CITIES)
    return iter(random_city(country, populus, returnobj, pop) for _ in range(n))
