...
class EmojiHunt():
    def __init__(self, config):
        ...
        self.emoji_size = 48
        print('Downloading background objects...')
    ...
    def update_config(self, config,seed=None):
        if not seed is None: 
            random.seed(seed)
        self.config = config
        self.emoji_augs = []
    ...
    def get_random_emoji_img(self):
        ...
        emoji_selected = random.choice(self.emojis_master_list)
        with Image.new('RGB', (self.emoji_size, self.emoji_size), (0, 0, 0)) as image:
            with Pilmoji(image) as pilmoji:
                pilmoji.text((5, 5), emoji_selected.strip(), (0, 0, 0), font, emoji_scale_factor=1.5)
        cv2_img = cv2.cvtColor(np.array(image),cv2.COLOR_RGB2BGR)
        if np.sum(cv2_img) == 0: # handel broken emojis
            return self.get_random_emoji_img() 
    ...
    def offical_test(self, function, config):
        ...
        '''
        self.update_config(config,seed=time.time())
        scores = []
        for x in range(10):
