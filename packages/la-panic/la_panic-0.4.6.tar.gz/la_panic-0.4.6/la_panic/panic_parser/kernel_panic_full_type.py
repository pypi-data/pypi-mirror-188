...
class KernelPanicFullType(Enum):
    ...
    def parse_kernel_panic_additional_info(self, additional_info_lines: [str], kernel_cache_slide: hex) -> str:
        additional_info = ""
        if self == KernelPanicFullType.DATA_ABORT:
            # Remove Debugger message, Message ID
            additional_info_lines = additional_info_lines[:-2]
            crashed_core_state = CrashedCoreState(additional_info_lines, kernel_cache_slide)
