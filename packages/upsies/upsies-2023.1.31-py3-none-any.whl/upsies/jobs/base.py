...
class JobBase(abc.ABC):
    ...
    def is_enabled(self):
        ...
        A job is enabled if :attr:`autostart` is `True`, :attr:`condition`
        returns `True`, and all :attr:`prejobs` are either finished or disabled.

        This property must be checked by the UI every time any job finishes. If
        it is `True`, this job must be :meth:`started <start>` and displayed
    ...
    def __init__(self, *, home_directory=None, cache_directory=None,
                 ...
                 condition=None, prejobs=(), callbacks={}, **kwargs):
        ...
        self._signal = utils.signal.Signal('output', 'info', 'warning', 'error', 'executed', 'finished')
    ...
    def start(self):
        ...
        if self.is_finished:
            ...
            # Job is waiting for another job (see `prejobs` property) or for a
            # condition (see `condition` property)
