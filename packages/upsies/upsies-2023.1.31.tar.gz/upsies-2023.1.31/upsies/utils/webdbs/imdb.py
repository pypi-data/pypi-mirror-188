...
class ImdbApi(WebDbApiBase):
    ...
    async def poster_url(self, id, season=None):
        if id and await self.type(id) != ReleaseType.episode:
            soup = await self._get_soup(f'title/{id}/')
    ...
    async def _runtimes(self, id):
        runtimes = await self._runtimes_new_design(id)
        if not runtimes:
            runtimes = await self._runtimes_old_design(id)
        return runtimes

    async def _runtimes_new_design(self, id):
        runtimes = {}
        if id:
            soup = await self._get_soup(f'title/{id}/technical')
            runtimes_label_tag = soup.find('button', string=re.compile(r'\bRuntime\b'))
            if runtimes_label_tag:
                values_tag = runtimes_label_tag.next_sibling
                if values_tag:
                    for li_tag in values_tag.find_all('li'):
    ...
    async def _runtimes_old_design(self, id):
        runtimes = {}
        if id:
            soup = await self._get_soup(f'title/{id}/technical')
    ...
    async def _title_original(self, id):
        ...
        return akas.get('original', '')

    async def _titles_english(self, id):
        akas = await self._get_akas(id)
        return [akas.get('english', '')]

    @utils.asyncmemoize
    async def _get_akas(self, id):
        soup = await self._get_soup(f'title/{id}')
        main_title = sub_title = None

        # The big title on the main page. This is usually the English title,
        # even for non-English movies.
        tag = soup.find(attrs={'data-testid': 'hero-title-block__title'})
        if tag:
            main_title = utils.html.as_text(tag)

        # The small title below the main title. This is the original title for
        # non-English movies and not displayed for English movies.
        tag = soup.find(attrs={'data-testid': 'hero-title-block__original-title'})
        if tag:
            text = utils.html.as_text(tag)
            sub_title = re.sub(r'^Original\s+title:\s+', '', text, flags=re.IGNORECASE)

        if sub_title:
            # Probably non-English title
            return {'original': sub_title, 'english': main_title}
        else:
            # Probably English title
            return {'original': main_title, 'english': main_title}
