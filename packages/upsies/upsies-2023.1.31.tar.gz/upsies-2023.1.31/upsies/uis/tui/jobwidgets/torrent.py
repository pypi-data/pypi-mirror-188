...
class CreateTorrentJobWidget(JobWidgetBase):
    def setup(self):
        ...
        @self.bind_key('escape', filter=Condition(lambda: self.job.activity in ('searching', 'verifying')))
        @self.bind_key('c-g', filter=Condition(lambda: self.job.activity in ('searching', 'verifying')))
        def _(event):
    ...
    def _get_current_activity(self):
        ...
        if self.job.activity == 'announce_url':
            ...
            activity = 'Press Escape to skip searching for reusable torrent'
        else:
