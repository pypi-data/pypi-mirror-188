...
class ScaleConstraints:
  ...
  def _scaleExpr(self, lp_expr, rhs = None):
    ...
    to_gdc = [int(c * multiplier) for c in coeffs]
    gcd = reduce(lambda x,y : math.gcd(x, y), to_gdc)
...
class IP2Qubo:
  ...
  def __init__(self, sense = HybridSolverOptSenses.MINIMIZE, verbose : bool = False):
    super().__init__()
