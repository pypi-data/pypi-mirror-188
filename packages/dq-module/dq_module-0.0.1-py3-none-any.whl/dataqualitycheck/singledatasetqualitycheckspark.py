    ...
    def date_format_function(self, infer_df, date_col_names, date_col_format, column_names):
        try:
            ...
            for date_column_name in date_col_names:
                failed_records = infer_df.withColumn(f"{date_column_name}_bad_record", when(to_date(col(date_column_name),
                                                                                                    date_col_format[date_column_name]).isNotNull(), True).otherwise(False)).\
                    filter(col(f"{date_column_name}_bad_record") == False).\
                    drop(col(f"{date_column_name}_bad_record")).collect(
