...
class SingleDatasetQualityCheckPolars():
    def __init__(self, tables_list, interaction_between_tables, data_read_ob, data_write_ob, data_right_structure, job_id, output_db_name="data_quality_output", no_of_partition=4):
    ...
    def write_failed_records(self, rule, failed_df, add_columns_in_folder_path=True):
        if self.data_right_structure == 'file':
            ...
            failed_record_file_name = get_failed_record_file_name()
    ...
    def drift_report(self, config_col, dataframe_col):
        ...
        return drift_df.with_column(pl.when((pl.col('config_cols') == pl.col('dataframe_cols')) & (pl.col('config_cols').is_not_null())).then(("No Drift")).otherwise(("Drift")).alias("Drift"))

    def date_format_function(self, infer_df, date_col_names, date_col_format, column_names):
        try:
            total_records_rows = []
            for date_column_name in date_col_names:
                failed_records = infer_df.with_column(pl.when(pl.col(date_column_name).str.strptime(pl.Date, date_col_format[date_column_name], strict=False).is_not_null()).then(True).otherwise(False).alias(f"{date_column_name}_bad_record")).\
            ...
            raise Exception("This date format is not supported in spark.")
        ...
        try:
            ...
            if len(date_col_names) > 0:
                failed_date_records = self.date_format_function(
                    infer_df_config_cols, date_col_names, date_col_config, rule["column_to_be_checked"])
        ...
        if total_row_count == int(rule["value"]):
            ...
            failed_df = pl.DataFrame([], columns=df.schema).with_columns(
                [
                    ...
                    pl.lit("All columns").alias("Column_Tested"),
                    pl.lit(1 if ruletype == 'Mandatory' else 0).alias(
                        "Mandatory"),
