...
mapping_datatype_dict_polars={'integer' : ['Int8','Int16','Int32','Int64','UInt8','UInt16','UInt32','UInt64',pl.Int64] ,'int' :['Int8','Int16','Int32','Int64','UInt8','UInt16','UInt32','UInt64',pl.Int64], 'long' : ['Int8','Int16','Int32','Int64','UInt8','UInt16','UInt32','UInt64',pl.Int64] ,  'str' :['Utf8','Object' ,'Categorical',pl.Utf8] , 'string' : ['Utf8','Object' ,'Categorical',pl.Utf8] , 'double' : ['Float32','Float64',pl.Float64], 'float' : ['Float32','Float64',pl.Float64] , 'date' : ['Date',pl.Date], 'bool': ["Boolean",pl.Boolean] }
...
def read_dataset(tables_list, data_read_ob, no_of_partition, run_engine = 'pyspark'):
  d={}
  try:
    for table in tables_list.keys():
      if run_engine.lower() == 'polars' :
        d[table]= data_read_ob.read_into_polars(tables_list[table], no_of_partition)
      else:
...
def get_failed_record_file_name():
  return 'failed_records' + date.today().strftime("%Y_%m_%d_") + time.strftime("%H_%M_%S", time.localtime())
...
def column_type_identifier(df, column_to_be_checked, n=1):
