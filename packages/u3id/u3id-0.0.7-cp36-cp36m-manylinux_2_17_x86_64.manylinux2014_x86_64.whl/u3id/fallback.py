...
class U3IDFactory:
    ...
    def generate(
            ...
            # ordered_component=int(time.time() * 1000),
            # chaotic_component=None,
    ):
...
if __name__ == "__main__":
    ...
    integer_time_part = 100
    decimal_time_part_nanoseconds = 999999999
    chaotic_part_seed = "this is a test"

    u3id = test_class.generate(chaotic_part_seed='this is a test')
    u3id = test_class.generate()

    u3id = test_class.generate(
        integer_time_part=integer_time_part,
        decimal_time_part_nanoseconds=decimal_time_part_nanoseconds,
        chaotic_part_seed=chaotic_part_seed
    )

    u3id_bytes = u3id.bytes
    bits = ""
    for i in range(len(u3id_bytes) - 1):
        byte = u3id_bytes[i]
        bits += f'{byte:0>8b}'
    print("py:")
    print(bits)
