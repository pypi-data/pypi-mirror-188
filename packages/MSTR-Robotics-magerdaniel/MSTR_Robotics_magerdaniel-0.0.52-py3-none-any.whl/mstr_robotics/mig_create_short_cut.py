...
class get_change_log:
    ...
    def bld_change_log_shortcut_df(self,conn,change_log_df):
        ...
        obj_df = change_log_df.groupby(["USER_ID", "USER_LOGIN", "OBJECT_ID", "OBJECT_NAME", "OBJECT_TYPE"])[
            "TRANSACTION_ID"].agg("count").reset_index()
        ...
        for index, obj in obj_df.iterrows():
            ...
            shortcut = []
            shortcut.append(obj["OBJECT_ID"])
            shortcut.append(obj["OBJECT_TYPE"])
            shortcut.append(obj["USER_LOGIN"]+ "__" + obj["OBJECT_NAME"] + "__" + obj["OBJECT_ID"])
            shortcut.append(self._bld_desc_str(change_log_df.loc[change_log_df["OBJECT_ID"] == obj["OBJECT_ID"]]))
            shortcut_l.append(shortcut)
        shortcut_df=pd.DataFrame(shortcut_l, columns=["OBJECT_ID","OBJECT_TYPE","OBJECT_NAME","OBJECT_DESC"])
        return shortcut_df

    def parse_chglog_rep_cols(self,rep_def):
        #reads out the attributes ot the changelog report
        cols=[]
