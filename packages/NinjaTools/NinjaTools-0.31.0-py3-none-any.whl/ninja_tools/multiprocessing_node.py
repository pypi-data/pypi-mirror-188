...
class Node:
    ...
    def send(self, channel, data=None):
        """
        ...
        """
        data = Object(data=data, channel=channel, namespace=self.namespace)
...
class Object:
    def __init__(self, data, channel, namespace):
        """
...
#
#     def start(self):
#         update = GetData(window_name='Classic Sakray | www.classic-sakray.com | Gepard Shield 3.0 (^-_-^)')
#         while True:
...
#     def start(self):
#         while True:
#             data = self.node.receive('update').data
#             if data:
