...
class SwitchShell(Switch):
    ...
    def get_inventory_file(self):
        ...
        password = config.git_token
        pull_repo = config.repository_to_load_inventory
    ...
    def save_config(switch_config_file: str):
        ...
        username = config.git_username
        password = config.git_token
        push_repo = config.repository_to_save_configs_for_all_switches
        temp = tempfile.mkdtemp()
    ...
    def save_configs_all_switches():
        ...
        try:
            ...
            for file in os.scandir(config.switch_configs_local_directory):
                SwitchShell.save_config(file.path)
                logging.info(f"Le fichier de configuration {file.name} a été sauvegardé sur le dépôt de sauvegarde")
        except Exception as e:
