...
log = logging.getLogger("Octrapy.http")

user_agent = "OctraBot (https://github.com/Octra/Octra.py {0}) Python/{1[0]}.{1[1]} aiohttp/{2}"


class Route:
    """Represents a Octra route"""
...
class HTTPClient:
    """Represents an HTTP client making requests to Octra."""

    def __init__(self, token: str, loop: asyncio.BaseEventLoop):
        self._token = token
        self._base_url = f"https://api.Octra.org/bot{self._token}/"
        self.messages_dict = {}
    ...
    async def request(self, route: Route, **kwargs):
        ...
        Parameters
        ----------
        route: :class:`Octrapy.http.Route`
        ...
        for tries in range(5):
            async with self.session.request(method, url, **kwargs) as resp:
                # Octra docs say all responses will have json
                ...
                # Some sort of internal Octra error.
