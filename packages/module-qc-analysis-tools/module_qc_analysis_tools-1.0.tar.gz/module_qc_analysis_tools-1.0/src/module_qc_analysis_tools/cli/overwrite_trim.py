...
class WriteConfig:
    ...
    def find_trim(self, in_file, vdd):
        # find the trim values that gives the closest to nominal vdd value.
        vdd_arr = np.array(self.lookup(in_file, vdd))
        diff_arr = np.absolute(vdd_arr - 1.2)
            ...
            for in_file in self.in_files:
                ...
                for chip_data in in_file_data:
                    ...
                    if in_chip_serial != config_chip_serial:
                        ...
                        found_chip = True
                        trimA = int(self.find_trim(chip_data, "AR_VDDA_VS_TRIM"))
                        trimD = int(self.find_trim(chip_data, "AR_VDDD_VS_TRIM"))
                        self.overwrite(config_file, "SldoTrimA", trimA)
                        self.overwrite(config_file, "SldoTrimD", trimD)
                        break
                if found_chip:
                    break
            if not found_chip:
