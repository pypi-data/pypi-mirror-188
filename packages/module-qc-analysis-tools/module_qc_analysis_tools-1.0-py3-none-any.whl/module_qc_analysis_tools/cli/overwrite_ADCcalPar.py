...
class WriteConfig:
    ...
    def find_ADCcalPar(self, in_file):
        # find the trim values that gives the closest to nominal vdd value.
        ADC_CALIBRATION_SLOPE = float(self.lookup(in_file, "ADC_CALIBRATION_SLOPE"))
        ADC_CALIBRATION_OFFSET = float(self.lookup(in_file, "ADC_CALIBRATION_OFFSET"))
        return ADC_CALIBRATION_SLOPE, ADC_CALIBRATION_OFFSET

    def overwrite(self, config_file, search_key, set_slope, set_offset):
        ...
        log.info(
            f"[{search_key}] Change from {original_search_key} to {set_offset}, {set_slope}."
        )
        original_search_key[0] = set_offset
        original_search_key[1] = set_slope
        self.stack.reverse()
