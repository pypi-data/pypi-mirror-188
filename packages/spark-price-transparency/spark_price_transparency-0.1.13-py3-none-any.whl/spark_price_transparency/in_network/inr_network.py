...
class Inr_network(TableStreamTgt):
    ...
    negotiated_rates = ArrayType(StructType([
        StructField("negotiated_prices", ArrayType(StructType([
            StructField("negotiated_type", StringType(), False),
            StructField("negotiated_rate", FloatType(), False),
            StructField("expiration_date", StringType(), False),
            StructField("service_code", ArrayType(StringType()), False),
            StructField("billing_class", StringType(), False),
            StructField("billing_code_modifier", ArrayType(StringType()), False)])), False),
        ...
        StructField("provider_references", ArrayType(LongType()), False)]))
    ...
    definition = \
        [("file_name",                 StringType(),     False, "File name of in network rate json"),
         ...
         ("negotiated_rates",          negotiated_rates, False, "Array of negotiated rate & details"),
         ("bundled_codes",             billing_array,    False, "Codes if negotiation_arrangement is a bundle"),
         ("covered_services",          billing_array,    False, "Codes is negotiation_arrangement is capitation")]
