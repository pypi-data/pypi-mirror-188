...
from jubeatools.utils import mixed_number_format
    ...
    def raise_if_timing_unfit(self, bytes_per_panel: int) -> None:
        ...
        if length % bytes_per_panel != 0:
            raise SyntaxError(
                ...
                f"{self.timing!r} is {length} bytes long, which isn't"
            )
...
class UnfinishedLongNote:
    ...
    def ends_at(self, end: BeatsTime) -> LongNote:
        if end < self.time:
            raise ValueError(
                "Tried creating a long note with an invalid end time. A long "
                f"note starting at beat {mixed_number_format(self.time)} cannot "
                f"end at beat {mixed_number_format(end)} (which is earlier)"
            )
        return LongNote(
            time=self.time,
...
class JubeatAnalyserParser:
    ...
    def do_pw(self, value: str) -> None:
        if int(value) != 4:
            raise ValueError(
                f"Found a #pw command with value {value}. This means that the "
                f"chart was written for a controller that is {value} buttons "
                "wide. jubeatools only supports charts for 4×4 controllers"
            )

    def do_ph(self, value: str) -> None:
        if int(value) != 4:
            raise ValueError(
                f"Found a #ph command with value {value}. This means that the "
                f"chart was written for a controller that is {value} buttons "
                "high. jubeatools only supports charts for 4×4 controllers"
            )
    ...
    def _wrong_format(self, f: str) -> None:
        raise ValueError(
            f"Found a {f} command, this means that this file uses another "
            "jubeat analyser format than the one the currently selected parser "
            "is designed for"
        )
