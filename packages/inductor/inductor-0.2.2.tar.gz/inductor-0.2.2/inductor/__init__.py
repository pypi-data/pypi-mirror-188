...
class _InductorFunction:
    ...
    def fastapi_wrapper_ui_internal(self, env: environment.Env) -> Coroutine:
        ...
        Args:
            env: The environment in which this function is being called.
        """
        # Wrap f
        fn = self.fn
        @functools.wraps(fn)
        async def wrapper(*args, **kwargs):
            ...
            # exception's traceback
                ...
                if isinstance(env, environment.Local):
...
class App:
    ...
    def __init__(self, env: Union[environment.Env, Dict[str, environment.Env]]):
        ...
        if isinstance(self._env, environment.Local):
            ...
            # Path for sqlite table storing Inductor-internal table catalog
            self._internal_table_catalog_path = os.path.join(
                self._table_dir, "internal_table_catalog")
            # Base directory for storage of Inductor-internal tables
            self._internal_table_storage_dir = os.path.join(
                self._table_dir, "internal")
            # Path for sqlite table storing unique ID state
            self._unique_id_state_table_path = os.path.join(
                self._internal_table_storage_dir, "unique_id_state")
            # Path for sqlite table storing app table catalog
            self._app_table_catalog_path = os.path.join(
                self._table_dir, "app_table_catalog")
            # Base directory for storage of app-created tables
            self._app_table_storage_dir = os.path.join(
                self._table_dir, "app", "__default_namespace__")
    ...
    def up(self):
        ...
        if isinstance(self._env, environment.Local):
            ...
            # Create directory for storage of internal tables
            os.makedirs(self._internal_table_storage_dir, exist_ok=True)
            # Create directory for storage of app-created tables
            os.makedirs(self._app_table_storage_dir, exist_ok=True)
            # Initialize table storing state for unique id generation, and
            # add it to the internal table catalog
            ...
            self._internal_tables.add_existing_sqlite_table(
                "unique_id_state",
                self._unique_id_state_table_path,
                "unique_id_state",
                "universe_id")
            ...
            #     "ml_model_metadata",
            #     os.path.join(
            #         self._internal_table_storage_dir,
            #         "ml_model_metadata"),
            #     "ml_model_metadata",
            #     "model_name")
            ...
            for f in self._functions:
                if f.http_url_path:
                    # Add HTTP endpoint.
                    getattr(fastapi_app, f.http_method)(f.http_url_path, response_model=None)(
                        f.fastapi_wrapper())
    ...
    def _unique_id_gen(self) -> util.UniqueIdGen:
        ...
        with App._unique_id_gen_lock:
            ...
            if isinstance(self._env, environment.Local):
                # Get prefix value from unique_id_state table
                unique_id_state_table = sqlite.SqliteKeyedTable(
                    self._unique_id_state_table_path,
                    "unique_id_state",
                    "universe_id")
    ...
    def _internal_tables(self) -> table_catalog.TableCatalog:
        ...
        if not self._run_time:
            if hasattr(self, "_internal_tables_instance_pre_run"):
                # pylint: disable=access-member-before-definition, protected-access
                self._internal_tables_instance_pre_run._run_time = self._run_time
            else:
                self._internal_tables_instance_pre_run = table_catalog.TableCatalog(
                    None,
                    None,
                    self._env,
                    self._run_time,
                    os.path.join(self._internal_table_storage_dir, "table_catalog"),
                    self.unique_id)
        ...
        internal_tables_instance = table_catalog.TableCatalog(
            sqlite.SqliteKeyedTable(
                ...
                "internal_table_catalog_metadata",
                primary_key_column="id"),
            sqlite.SqliteKeyedTable(
                self._internal_table_catalog_path,
                "internal_table_catalog_name_to_id",
            ...
            self.unique_id)
        if hasattr(self, "_internal_tables_instance_pre_run"):
            internal_tables_instance._table_creation_instructions = (
                self._internal_tables_instance_pre_run._table_creation_instructions)
    ...
    def tables(self) -> table_catalog.TableCatalog:
        ...
        tables_instance = table_catalog.TableCatalog(
            sqlite.SqliteKeyedTable(
                ...
                "app_table_catalog_metadata",
                primary_key_column="id"),
            sqlite.SqliteKeyedTable(
                self._app_table_catalog_path,
                "app_table_catalog_name_to_id",
                primary_key_column="name"),
            self._env,
            self._run_time,
            self._app_table_storage_dir,
            ...
            self._get_secrets().get("tables"))
        if hasattr(self, "_tables_instance_pre_run"):
            tables_instance._table_creation_instructions = (
                self._tables_instance_pre_run._table_creation_instructions)
    ...
    def Page(self, title: str) -> ui.Page:
