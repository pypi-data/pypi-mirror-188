...
class HardwareMachineModel(str, Enum):
    ...
    A10G = "g5.2xlarge"
    EPYC = "c5a.2xlarge"
    # CPU is deprecated, cascade_lake is relevant,
    # python aliases duplicate enum values to the first key to use them,
    # so cascade_lake should be before cpu. cpu's label will be broken :(, but that's fine.
    # the reverse isn't.
...
def type_with_model(model: HardwareMachineModel, override_label: str = None) -> str:
...
class HardwareTypeLabel(str, Enum):
    ...
    # Cascade_lake must me above cpu, see comment above.
    ...
    # deprecated
    CPU = type_with_model(HardwareMachineModel.CPU)
