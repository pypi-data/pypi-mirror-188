...
@unittest.skip('Test added wih connected data asset')
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_03_initialize_optimizer(self):
        ...
        if is_cp4d():
            #snap estimators are not available for CPD
            estimators = [self.experiment.RegressionAlgorithms.LR,
                          self.experiment.RegressionAlgorithms.EX_TREES,
                          self.experiment.RegressionAlgorithms.RIDGE]

        else:
            estimators = [self.experiment.RegressionAlgorithms.LR,
                          self.experiment.RegressionAlgorithms.EX_TREES,
                          self.experiment.RegressionAlgorithms.SnapBM]
        ...
        TestAutoAIRemote.remote_auto_pipelines = self.experiment.optimizer(
            ...
            include_only_estimators=estimators,
    ...
    def test_08a_get_feature_importance(self):
        ...
        for column in self.text_columns:
            self.assertIn(f'word2vec({column})', str(str_feature_importance_index_list),
                          msg=f"word2vec({column}) is not in features importance table."
                              f" Full features list: {str_feature_importance_index_list}")
    ...
    def test_14__get_pipeline__load_lale_pipeline__pipeline_loaded(self):
        TestAutoAIRemote.pipeline = self.remote_auto_pipelines.get_pipeline(pipeline_name='Pipeline_6')
