...
class AutoPipelinesRuns(BaseAutoPipelinesRuns):
    ...
    def list(self) -> 'DataFrame':
        ...
        def get_value(wml_pipeline_id, timestamp, run):
            if not self.auto_pipeline_optimizer_name or self.auto_pipeline_optimizer_name in run['metadata'].get('description', {}):
                try:
        ...
        **Example**
        ...
        if kb_node_number is not None:
            ...
            params = {
                ...
                'scoring': tsad_parameters.get('evaluation_metric', 'f1'),
