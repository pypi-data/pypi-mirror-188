...
def refresh_bi_to_warehouse_connections(
    ...
    BiService(config=ctx["config"]).refresh_bi_to_warehouse_connections(
        bi_container_id, warehouse_source_details
    )
...
def update_databricks_notebook(ctx, **kwargs):
    DatabricksOnboardingService(config=ctx["config"]).update_databricks_notebook(
        **kwargs
    )
