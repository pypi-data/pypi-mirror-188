...
def replica(name, pod):
    # it has been tested for the else case with the following values
    # pod = 'acc-uc2orbk-0-0-4-00036-gameserver13-7reio-min1'
    # name = 'gameserver13'
    m = re.search(r'\d+$', name)
    if m is None:
        replica_name = name + str(0)
    else:
        matching_number = m.group()
        number = int(matching_number) + 1
        if matching_number in name:
            replica_name = name.replace(matching_number, "")
            replica_name = replica_name + str(number)
    if name in pod:
        pod = pod.replace(name, replica_name)
    return pod


def find_application_instance(componentInfo):
    # removes minID
    namespace = re.sub(r'min[0-9]', "", componentInfo)
...
def scale_out_to_k8s(pod, intermediate_model):
    ...
    pod = replica(component_name, pod)
                            ...
                            if ports:
                                for port in ports:
                                    if port.get('protocol') is None:
                                        content = {'port': int(port.get('port')),
