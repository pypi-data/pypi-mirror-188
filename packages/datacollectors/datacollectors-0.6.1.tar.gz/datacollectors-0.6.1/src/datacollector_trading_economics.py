...
parms_path = f"{Path(os.getcwd()).resolve().parent}/parms/"

def main():
    start_date=datetime(2021,1,1)
    for commodity in ['ttf_gas','urea_ammonium','EUA']:
...
class Trading_economics():
    ...
    def commodity_prices(commodity,start_date):
        indata_list=[]
        commodity_list = utils.load_csv(f"{parms_path}trading_economics_links.csv")
