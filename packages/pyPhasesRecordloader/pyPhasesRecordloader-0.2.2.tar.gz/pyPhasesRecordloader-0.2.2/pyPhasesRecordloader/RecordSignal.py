    ...
    def getFirstSignalByName(self, nameList) -> Signal:
        try:
            ...
            if "FLATLINE" in nameList:
                ...
                return self.getFirstSignalByName(["FLATLINE"])
            raise Exception("Non of the target signals (%s) is present in the record: %s"%(nameList, self.signalNames))
    ...
    def plot(
        ...
        if len(self.labelSignals) > 0:
            for labelIndex, labelSignal in enumerate(self.labelSignals):
                ...
                if classification:
                    classNames = classification["classNamesTest"] if "classNamesTest" in classification else classification["classNames"]
