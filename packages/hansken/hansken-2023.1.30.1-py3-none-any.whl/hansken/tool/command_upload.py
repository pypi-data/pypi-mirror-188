...
def upload(connection_or_endpoint, image_id, image_file, key,
    ...
    def run_image_tool(destination, uploaded_image):
        ...
        # limit number of lines output to 1024, but count the total number of lines captured
        console_lines = deque(maxlen=1024)
        num_lines = 0
        ...
        message = 'conversion process (image id {image_id}) terminated with return code {return_code}, ' \
                  'output follows {tail}\n---\n{output}'
        tail = '' if len(console_lines) == num_lines else f'(last {len(console_lines)} of total {num_lines} lines)'
        if process.wait() != 0:
