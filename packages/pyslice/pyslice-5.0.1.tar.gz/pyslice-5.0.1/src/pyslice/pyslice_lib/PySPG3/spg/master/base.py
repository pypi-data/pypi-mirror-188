...
class SPGMasterDB:
    ...
    def __init__(self, connection=None, EnsembleConstructor=ParameterEnsemble):
        if not connection:
            ...
            self.connection = sql.connect(
                "%s/spg-pool.sqlite" % CONFIG_DIR, timeout=TIMEOUT
            )
        else:
    ...
    def __init_db(self):
        ...
        self.cursor.execute(
            "INSERT INTO queues (name, max_jobs, status) VALUES ('default',1,'R') "
        )
    ...
    def update_list_ensemble_dbs(
        self, status=None
    ):  # These are the dbs that are registered and running
        self.result_dbs = {}
        if status:
            res = self.cursor.execute(
                "SELECT id, full_name, weight, queue, status FROM dbs  WHERE status = ?",
                status,
            )
        else:
            res = self.cursor.execute(
                "SELECT id, full_name, weight, queue, status FROM dbs "
            )
    ...
    def write_ensemble_to_master(self, param_db):
        ...
        res = self.cursor.execute(
            "SELECT * FROM dbs WHERE full_name = ?", (param_db.full_name,)
        ).fetchone()
        ...
        if db_status["process_error"] == 1:
            self.cursor.execute(
                "UPDATE dbs SET status = ? WHERE full_name = ? ",
                ("D", param_db.full_name),
            )
