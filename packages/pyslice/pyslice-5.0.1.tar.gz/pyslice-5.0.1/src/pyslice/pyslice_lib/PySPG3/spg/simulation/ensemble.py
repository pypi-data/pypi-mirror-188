...
class ParameterEnsemble:
    ...
    def __connect_db(self):
        try:
            ...
            utils.newline_msg(
                "ERR", "database '%s' does not exist... exiting" % self.full_name
            )
    ...
    def parse_output_line(self, output_line):
        ...
        if table_name not in self.table_columns:
            utils.newline_msg(
                "ERR", "DB does not contain table named '%s'" % table_name
            )
            sys.exit(1)
        # try:
    ...
    def init_db(self):
        try:
            (self.command,) = self.execute_query_fetchone(
                "SELECT value FROM information WHERE key = 'command'"
            )
        except:
            utils.newline_msg(
                "FATAL",
                "Cannot retrieve information from database '%s'..."
                % utils.shorten_name(self.db_name),
            )
            sys.exit(2)
        ...
        #:::~ get the names of the columns
        sel = self.execute_query(
            "SELECT name FROM entities WHERE varies = 1 ORDER BY id"
        )
        ...
        table_names = [
            i[0] for i in self.execute_query("SELECT DISTINCT name from output_tables")
        ]
        ###   print table_names
        for table in table_names:
            fa = self.execute_query(
                "SELECT column FROM output_tables WHERE name = '%s';" % table
            )

            self.table_columns[table] = ["spg_uid", "spg_vsid", "spg_rep"] + [
                i[0] for i in fa
            ]
    ...
    def query_set_run_status(self, status, id=None, run_time=None):
        ...
        if run_time:
            self.execute_query(
                'UPDATE run_status SET status ="%s", run_time=%f WHERE id = %d'
                % (status, run_time, id)
            )
        else:
            self.execute_query(
                'UPDATE run_status  SET status ="%s" WHERE id = %d' % (status, id)
            )
    ...
    def __next__(self):
        ...
        query = (
            "SELECT r.id, r.spg_vsid, r.spg_rep, %s FROM run_status AS r, values_set AS v "
            % ", ".join(["v.%s" % i for i in self.entities])
        ...
        for (pos, ent) in enumerate(self.entities):
            self.values[ent] = res[
                pos + 3
            ]  # There are three parameters we get from the table, the others are the entities
    ...
    def get_updated_status(self):
        ...
        ret_dict = {}
        (ret_dict["value_set_with_rep"],) = self.execute_query_fetchone(
            "SELECT COUNT(*) FROM run_status ;"
        )
        (ret_dict["value_set"],) = self.execute_query_fetchone(
            "SELECT COUNT(*) FROM values_set ;"
        )
        ret_dict["process_done"] = 0
        ret_dict["process_not_run"] = 0
        ret_dict["process_running"] = 0
        ret_dict["process_error"] = 0
        (ret_dict["max_run_time"],) = self.execute_query_fetchone(
            "SELECT max(run_time) FROM run_status WHERE status = 'D';"
        )
        (ret_dict["min_run_time"],) = self.execute_query_fetchone(
            "SELECT min(run_time) FROM run_status WHERE status = 'D';"
        )
        (ret_dict["avg_run_time"],) = self.execute_query_fetchone(
            "SELECT avg(run_time) FROM run_status WHERE status = 'D';"
        )
        (ret_dict["total_run_time"],) = self.execute_query_fetchone(
            "SELECT sum(run_time) FROM run_status WHERE status = 'D';"
        )

        ret = self.execute_query(
            "SELECT status, COUNT(*) FROM run_status GROUP BY status"
        )
...
class ParameterEnsembleExecutor(ParameterEnsemble):
    def __init__(
        ...
        ParameterEnsemble.__init__(
            self, full_name, id, weight, queue, status, repeat, init_db
        )
        #        self.init_db()
        os.chdir(self.path)
    ...
    def launch_process(self, remove_files=True):
        ...
        configuration_filename = "%s_%d.tmp_input" % (
            self.base_name,
            self.current_spg_uid,
        )
        ...
        )  # , env = {'PYTHONPATH':"${PYTHONPATH}:%s"%ROOT_DIR})
    ...
    def dump_result(self):
        ...
        for line in self.output:
            ...
            output_columns = [
                self.current_spg_uid,
                self.current_spg_vsid,
                self.current_spg_rep,
            ] + output_columns
            #            print(self.table_columns[table_name],["'%s'" % str(i) for i in output_columns])

            cc = "INSERT INTO %s (%s) VALUES (%s) " % (
                table_name,
                ", ".join(self.table_columns[table_name]),
            ...
            try:
                ...
                utils.newline_msg(
                    "DB",
                    "Fatal, '%d' values for the '%d' output columns %s expected "
                    % (nv - 3, nc - 3, self.table_columns[table_name][3:]),
                )
                sys.exit(1)
            #     self.query_set_run_status("E")
    ...
    def launch_process(self, current_run_id, current_vsid, current_rep, values):
        ...
        if self.test_run:
            ...
            os.rename(
                ...
                "configuration-%s/%s" % (self.base_name, configuration_filename),
            )
            os.rename(
                fname_stdout, "configuration-%s/%s" % (self.base_name, fname_stdout)
            )
            os.rename(
                fname_stderr, "configuration-%s/%s" % (self.base_name, fname_stderr)
            )
...
class ResultsDBQuery(ParameterEnsemble):
    ...
    def setup_vars_in_table(self, conf):
        ...
        if set(in_table_vars).issubset(set(self.variables)):
            ...
            self.coalesced_vars = [
                i for i in self.coalesced_vars if (i not in self.vars_in_table)
            ]
            self.separated_vars = [
                i for i in self.separated_vars if (i not in self.vars_in_table)
            ]

            orphaned = (
                set(self.variables)
                - set(self.separated_vars)
                - set(self.vars_in_table)
                - set(self.coalesced_vars)
            )
            if len(orphaned) > 0:
                utils.newline_msg(
                    "VAR",
                    "orphaned variables '%s' added to separated variables" % orphaned,
                    indent=4,
                )
                for i in orphaned:
                    self.coalesced_vars.append(i)
            print(
                "  +- structure = %s - %s - %s "
                % (self.separated_vars, self.coalesced_vars, self.vars_in_table)
            )
        else:
            #    print in_table_vars, conf
            utils.newline_msg(
                "VAR",
                "the variables '%s' are not recognised" % set(in_table_vars)
                - set(self.variables),
            )
    ...
    def setup_vars_separated(self, conf):
        ...
        if set(separated).issubset(set(self.variables)):
            ...
            self.coalesced_vars = [
                i for i in self.coalesced_vars if (i not in self.separated_vars)
            ]
            self.vars_in_table = [
                i for i in self.vars_in_table if (i not in self.separated_vars)
            ]
            orphaned = (
                set(self.variables)
                - set(self.separated_vars)
            ...
            utils.newline_msg(
                "VAR",
                "the variables '%s' are not recognised" % set(separated)
                - set(self.variables),
            )
    ...
    def setup_vars_coalesced(self, conf):
        ...
        if set(coalesced).issubset(set(self.variables)):
            ...
            self.separated_vars = [
                i for i in self.separated_vars if (i not in self.coalesced_vars)
            ]
            self.vars_in_table = [
                i for i in self.vars_in_table if (i not in self.coalesced_vars)
            ]
            orphaned = (
                set(self.variables)
                - set(self.separated_vars)
            ...
            utils.newline_msg(
                "VAR",
                "the variables '%s' are not recognised" % set(coalesced)
                - set(self.variables),
            )
    ...
    def clean_dict(self, dict_to_clean):
        ...
        for i in dict_to_clean:
            try:
                ...
                dict_to_clean[i] = "'%s'" % (
                    dict_to_clean[i].replace("'", "").replace('"', "")
                )
    ...
    def values_set_table(self):
        ...
        if len(local_vars_in_table) > 0:
            local_var_query = "%s " % ",".join(
                ["v.%s" % v for v in local_vars_in_table]
            )

        query = (
            "SELECT rs.spg_vsid, rs.spg_rep, rs.id, %s FROM run_status AS rs, values_set AS v WHERE rs.spg_vsid = v.id "
            % (local_var_query)
        )
        ...
        ret_table = [
            [
                utils.replace_values(el, dict(list(zip(query_cols, row))))
                for el in row[2:]
            ]
    ...
    def result_table(
        self, table="results", table_selector="grouped_vars", restrict_to_values={}
    ):
        """
        Args:
        ...
        if table_selector == "grouped_vars" or table_selector == "raw_vars":
            ...
            output_columns = self.table_columns[table][
                3:
            ]  #:::~ skips the spg_uid, spg_vsid and spg_rep columns ...
        ...
        if table_selector == "grouped_vars":
            ...
            output_columns_query = " %s" % ",".join(
                ["AVG(r.%s)" % v for v in output_columns]
            )
        else:
            output_columns_query = " %s" % ",".join(
                ["r.%s" % v for v in output_columns]
            )

        query = (
            "SELECT %s %s %s FROM %s AS r, values_set AS v WHERE r.spg_vsid = v.id "
            % (id_cols_query, local_var_query, output_columns_query, table)
        )

        if restrict_to_values:
            self.clean_dict(restrict_to_values)
            restrict_to_values_query = " AND ".join(
                [
                    "v.%s = '%s'" % (v, restrict_to_values[v])
                    for v in list(restrict_to_values.keys())
                ]
            )
    ...
    def __iter__(self):
        ...
        if not vars_to_separate:
            ...
            query = "SELECT DISTINCT %s FROM values_set " % (
                ",".join([v for v in vars_to_separate])
            )
    ...
    def update_results_from_data(self, table_file, table_name="results", sep=","):
        ...
        for row in table:
            uid = int(row[0])
            (vsid, rep) = self.execute_query_fetchone(
                "SELECT spg_vsid, spg_rep FROM run_status WHERE id = %s" % uid
            )
            vals = row[1:]
