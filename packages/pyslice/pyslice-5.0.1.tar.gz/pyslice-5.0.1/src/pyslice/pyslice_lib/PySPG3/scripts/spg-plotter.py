...
class SPGPlotter:
    def __init__(self, parameter_file):
        ...
        if x_axis_iter.type == "*":
            ...
            self.add_setting(
                self.x_axis, "lim = (%s,%s)" % (x_axis_iter.xmin, x_axis_iter.xmax)
            )
        ...
        for line in open(cfgFile):
            ...
            for o in l:
                ...
                if k not in possible_keys:
                    newline_msg(
                        "SYN", "in column '%s', unrecognised key '%s'" % (name, k)
                    )
                    sys.exit(1)
                if k == "lim":
                    values[k] = eval(v)
    ...
    def add_setting(self, var, line):
        ...
        for o in l:
            ...
            if k not in possible_keys:
                spgu.newline_msg(
                    "SYN", "in column '%s', unrecognised key '%s'" % (var, k)
                )
    ...
    def plot_all(self, Plotter, mean, std):
        newline_msg(
            "INF",
            "%s - %s - %s" % (self.separated_vars, self.coalesced_vars, self.x_axis),
        )
...
def parse_command_line():
    ...
    parser.add_option(
        "--mean", action="store_true", dest="mean", default=None, help="averages"
    )

    parser.add_option(
        "--std", action="store_true", dest="std", default=[], help="averages"
    )
