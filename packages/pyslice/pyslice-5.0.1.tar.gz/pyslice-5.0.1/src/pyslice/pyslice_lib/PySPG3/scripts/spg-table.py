...
class SPGResultsCommandLine(BaseSPGCommandLine):
    ...
    def do_save_input_table(self, c):
        ...
        writer = csv.writer(
            open(output_fname, "w"), delimiter=self.sep, lineterminator="\n"
        )
    ...
    def do_setup_vars_separated(self, c):
        ...
        if "restore" in flags:
            self.current_param_db.setup_vars_separated(
                ",".join(self.current_param_db.variables[:-1])
            )
        elif "empty" in flags:
            self.current_param_db.setup_vars_separated("")
        else:
    ...
    def do_setup_output_column(self, c):
        ...
        if not set(cols).issubset(self.current_param_db.table_columns[self.table]):
            utils.newline_msg(
                "ERR",
                "the column(s) is (are) not in the output: %s"
                % (set(cols) - set(self.current_param_db.table_columns[self.table])),
            )
    ...
    def do_set_as_var(self, c):
        ...
        if not set(ls_vars).issubset(set(self.current_param_db.entities)):
            utils.newline_msg(
                "VAR",
                "the variables '%s' are not recognised" % set(ls_vars)
    ...
    def do_set_table(self, c):
        ...
        if c not in list(self.current_param_db.table_columns.keys()):
            utils.newline_msg(
                "ERR",
                "table '%s' not among the ones found in the DB: (%s)"
                % (c, ", ".join(list(self.current_param_db.table_columns.keys()))),
            )
            return
    ...
    def do_conf(self, c):
        ...
        print(
            "  + vars (separated-coalesced-in_table) = %s - %s - %s "
            % (
                self.current_param_db.separated_vars,
                self.current_param_db.coalesced_vars,
