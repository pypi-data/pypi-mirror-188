...
class BaseDataLoader:
    def describe_variables(self):
        for v in self.variables:
            ...
            print(
                "%-16s: %4s values || %s -- %s"
                % (v, len(red_d), min(red_d), max(red_d))
            )
...
class CSVDataLoader(BaseDataLoader):
    ...
    def configure_vars(
        ...
        recalculate_output_columns=True,
    ):
        ...
        if recalculate_output_columns:
            self.output_columns = [
                kn for kn in list(self.data.keys()) if kn not in self.variables
            ]
...
class SPGDataLoader(BaseDataLoader):
    ...
    def __init__(self, exec_name, simulation_filename):
        ...
        try:
            ...
            spgu.newline_msg(
                "INF", "no 'input' file found: %s" % self.simulation.command
            )
        ...
        x_axis_iter = self.simulation.data[
            self.simulation.position_of(self.independent_var)
        ]
        if self.independent_var not in self.settings:
            self.settings[self.independent_var] = spgu.SPGSettings()
        if x_axis_iter.type == "*":
            self.settings[self.independent_var]["scale"] = "log"
            self.settings[self.independent_var]["lim"] = (
                x_axis_iter.xmin,
                x_axis_iter.xmax,
            )
        ...
        print("[__init__] constants: %s" % list(self.constants.keys()))
        print(
            "[__init__] independent variables:",
            self.separated_vars,
            self.coalesced_vars,
    ...
    def configure_vars(self, separated_vars, coalesced_vars, independent_var=None):
        ...
        try:
            ...
            print(
                "[configure_vars] ",
                set(all_vars) - set(self.data.keys()),
                "not recognised as column name",
            )
            return
        # All sets are disjoint
        try:
            ...
            print("[configure_vars] ", independent_var, "is  in separated_vars")
            return
...
class SPGInteractivePlotter:
    ...
    def __init__(self, splotter, xscale="linear", yscale="linear"):
        ...
        #            print vec_labels
        self.dd_filter = ipyw.Dropdown(options=vec_labels)  # ,
        # description = "%s :"%(self.separated_vars) )
        self.dd_output = ipyw.Dropdown(options=self.output_columns)  # ,

        self.select_xscale = ipyw.Checkbox(
            value=xscale == "log", description="x log scale", icon="check"
        )

        self.select_yscale = ipyw.Checkbox(
            value=yscale == "log", description="y log scale", icon="check"
        )
        ...
        display(
            ipyw.HBox(
                [
                    ipyw.HBox(
                        [ipyw.Label("%s: " % (self.separated_vars)), self.dd_filter]
                    ),
    ...
    def __separated_value_change(self):
        query_str = " & ".join(
            ["(%s==%s)" % i for i in zip(self.separated_vars, self.separated_values)]
        )
        # print query_str
        if len(query_str) > 0:
    ...
    def redraw(self):
        ...
        for ix, iv in enumerate(self.coalesced_values):
            query_inn_str = " & ".join(
                ["(%s==%s)" % i for i in zip(self.coalesced_vars, iv)]
            )
            ...
            self.axis.lines[ix].set_data(
                local_df[self.independent_var], local_df[self.dependent_var]
            )
    ...
    def draw(self):
        ...
        for iv in self.coalesced_values:
            ...
            #                print query_inn_str
            self.axis.plot(
                local_df[self.independent_var],
                local_df[self.dependent_var],
                linestyle="",
    ...
    def savefig(self, filename, draw_legend=True):
        ...
        if "label" in self.settings[self.independent_var]:
            xlabel = self.settings[self.independent_var]["label"]
        else:
            xlabel = self.independent_var.replace("_", " ")

        if "label" in self.settings[self.dependent_var]:
        ...
        fig_out.savefig(filename, bbox_inches="tight", transparent=True, pad_inches=0)
...
class SPGInteractiveScalingPlotter:
    ...
    def __init__(self, splotter):
        ...
        self.variables = [
            i for i in list(self.full_data.keys()) if i not in self.constants
        ]
        ...
        self.select_xscale = ipyw.Checkbox(
            value=False, description="x log scale", icon="check"
        )

        self.select_yscale = ipyw.Checkbox(
            value=False, description="y log scale", icon="check"
        )
    ...
    def interact(self):
        ...
        for sp in sorted(self.__scaling_parameters.keys()):
            ...
            new_slider = ipyw.FloatSlider(
                value=value, min=value_min, max=value_max, step=step
            )
        ...
        display(
            ipyw.VBox(
                [
                    ipyw.HBox(
                        [
                            ipyw.Label("%s: " % (self.separated_vars)),
                            self.dd_filter,
                            self.select_xscale,
                            self.select_yscale,
                        ]
    ...
    def setup_transforms(self, xt, yt):
        ...
        possible_keys = (
            ...
            + self.variables
            + self.output_columns
            + list(self.__scaling_parameters.keys())
        )
        #        print possible_keys
    ...
    def add_scaling_parameter(
        self, parameter_name, value, value_min=0.0, value_max=None, step=0.01
    ):
    ...
    def redraw(self):
        ...
        for ix, iv in enumerate(self.coalesced_values):
            ...
            self.axis.lines[ix].set_data(
                local_df["transformed_x"], local_df["transformed_y"]
            )
    ...
    def draw(self):
        ...
        for iv in self.coalesced_values:
            ...
            if len(query_inn_str) > 0:
                ...
                local_df = self.data_transformed
            self.axis.plot(
                local_df["transformed_x"],
                local_df["transformed_y"],
                linestyle="",
                marker=next(marker_it),
                color=next(color_it),
                label=query_inn_str,
