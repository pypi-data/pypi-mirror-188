...
class Manager:
    ...
    def run(self) -> int:
        if self.simulate:
            print(bcolors.WARNING + '+ Simulate' + bcolors.ENDC)
    ...
    if hasattr(args, 'which'):
        if args.which == 'deploy':
            try:
                ...
                print(
                    bcolors.FAIL + 'Config file is not compatible with this version. Please run `init` again.' + bcolors.ENDC)
                return 1
            ...
            print(bcolors.OKGREEN + 'Deployed eventbus' + bcolors.ENDC)
            return 0
        elif args.which == 'init':

            print('# GCP Config')
            config.project = args.project or helpers.ask_for('Project')
            config.region = args.region or helpers.ask_for('Region', 'europe-west1')
            ...
            if args.secrethub_endpoint_user is None and args.secrethub_endpoint_pwd is None:
                ...
                if helpers.ask_for_yes_no('Create items on secrethub?'):
                    ...
                    hub.write(config.secrethub_endpoint_user, 'eventbus')
            ...
            config.save()
            print(bcolors.OKGREEN + f'Config saved under `{Config.FILENAME}`' + bcolors.ENDC)
            return 0
