    ...
    if ALLOW_PARAM == True:
        ...
        if str(_type).strip()!='':
            if str(_type).strip() == ConstTypeProcessOfAirtable.log.value or str(_type).strip() == 'Log-pod':
                ...
                bStatus_cmplte_sshcmd = False;except_msg_handler = ''
                if len(_lst_target) == 0 and ("restart>0" in _lst_patterns):
                    ...
                    if bStatus_cmplte_sshcmd == True:
                        if len(lst_pods_restarted) == 0:
                            SKIP_START_PROCESS = True
                            Q.logger(time7.currentTime7(),'       Skip Start Process / lst_pods_restarted is None')
                        else: _lst_target = lst_pods_restarted
        ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        try: 
            dict_rawiPodC = []
        ...
        if len(DICT_RELEASE_PodC) !=0 and len(_lst_patterns)!= 0 and SKIP_START_PROCESS == False:
        ...
        if len(lst_event_result_rollout_restart)!=0:
            ...
            lst_event_result_rollout_restart_filter_successful = list(filter(lambda x: x['response rollout restart'] == 'True', lst_event_result_rollout_restart))    # <---- FILTER LIST
            lst_event_result_rollout_restart_filter_fail = list(filter(lambda x: x['response rollout restart'] == 'False', lst_event_result_rollout_restart))         # <---- FILTER LIST
            ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            ## Level 2
        ...
        if len(lst_dtect_obj)!=0 and (AIRTABLE_PROCESS_TYPE == 'LOG-POD' or AIRTABLE_PROCESS_TYPE == 'RESTART-POD'):
