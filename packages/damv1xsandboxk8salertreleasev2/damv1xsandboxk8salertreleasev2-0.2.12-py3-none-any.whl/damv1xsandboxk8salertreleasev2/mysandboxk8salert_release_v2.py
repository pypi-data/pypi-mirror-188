...
def execution( _number, _nameof_msg_rpt, _type, _namespace, _sincelast, _lst_patterns, _lst_target=[], **kwargs):
    ...
    if ALLOW_PARAM == True:
                                ...
                                if get_bStatus_complete_sshcmd == False:
                                    ...
                                    ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                                        if AIRTABLE_PROCESS_TYPE == 'LOG-POD':
                                            ...
                                            Q.logger(time7.currentTime7(),'             Filtering by pattern for restart-pod . . .')
                                            Q.logger(time7.currentTime7(),'             - Check pod name | container: {0} | {1}'.format(str(pod_name), str(cont_name)))
                                            Q.logger(time7.currentTime7(),'             - Check count restart:',str(cont_restart))
                                            ## * * * * * * * * * * * * * * * *
                                            bDetected = False
                                            ## * * * * * * * * * * * * * * * *
                                            # Q.logger(time7.currentTime7(),'             Core of collect information is finish')
        ...
        if AIRTABLE_PROCESS_TYPE == 'LOG-POD' or AIRTABLE_PROCESS_TYPE == 'RESTART-POD':
            ...
            ## - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            ## -- Change this for protocol type process airtable | 26 Januari 2023
            if AIRTABLE_PROCESS_TYPE == 'LOG-POD' and len(lst_grplines) != 0:
                try: evrnt_urlshareable = eCls.evernote_generate_report_v2(contexid, nameof_msg_rpt, lst_grplines, tCls.tmplt_evrntwrapper_v2.value, \
                ...
                try:
                    Q.logger(time7.currentTime7(),'             Evernote template for this type is under construction.')
                except Exception as e: Q.logger(time7.currentTime7(),'Error Handling ( エラー ):',str(e))
