...
class SocketEventBreeze(socketio.ClientNamespace):
    ...
    def connect(self):
        ...
        self.sio.connect(self.hostname, headers={"User-Agent": "python-socketio[client]/socket"}, auth=auth, transports="websocket", wait_timeout=50)
...
class BreezeConnect():
    ...
    def subscribe_feeds(self, stock_token="", exchange_code="", stock_code="", product_type="", expiry_date="", strike_price="", right="", get_exchange_quotes=True, get_market_depth=True, get_order_notification=False,get_strategy = None):
        if self.sio_handler:
            return_object = {}
    ...
    def parse_data(self, data):
        ...
        elif data and type(data) == list and len(data) > 0 and type(data[0]) == str and "!" not in data[0]:
            order_dict = {}
...
class ApificationBreeze():
    ...
    def get_names(self, exchange_code, stock_code):
        try:            
            ...
            result["company_name"] = " ".join(requiredresult[' "CompanyName"'].to_string().split()[1:])
