...
class GPF:
    ...
    def __init__(
        ...
        # TODO: ADD WAY TO BUILD TRAIN_X AND TEST_X USING THE SAMPLES THAT COME FROM THE DATASET AT IRREGULAR INTERVALS
    ...
    def _build_model(
        ...
        for i in range(self.groups["train"]["s"]):
            ...
            if self.gp_type == "exact" or self.gp_type == "ski":
                model_list.append(
                    ExactGPModel(
                        train_x=x,
                        train_y=y[:, i],
            ...
            elif self.gp_type == "svg" or self.gp_type == "svg_predloglike":
                model_list.append(
                    SvgGPModel(
                        train_x=x,
                        train_y=y[:, i],
            ...
            elif self.gp_type == "ngd" or self.gp_type == "ngd_predloglike":
                model_list.append(
                    NgdGPModel(
                        train_x=x,
                        train_y=y[:, i],
    ...
    def train(
        ...
        elif self.gp_type in ("svg_predloglike", "ngd_predloglike"):
            self.mll = SumVariationalELBO(
                likelihood,
                model,
