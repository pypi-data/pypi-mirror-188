...
def make_pack(box):
    ...
    return {"data": list(box), "vars": serialise_vars(box)}
...
class Manager:
    ...
    def __init__(self, default_enabled=True):
        self.handlers = {"box": JSON(), "cbox": CJSON(), "npz": NPZ()}
        self.specified = None
    ...
    def _load(self, source, handler=None, **kwargs):
        ...
        if h is None and self.default_enabled:
            ...
            source += ".cbox"
        if h is None:
            raise ValueError("No valid handler found for source: " + str(source))
    ...
    def save(self, box, target, handler=None, **kwargs):
        ...
        if h is None and self.default_enabled:
            ...
            target += ".cbox"
        if h is None:
            raise ValueError("No valid handler found for target: " + str(target))
