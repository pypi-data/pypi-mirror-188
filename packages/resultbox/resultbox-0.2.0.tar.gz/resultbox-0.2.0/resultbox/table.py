        ...
        if isinstance(variable, (list, tuple)):
            for v in variable:
                if not isinstance(v, Variable):
                    raise ValueError(
                        "Did not receive a Variable instance for " + str(v)
                    )
                s += v + "\n" + v.doc + "\n\n"
        else:
            if not isinstance(variable, Variable):
                raise ValueError(
                    "Did not receive a Variable instance for " + str(variable)
                )
            s += variable + "\n" + variable.doc + "\n\n"
...
class Tabulator:
    ...
    def vector_table(
        ...
        """
        values_list, index_list, labels = box.vectors(
            [values, index], labels="dict", combine=combine
        )
        ...
        for vec, ind_vec in zip(values_list, index_list):
            ...
            interpolated = utils.interp(ind_v, v, index_vals, min_diff=min_diff)
            interp_list.append(interpolated)
        if np.ndim(interp_list) == 3:
            if isinstance(values, variable.Variable):
                lab = values.label
                components = [
                    values._append_unit(c, values.unit) for c in values.components
                ]
            else:
                lab = values + ":"
            interp_list, labels = self._unfold_3D(interp_list, labels, lab, components)
