...
class Box(list):
    ...
    def iwhere(self, dct=None, lst=None, **kwargs):
        ...
        def filt_func(d):
            if filt_dep:
                return all(
                    [v == d[INDEP].get(k, d[DEP].get(k, None)) for k, v in m.items()]
                )
            else:
    ...
    def vectors(self, keys, dct=None, labels="str", combine=True, indep_keys=None):
    ...
    def grouped(self, keys, labels="dict", as_dicts=False):
        ...
        for dct in filtered:
            ...
            if hsh not in out:
                ...
                out[hsh] = {"labels": f_labels, "values": {k: [] for k in keys_copy}}
