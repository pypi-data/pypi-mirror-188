...
__all__ = ('PermissionService', 'GroupService', 'UserService', 'PERMISSIONS_MAP')


PERMISSIONS_MAP = {
    'user': AbstractRPCCompatible.PermissionKeys.GLOBAL_USER_PERMISSION,
    'system': AbstractRPCCompatible.PermissionKeys.GLOBAL_SYSTEM_PERMISSION,
}
...
class UserService(SQLService, Contextable, AbstractRPCCompatible):
    ...
    async def _get_user_and_permissions(self, id, columns='*'):
        user, _permissions = await asyncio.gather(self.get(id, columns=columns), self.get_user_permissions(id))
