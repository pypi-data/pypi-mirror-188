...
def get_indicator_columns_data_for_grain(
    ...
    :param expr_store_for_worker: The experiment store to get the column profile from.\
    :param target_column_name: The target column.
    :param grain_keyvalues_and_path: The path to current grain.
    :return: The dictionary, relating column and boolean value, saying if this is an
             indicator column.
    """
    data_profile = expr_store_for_worker.metadata.timeseries.get_featurized_data_profile_by_grain(
        grain_keyvalues_and_path.key_values)
    indicator_column_data = {}
    for col in data_profile.columns.keys():
        if col == target_column_name:
            continue
        if len(data_profile.columns[col].value_counts) > 2:
            indicator_column_data[col] = False
        else:
            indicator_column_data[col] = all(
                [val.value == 1 or val.value == 0 for val in data_profile.columns[col].value_counts])
