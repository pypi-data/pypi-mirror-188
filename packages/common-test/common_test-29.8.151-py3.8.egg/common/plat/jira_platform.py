...
class JiraPlatForm(object):
    ...
    @classmethod
    def updatecaseByrunId(self, runid, temp, comment):
        if DataProcess.isNotNull(get_system_key(Constant.ISSUE_KEY)) and DataProcess.isNotNull(get_system_key(Constant.TEST_SRTCYCLE_ID)):
            result = 4
            if temp == 'passed':
                result = 1
            ...
            if temp == '自动化执行':
                result = 6
            try:
                if JiraPlatForm._isNotNull(temp):
                    APIDriver.http_request(
                ...
                if JiraPlatForm._isNotNull(comment):
                    APIDriver.http_request(
                ...
                logger.info(f'用例运行ID：{runid}  结果:{result}  结果描述:{comment}  推送异常：{e}')
    ...
    @classmethod
    def updateRender(self,testCaseIssueKey, desc):
        try:
            APIDriver.http_request(
                url=f"{Constant.JIRA_URL}/rest/api/1.0/render",
                method='post',
                parametric_key='json',
                data={
                    "rendererType":"atlassian-wiki-renderer",
                    "unrenderedMarkup": f"{desc}",
                    "forWysiwyg":true,
                    "issueKey":f"{testCaseIssueKey}",
                    "formToken":"undefined"},
                _auth=HTTPBasicAuth(get_system_key(Constant.JIRA_USERNAME), get_system_key(Constant.JIRA_PASSWORD))
            )
        except Exception as e:
...
if __name__ == '__main__':
    os.environ[Constant.JIRA_USERNAME] = 'xx'
    os.environ[Constant.JIRA_PASSWORD] = 'xxx@@'
    os.environ[Constant.ISSUE_KEY] = 'DS21059-2975'
    JiraPlatForm.updateRender('DS21059-2975','3838333333')
