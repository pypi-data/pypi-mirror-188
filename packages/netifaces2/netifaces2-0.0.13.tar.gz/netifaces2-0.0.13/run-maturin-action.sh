...
if [[ ! -d $(rustc --print target-libdir --target s390x-unknown-linux-gnu) ]]; then rustup target add s390x-unknown-linux-gnu; fi
echo "::endgroup::"
maturin build --target s390x-unknown-linux-gnu --manylinux 2014 --release -o dist -i python3.6