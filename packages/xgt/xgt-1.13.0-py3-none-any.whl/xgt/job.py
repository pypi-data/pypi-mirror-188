...
from .common import (XgtError, XgtInternalError, XgtNotImplemented,
                     ...
                     _get_data_python_from_table, _get_data_pandas_from_table,
                     _get_data_arrow, _code_error_map)
...
class Job(object):
  ...
  def __init__(self, conn, job_response = None, python_errors = None):
    ...
    if job_response is not None:
      ...
      self._id = 0
      self._user = ""
      self._data = { 'status' : 'completed', 'total_rows' : 0 }
  ...
  def _parse_job_data(self, job_response):
    ...
    if job_response.HasField('schema'):
      schema = self._conn._translate_schema_from_server(job_response.schema)[0]
  ...
  # Returns a view of the immediate results in arrow table form taking into
  # account the requested offset and length.  If the results are not yet
  # available, None is returned.
  def _load_arrow_table(self, offset, length):
    ...
    if self._is_status_final():
      ...
      # Skip getting the Arrow table if there are no results.
      # The schema was not set for jobs without immediate results.
      if 'schema' not in self._data:
        return None

      if self._arrow_table is None:
        # Cache the entire results in an Arrow table to be able to return them
        # to the user in the future without having to retransmit them from the
        # server.  Any offset or length requested will be handled below.
        ticket = _create_flight_ticket('xgt__Job_History',
                                       offset = 0, length = None,
                                       include_row_labels = False,
                                       job_id = self._id)

        self._arrow_table = _get_data_arrow(self._conn, ticket)

      # Return a view of the arrow table taking into account the requested
      # offset and length.
      total_length = self._arrow_table.num_rows
      if length is None:
        length = total_length
      offset = min(offset, total_length)
      length = min(length, total_length - offset)

      return self._arrow_table.slice(offset = offset, length = length)
  ...
  @property
  def total_ingest_errors(self):
    """
    int: Get the number of errors that were thrown during ingest.
    """
    if not self._is_status_final():
      self._get_job_data()
  ...
  def get_data(self, offset=0, length=None):
    """
    Returns immediate results data starting at a given offset and spanning a
    given length.
    ...
    -------
    list of lists
    If the job object represents a TQL query with no RETURN clause, this returns
    None.
    ...
    return _get_data_python_from_table(self._load_arrow_table(offset, length),
                                       self._data['schema'], self._conn)

  def get_data_arrow(self, offset=0, length=None):
    """
    Returns an Apache Arrow Table containing immediate results data starting at
    a given offset and spanning a given length.
  ...
  def get_ingest_errors(self, offset=0, length=None):
    """
