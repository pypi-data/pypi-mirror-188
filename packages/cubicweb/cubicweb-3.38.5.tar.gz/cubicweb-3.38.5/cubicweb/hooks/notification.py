...
class _RenderAndSendNotificationOp(hook.DataOperationMixIn, hook.Operation):
    ...
    def postcommit_event(self):
        ...
        views_to_render = self.get_data()
        for view, viewargs in views_to_render:
...
class NotificationHook(hook.Hook):
    ...
    def select_view(self, vid, rset, row=0, col=0):
        try:
            return self._cw.vreg["notification-views"].select_or_none(
...
class EntityUpdateHook(NotificationHook):
    ...
    def __call__(self):
        ...
        attrs = [k for k in self.entity.cw_edited if k not in self.skip_attrs]
        if not attrs:
            return
