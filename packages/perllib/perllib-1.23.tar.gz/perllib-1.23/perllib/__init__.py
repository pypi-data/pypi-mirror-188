# perllib module for pythonizer, generated by makelib.py on Mon Jan 30 23:22:47 2023
#
...
INPUT_LAYERS = ''
EVAL_ERROR = ''
_PYTHONIZER_KEYWORDS = {'and','hasattr','False','iter','signal','tempfile','try','sum','format','struct','wantarray','argparse','except','yield','hex','traceback','anext','casefold','repr','builtins','Hash','continue','glob','values','upper','map','functools','issubclass','for','async','isinstance','max','sorted','find','with','oct','set','enumerate','itertools','help','lower','in','as','pop','insert','getattr','break','fileinput','global','list','assert','bytearray','while','delattr','zip','bin','round','callable','subprocess','ascii','memoryview','os','codecs','io','property','divmod','ord','lambda','return','tm_py','frozenset','types','all','dataclasses','bytes','if','filter','globals','eval','True','int','math','ArrayHash','hash','locals','str','nonlocal','Array','fcntl','import','close','sys','type','await','finally','dir','vars','breakpoint','not','reversed','pow','len','min','warnings','any','inspect','copy','exec','next','else','staticmethod','or','input','range','elif','classmethod','rfind','compile','super','complex','rstrip','random','is','open','pass','stat','calendar','object','abs','slice','None','del','id','keys','pdb','update','aiter','collections.abc','atexit','bool','print','re','float','def','raise','get','dict','class','extend','from','perllib','chr','getopt','tuple','setattr'}
CHILD_ERROR = 0
OUTPUT_LAYERS = ''
_DUP_MAP = dict(STDIN=0, STDOUT=1, STDERR=2)
OUTPUT_FIELD_SEPARATOR = ''
_TIE_MAP = {'CLEAR': 'clear','PUSH': 'append','SCALAR': '__len__','UNTIE': '__untie__','EXISTS': '__contains__','FETCH': '__getitem__','DELETE': '__delitem__','STORE': '__setitem__','FETCHSIZE': '__len__','DESTROY': '__del__'}
TRACEBACK = 0
LIST_SEPARATOR = ' '
...
def caller_s(expr=None):
    ...
    result = caller(2 if expr is None else (max(int(expr),0)+2))
    if result is None:
        return result
    return result[0]
