...
def get_dependencies(code_segment_list):
    ...
    print("new list")
    try:
        exec("""import os; print("path exists: ", os.path.exists('./requirements.txt'))""")
        install_statements = []
...
def parse_pip_installation(file_line):
  # Get packages name using regex
  packages = re.findall(r'\w+', file_line)
  # Open requirements.txt
  with open('requirements.txt', 'a+') as req_file:
    # Append each package to the requirements.txt
