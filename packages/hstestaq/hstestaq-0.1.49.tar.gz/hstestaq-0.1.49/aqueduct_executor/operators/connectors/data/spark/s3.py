...
class SparkS3Connector(s3.S3Connector):
    ...
    def _fetch_object_spark(self, key: str, params: extract.S3Params, sparkSessionObj: SparkSession) -> Any:
        ...
        if params.artifact_type == ArtifactType.TABLE:
            ...
            try:
                if params.format == common.S3TableFormat.CSV:
                    return sparkSessionObj.read.option("header","true").csv(data_path)
                elif params.format == common.S3TableFormat.JSON:
                    return sparkSessionObj.read.json(data_path)
                elif params.format == common.S3TableFormat.PARQUET:
                    return sparkSessionObj.read.parquet(data_path)
            except Exception:
                raise Exception(
    ...
    def extract_spark(self, params: extract.S3Params, sparkSessionObj: SparkSession) -> Any:
        ...
        if not isinstance(path, List):
            ...
            if path[-1] == "/":
                ...
                for obj in self.s3.Bucket(self.bucket).objects.filter(Prefix=path):
                    ...
                    if (obj.key)[-1] != "/":
                        files.append(self._fetch_object_spark(obj.key, params, sparkSessionObj))
                ...
                return self._fetch_object_spark(path, params, sparkSessionObj)
        else:
            ...
            for key in path:
                ...
                files.append(self._fetch_object_spark(key, params, sparkSessionObj))
    ...
    def unionAll(self, dfs: Any) -> DataFrame:
