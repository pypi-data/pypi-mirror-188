...
class M0deler:
  ...
  def gen_sandbox(cl,name,economy,cbdc):
    ...
    data = {'economy_params':sandbox.economy.params,
          ...
          'sandbox_name':name}
    print(data)
    r = requests.post(f'http://{host}/api/sandbox/generate',json=data)
  ...
  def list_sandboxes(cls):
    r = requests.get(f'http://{host}/api/sandbox/list')
    print(r.content) 
  @classmethod
  def list_economies(cls):
    r = requests.get(f'http://{host}/api/economy/list')
    print(r.content)
  @classmethod
  def list_cbdcs(cls):
    r = requests.get(f'http://{host}/api/cbdc/list')
    print(r.content)
