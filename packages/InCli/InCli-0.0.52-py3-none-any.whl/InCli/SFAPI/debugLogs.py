...
def print_parsed_line(pc,d):
    ...
    time_stamp_exit = d['timeStamp'][1] if len(d['timeStamp'])>1 else d['timeStamp'][0]
    ...
    if level == context['prevLevel']:  wait_time = d['timeStamp'][0] - context['prevTimes'][level][1]
    if level >  context['prevLevel']:  wait_time = d['timeStamp'][0] - context['prevTimes'][context['prevLevel']][0]
    if level <  context['prevLevel']:  wait_time = d['timeStamp'][0] - context['prevTimes'][level][1]

    if wait_time <0 and '***' in d['output']:   wait_time = d['timeStamp'][0] - context['prevTimes'][level][0]

    context['prevTimes'][level] = [d['timeStamp'][0],time_stamp_exit]

    wait_time = f"{wait_time/1000000:.0f}"

    context['prevLevel'] = level

    soql_queries_entry = d['SOQLQueries'][0]
    soql_queries_exit = d['SOQLQueries'][1] if len(d['SOQLQueries'])>1 else ''
    soql_queries_cmt_exit = d['cmtSOQLQueries'][1] if len(d['cmtSOQLQueries'])>1 else d['cmtSOQLQueries'][0]
    ...
    Qmct_estimate = soql_total_queries_entry - int(soql_queries_entry)
   ...
   # if element_type not in ['EXCEPTION','DEBUG-ERROR']:     output_string = output_string[:150]
    ...
    if context['firstLineOut'] == True:
        ...
        print(f"{utils.CFAINT}{'time entry':^12}{s}{utils.CFAINT}{'time exit':^12}{s}{'ts':^8}{s}{'cpuD':^6}{s}{'CPU':^6}{s}{'Qt':^3}{s}{'Q':^3}{s}{'Qcm':^3}{s}{'Qe':^3}{s}{'type':^21}{s}{'line':^4}{s}{valname:50}")
        context['firstLineOut'] = False

    output_string = utils.CFAINT +f"{wait_time:>5}{identation}{element_time:>5}"+utils.CDARK_GRAY+f"{soql_total_queries_delta:>4}" +utils.CEND + Cinit +f"    {output_string}"

    print(f"{time_stamp_entry:12}{s}{time_stamp_exit:12}{s}{time_ms:>8}{s}{cpu_time_elapsed:6}{s}{cpu_time_entry:>6}{s}{soql_total_queries_entry:>3}{s}{soql_queries_entry:>3}{s}{soql_queries_cmt_exit:3}{s}{(Qmct_estimate):>3}{s}{element_type_color}{element_type:21}{utils.CEND}{s}{apex_line_number:>4}{s}{output_string:50}"+utils.CEND)
