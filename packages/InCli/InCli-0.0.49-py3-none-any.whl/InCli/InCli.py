...
def option_cc(args):
    ...
    if '-h' in args:
        help = f"""
        {utils.CYELLOW}-cc{utils.CEND}       {utils.CGREEN}InCli -u "Org alias" -cc {utils.CEND}
...
def option_logs(args):
    help = f"""
        ...
        {utils.CYELLOW}-logs{utils.CEND} --> lists log records in the org. 
            ...
            {utils.CYELLOW}-loguser field:value{utils.CEND}, filter the logs for the specified user. The user can be specified by any field in the User Object. 
                ...
                {utils.CLIGHT_PURPLE}Defaultable loguser{utils.CEND}: Can be set to default. {utils.CGREEN}-default:set loguser "Alias:John.Doe{utils.CEND}"
    ...
    if tail:
        ...
        connectionInit(args)
        return debugLogs.do_parse_logId(logId,parseContext)
    else:
        connectionInit(args)
...
def _main(argsIn):
    ...
    if '-h' in argsIn:
        ...
        {utils.CYELLOW}-u{utils.CEND} --> username or org alias to be used to log into the org. 
            {utils.CLIGHT_PURPLE}Defaultable u{utils.CEND}: Can be set to a default value --> {utils.CGREEN}InCli -default:set u "xx@adas.com {utils.CEND} """
        print(help)
