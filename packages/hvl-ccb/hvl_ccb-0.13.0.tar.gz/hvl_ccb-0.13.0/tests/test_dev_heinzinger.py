...
from hvl_ccb.dev.heinzinger.base import (
    HeinzingerConfig,
    HeinzingerDI,
    HeinzingerPNC,
...
class ConcreteHeinzingerDI(HeinzingerDI):
    def start(self):
        super().start()
...
def start_pnc_device(com_config, dev_config):
    def _start_pnc_device(serial_number):
        ...
        def started_pnc():
            with HeinzingerPNC(serial_port, dev_config) as pnc:
                while serial_port.get_written() is not None:
                    pass
...
def test_di_com_error(com_config, dev_config):
    ...
    with pytest.raises(comm.SerialCommunicationIOError):
        di.set_number_of_recordings(HeinzingerConfig.RecordingsEnum.EIGHT)
    with pytest.raises(comm.SerialCommunicationIOError):
        di.measure_voltage()
    with pytest.raises(comm.SerialCommunicationIOError):
        di.set_voltage(1)
...
def test_pnc_instantiation(com_config, dev_config):
    pnc = HeinzingerPNC(com_config)
    assert pnc is not None

    wrong_config = dict(dev_config)
    wrong_config["default_number_of_recordings"] = 0
    with pytest.raises(ValueError):
        HeinzingerPNC(com_config, wrong_config)

    assert pnc.unit_voltage == HeinzingerPNC.UnitVoltage.UNKNOWN
    assert pnc.unit_current == HeinzingerPNC.UnitCurrent.UNKNOWN
...
devices_data = [
    ("PNChp 60000-1neg 354211082", 60000, 1, HeinzingerPNC.UnitVoltage.V, "mA"),
    ("314807440/PNChp 60000-1neg.", 60000, 1, "V", HeinzingerPNC.UnitCurrent.mA),
    ("PNChp 1500-40 ump. 375214277", 1500, 40, "V", "mA"),
    ("PNC 100000-6pos 375214277", 100, 6, HeinzingerPNC.UnitVoltage.kV, "mA"),
    ("PNC 600-3000pos 375214277", 600, 3, "V", HeinzingerPNC.UnitCurrent.A),
]
...
def test_pnc_not_recognized(start_pnc_device):
    with pytest.raises(HeinzingerPNCDeviceNotRecognizedError):
...
def test_pnc_set_negative(start_pnc_device):
    started_pnc = start_pnc_device("PNC 600-3000pos 375214277")
    com, pnc = next(started_pnc)
...
def test_pnc_set_voltage(start_pnc_device, device_data):
    ...
    com.put_text(str(device_data[1]))
    assert pnc.get_voltage() == device_data[1]

    # test if an error is raised when trying to set a too high voltage
    with pytest.raises(HeinzingerPNCMaxVoltageExceededError):
...
def test_pnc_set_current(start_pnc_device, device_data):
    ...
    assert com.get_written().strip() == device_data[2]
    com.put_text(str(device_data[1]))
    assert pnc.get_current() == device_data[1]

    # test if an error is raised when trying to set a too high current
    with pytest.raises(HeinzingerPNCMaxCurrentExceededError):
