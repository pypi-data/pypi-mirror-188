...
def __checker(inputText):
    scorelist = []
    inputTextToken = inputText.split(" ")
    inputTextToken = [i for i in inputTextToken if i not in __bangla_stopwords]
    ...
    for bookID in range(1, 201):
        ...
        for page in range(1, totalpages+1):
            ...
            # Removing stopwords
            dbtokens = [i for i in dbtokens if i not in __bangla_stopwords]

            # gives the similarity score
            score = __levenshtein(dbtokens, inputTextToken)
            if(score >= 10):
...
def check(userinput):
    return __findTitle(__checker(userinput))