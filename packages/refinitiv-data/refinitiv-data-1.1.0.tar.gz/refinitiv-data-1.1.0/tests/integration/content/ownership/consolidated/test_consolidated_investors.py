import allure
import pytest

from refinitiv.data.content.ownership.consolidated import investors
from tests.integration.constants_list import HttpStatusCode, HttpReason
from tests.integration.content.ownership.helpers import (
    check_response_dataframe_instrument_contains_expected_universes,
    get_expected_column,
)
from tests.integration.helpers import (
    get_async_response_from_definitions,
    check_response_status,
    check_non_empty_response_data,
    check_response_dataframe_contains_columns_names,
    check_dataframe_column_date_for_datetime_type,
    check_extended_params_were_sent_in_request,
)

EXPECTED_COLUMN_NAMES = [
    "instrument",
    "investorid",
    "investorpermid",
    "TR.InvestorFullName",
    "TR.InvestorType",
    "TR.PctOfSharesOutHeld",
    "TR.SharesHeld",
    "TR.SharesHeldChange",
    "TR.HoldingsDate",
    "TR.OwnTrnverRating",
]

EXPECTED_COLUMN_TITLES = [
    "Instrument",
    "Investor Id",
    "Investor Perm Id",
    "Investor Name",
    "Investor Type",
    "Percentage of SharesOutstanding",
    "SharesHeld",
    "SharesHeld Change",
    "Holdings Date",
    "Turnover Rating",
]


@allure.suite("Content object - Ownership Consolidated Investors")
@allure.feature("Content object - Ownership Consolidated Investors")
@allure.severity(allure.severity_level.CRITICAL)
class TestConsolidatedInvestors:
    @allure.title(
        "Create Consolidated Investors definition object with valid params and get data"
    )
    @pytest.mark.caseid("37556942")
    @pytest.mark.parametrize(
        "universe,limit,use_field_names_in_headers,extended_params,expected_universe",
        [
            ("VOD.L", 2, True, {"universe": "ONEX.CCP"}, "ONEX.CCP"),
            ("US02079K1079", 1, False, None, "US02079K1079"),
            (
                ["IBM.N", "ONEX.CCP", "AAPL.O"],
                None,
                True,
                {"universe": "ONEX.CCP"},
                ["ONEX.CCP"],
            ),
            (
                ["ONEX.CCP", "invalid universe"],
                5,
                False,
                None,
                ["ONEX.CCP", "invalid universe"],
            ),
        ],
    )
    @pytest.mark.smoke
    def test_create_consolidated_investors_object_with_valid_fields_and_get_data(
        self,
        open_platform_session,
        universe,
        limit,
        use_field_names_in_headers,
        extended_params,
        expected_universe,
    ):
        expected_column = get_expected_column(
            use_field_names_in_headers, EXPECTED_COLUMN_TITLES, EXPECTED_COLUMN_NAMES
        )
        response = investors.Definition(
            universe, limit, use_field_names_in_headers, extended_params
        ).get_data()
        check_response_status(response, HttpStatusCode.TWO_HUNDRED, HttpReason.OK)
        check_non_empty_response_data(response)
        check_response_dataframe_contains_columns_names(response, expected_column)
        check_response_dataframe_instrument_contains_expected_universes(
            response, expected_universe
        )
        check_dataframe_column_date_for_datetime_type(response)
        check_extended_params_were_sent_in_request(response, extended_params)

    @allure.title(
        "Create Consolidated Investors definition object with mix valid and invalid params and get data asynchronously"
    )
    @pytest.mark.caseid("37556943")
    @pytest.mark.parametrize(
        "universe,invalid_universe,limit",
        [
            (["ONEX.CCP", "invalid universe"], "invalid universe", 50),
        ],
    )
    @pytest.mark.smoke
    async def test_create_consolidated_investors_object_with_mix_valid_and_invalid_params_and_get_data_async(
        self, open_platform_session_async, universe, invalid_universe, limit
    ):
        valid_response, invalid_response = await get_async_response_from_definitions(
            investors.Definition(
                universe, use_field_names_in_headers=True, limit=limit
            ),
            investors.Definition(
                invalid_universe, use_field_names_in_headers=True, limit=limit
            ),
        )
        check_response_status(
            response=valid_response,
            expected_status_code=HttpStatusCode.TWO_HUNDRED,
            expected_http_reason=HttpReason.OK,
            expected_error_code=None,
            expected_error_message="Failed to resolve identifiers ['invalid universe']",
        )
        check_non_empty_response_data(valid_response)
        check_response_status(
            response=invalid_response,
            expected_status_code=HttpStatusCode.TWO_HUNDRED,
            expected_http_reason=HttpReason.OK,
            expected_error_code=HttpStatusCode.FOUR_HUNDRED_TWELVE,
            expected_error_message="Unable to resolve all requested identifiers. Requested items: ['invalid universe']",
        )
        check_response_dataframe_contains_columns_names(
            valid_response, EXPECTED_COLUMN_NAMES
        )
        check_response_dataframe_instrument_contains_expected_universes(
            valid_response, universe
        )
