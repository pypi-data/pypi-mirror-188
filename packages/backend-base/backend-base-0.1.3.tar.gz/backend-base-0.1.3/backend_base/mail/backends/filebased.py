...
from backend_base.mail.backends.console import (
    ...
    def __init__(self, *args, file_path=None, **kwargs):
        ...
        try:
            ...
            raise ImproperlyConfigured(
                'Path for saving email messages exists,'
                ' but is not a directory: %s' % self.file_path
            )
        except OSError as err:
            raise ImproperlyConfigured(
                'Could not create directory for saving'
                ' email messages: %s (%s)' % (self.file_path, err)
            )
