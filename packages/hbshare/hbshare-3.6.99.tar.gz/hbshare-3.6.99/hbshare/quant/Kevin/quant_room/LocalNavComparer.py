...
# from hbshare.quant.Kevin.quant_room.MyUtil.data_loader import get_fund_nav_from_sql
# local_nav = get_fund_nav_from_sql('20211231', '20221111', {"艾悉": "SX9349"})

nv_series = pd.read_excel('D:\\玄信周度复权净值_20221118.xlsx', sheet_name=0, header=1)
nv_series['净值日期'] = nv_series['净值日期'].apply(lambda x: datetime.strftime(x, "%Y%m%d"))
nv_series.rename(columns={"复权单位净值": "玄信500指增"}, inplace=True)
local_nav = nv_series.set_index('净值日期')[['玄信500指增']]
...
if __name__ == '__main__':
    LocalNavComparer("20220128", "20221118").get_construct_result()