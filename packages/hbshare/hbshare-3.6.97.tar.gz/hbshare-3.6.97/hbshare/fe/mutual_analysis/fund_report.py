...
class QuantitativeReport:
    ...
    def get_fund_size(self):
        ...
        fig, ax = plt.subplots(figsize=(12, 6))
        sns.barplot(ax=ax, x='报告日期', y='规模暴露（%）', data=fund_size_ts, hue='规模类型', hue_order=size_list, palette=['#C94649', '#8588B7', '#7D7D7E'])
    ...
    def get_fund_theme(self):
        ...
        plt.legend(handles=h[::-1], labels=l[::-1], loc=8, bbox_to_anchor=(0.5, -0.2), ncol=5)
        plt.gca().yaxis.set_major_formatter(FuncFormatter(to_percent))
        plt.xticks(rotation=45)
        plt.xlabel('')
        plt.ylabel('主题占比')
    ...
    def get_quantitative_report(self):
        ...
        document.add_picture('{0}{1}_brison_attribution.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        document.add_paragraph(text=fund_summary)
        fund_summary = '10. 绩效归因：从Brison绩效归因看，该产品最新一期{0}；从Barra风格归因看，该产品最新一期{1}'.format(fund_brison_attribution_text.split('从最新一期看，')[1].split('。')[0], fund_barra_text.split('如下，')[1])
        document.add_paragraph(text=fund_summary)
