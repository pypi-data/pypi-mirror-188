...
elseif strcmp(cmd,'init') & isempty(fig)
  ...
  % NEW DEMO FIGURE
  fig = nndemof2(me,'DESIGN','Adam Backpropagation','','Chapter 12');
  set(fig, ...
...
elseif strcmp(cmd,'lr')
  ...
  lr = get(lr_bar,'value');
  set(lr_text,'string',sprintf('%5.4f',round(lr*1000)*0.001))
...
elseif strcmp(cmd,'inc')
  ...
  set(inc_text,'string',sprintf('%5.4f',round(inc*1000)*0.001))
...
elseif strcmp(cmd,'dec')
  ...
  set(dec_text,'string',sprintf('%4.2f',round(dec*1000)*0.001))
...
elseif strcmp(cmd,'down') & ~isempty(fig) & (nargin == 1)
  ...
  if (x > xlim(1) & x < xlim(2) & y > ylim(1) & y < ylim(2))
    ...
    ep = 600;
    lr = get(lr_bar,'value'); % alpha
    inc = get(inc_bar,'value'); % beta1
    dec = get(dec_bar,'value'); % beta2
    ...
    for i=2:(ep+1)
      ...
      gW2 = D2*A1';
      gb2 = D2*ones(Q,1);
      mW1 = inc*mW1 + (1-inc)*gW1;
      mb1 = inc*mb1 + (1-inc)*gb1;
      mW2 = inc*mW2 + (1-inc)*gW2;
      mb2 = inc*mb2 + (1-inc)*gb2;
      vW1 = dec*vW1 + (1-dec)*gW1.*gW1;
      vb1 = dec*vb1 + (1-dec)*gb1.*gb1;
      vW2 = dec*vW2 + (1-dec)*gW2.*gW2;
      vb2 = dec*vb2 + (1-dec)*gb2.*gW2;
      inck = inc*inck;
      deck = dec*deck;
      lrk = lr*sqrt(1-deck)/(1-inck);
      
      dW1 = lrk*mW1./(sqrt(vW1) + mc);
      db1 = lrk*mb1./(sqrt(vb1) + mc);
      dW2 = lrk*mW2./(sqrt(vW2) + mc);
      db2 = lrk*mb2./(sqrt(vb2) + mc);
...
%       new_W2 = W2; new_b2 = b2;
      if (option == 1)
        x = W1(1,1) + dW1(1,1); W1(1,1) = x;
        y = W2(1,1) + dW2(1,1); W2(1,1) = y;
      elseif(option == 2)
        x = W1(1,1) + dW1(1,1); W1(1,1) = x;
        y = b1(1)   + db1(1);   b1(1) = y;
      else
