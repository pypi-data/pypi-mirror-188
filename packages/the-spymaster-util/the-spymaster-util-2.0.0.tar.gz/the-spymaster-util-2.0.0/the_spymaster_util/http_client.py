...
class BaseHttpClient:
    ...
    def _http_call(
        self, endpoint: str, method: Callable, parse: bool = True, errors: Optional[ErrorTypes] = None, **kwargs
    ) -> Union[dict, Response]:
    ...
    def _get(self, endpoint: str, data: dict, errors: Optional[ErrorTypes] = None, **kwargs) -> dict:
        return self._http_call(
            endpoint=endpoint, method=self.session.get, params=data, errors=errors, **kwargs  # type: ignore
        )

    def _post(self, endpoint: str, data: dict, errors: Optional[ErrorTypes] = None, **kwargs) -> dict:
        return self._http_call(
            endpoint=endpoint, method=self.session.post, json=data, errors=errors, **kwargs  # type: ignore
        )
