...
def _raise_custom_error(response: Response, http_error: HTTPError, error_types: Optional[ErrorTypes] = None):
    ...
    if not error_class:
        raise http_error
    parsed_error = _try_parse_custom_error(data=data, error_code=error_code, error_class=error_class)
    if not parsed_error:
        raise http_error
    raise parsed_error from http_error


def _try_parse_custom_error(data: dict, error_code: str, error_class: Type[E]) -> Optional[E]:
    try:
        return error_class(**data)  # type: ignore
    except Exception as error_init_error:  # noqa
        log.debug(f"Error code {error_code} exists, but failed to init: {error_init_error}")
    details = data.get("details", None)
    if not details:
        log.debug("No details in error data, can't parse using init args")
        return None
    init_args = list(inspect.signature(error_class.__init__).parameters.keys())
    found_args = {k: v for k, v in details.items() if k in init_args}
    log.debug(
        "Details init args (in extra)",
        extra={"init_args": init_args, "found_args": found_args},
    )
    try:
        return error_class(**found_args)  # type: ignore
    except Exception as error_init_error:  # noqa
        log.debug(f"Failed to construct {error_code} error using detail args: {error_init_error}")
    return None
...
def get_error_code(exception: Union[E, Type[E]]) -> str:
