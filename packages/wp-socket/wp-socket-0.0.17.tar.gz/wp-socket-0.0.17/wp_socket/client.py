...
_READ_DEFAULT_BUFFER_SIZE = 512  # 512 bytes
_QUEUE_DEFAULT_WAIT_TIMEOUT = 1.0
_RECEIVE_DEFAULT_INTERVAL = 0.3
_SEND_DEFAULT_INTERVAL = 0.4
_SEND_DEFAULT_RETRY_COUNT = 5
...
class WpSocketClient:
    ...
    def __init__(self, host: str, port: int, async_packets_handler, **kwds):
        self.host = host
        self.port = port
        self._async_receive_handler = async_packets_handler
        self._config = WpSocketConfig(
            kwds.pop('connection_timout', _CONNECTION_DEFAULT_TIMEOUT),
            kwds.pop('read_timeout', _READ_DEFAULT_TIMEOUT),
            kwds.pop('read_buffer_size', _READ_DEFAULT_BUFFER_SIZE),
            kwds.pop('queue_wait_timeout', _QUEUE_DEFAULT_WAIT_TIMEOUT),
            kwds.pop('receive_interval', _RECEIVE_DEFAULT_INTERVAL),
            kwds.pop('send_interval', _SEND_DEFAULT_INTERVAL),
            kwds.pop('send_retry_count', _SEND_DEFAULT_RETRY_COUNT)
        )
        ...
        while self.connected:
            try:
                ...
                while True:
                    ...
                    if send_interval > self._config.receive_interval \
                            and receive_interval > self._config.send_interval:
                    ...
                    if send_data.inc_try() > self._config.send_retry_count:
                        send_data.set(False)
                        _LOGGER.warning('Send fail: %s', send_data.packet.hex())
                        break

                    if send_interval > receive_interval:
                        delay = self._config.receive_interval - receive_interval
                        _LOGGER.info('Send, wait receive interval : %f', delay)
                        await asyncio.sleep(delay)
                    else:
                        delay = self._config.send_interval - send_interval
