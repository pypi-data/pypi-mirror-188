...
_READ_WRITE_INTERVAL = 0.3
_WRITE_RETRY_COUNT = 5
        ...
        while self.connected:
            try:
                send_data: _WpSendData = await asyncio.wait_for(self._send_packet_queue.get(),
                                                                timeout=_QUEUE_WAIT_TIMEOUT)
                while True:
                    interval = time.time() - self._last_io_time
                    if interval > _READ_WRITE_INTERVAL:
                    ...
                    if send_data.inc_try() > _WRITE_RETRY_COUNT:
                        send_data.set(False)
                        _LOGGER.warning('Send fail: %s', send_data.packet.hex())
                        break

                    delay = _READ_WRITE_INTERVAL - interval
                    _LOGGER.info('Send wait: %f', delay)
