...
class API:
    ...
    def table_add_permissions(self, projectId, tableId, groupId, create, read, update, delete):
        """Add permissions to a table"""
        path = "permissions"
        body = {
            "project": {
                "id": projectId
            ...
            "create": strtobool(create),
            "read": strtobool(read),
            "update": strtobool(update),
            "delete": strtobool(delete)
        }
        return self._make_api_request(path, "POST", body)

    def table_check_permission(self, projectId, tableId):
        """Get the permissions on the table of the groups you have access to"""
        path = 'permissions'
        return self._make_api_request(path, "GET", {
    ...
    def table_remove_permissions(self, permissionId):
        path = 'permission/{}'.format(permissionId)
        return self._make_api_request(path, "DELETE", {})
    ...
    def records_at_moment(self, projectId:str , tableId:str , timestamp:int, session:str = "", offset:int=0, limit:int=0, chunk_size:int = 10000):
        """Search in the records"""
    ...
    def tasks_read(self, license: str = "", project_id: str = "", status: str = "", user: str = "", today:bool = False) -> list:
        """ Get tasks"""
        depreciationMessage("param", "status", "01-02-2023", "taken")

        license = selectLicense(license, self._licenses)
        path = 'tasks?filter[license]={}&filter[today]={}'.format(license["id"], int(today))
        filters = []
        if project_id != "":
            filters.append({"status": "project", "operator": "=", "values": [project_id]})
        if status != "":
            filters.append({"status": "status", "operator": "=", "values": [status]})
        if user != "":
            filters.append({"status": "assigned_to", "operator": "=", "values": [user]})
        return self._make_api_request(path, 'POST', {
            "advanced_filters":filters
        })

    def task_create(self, project_id: str, name: str, description: str, status: str, due_date: str, assigned_user: str, start_date: str = "", appendix: object = {}, license:str = "", end_date:str = "") -> dict:
    ...
    def update_task(self, task_id:str, title:str = "", description:str = "", priority:str = "", planned_start:str = "", planned_end:str = "", assigned_to: str = "", due_date: str = "", sbs_code:str = "")-> dict:
        """ update a task info"""
        path = "tasks/{}".format(task_id)
        body = {}
        if title != "":
    ...
    def task_upload_appendix(self, task_id:str, appendix: object):
...
def selectLicense(license,licenses):
    if license == "":
        ...
        if len(licenses) > 1:
            print(bcolors.WARNING + "Warning" +bcolors.ENDC + ": No license provided. A random license ("+bcolors.OKGREEN+"{}".format(selectedLicenseName)+bcolors.ENDC+") is selected to handle the tasks. Please specify which license you are using, see {}".format(URL))
        else:
            print(bcolors.OKBLUE + "Info" +bcolors.ENDC + ": You didn't provided a license. Since you have only one license ("+bcolors.OKGREEN+"{}".format(selectedLicenseName)+bcolors.ENDC+"), this is automatically selected. Advised to add it, see {} ".format(URL))
        return licenses[0]
    else:
        return license
...
def depreciationMessage(type, name, date, doc):
    ...
    if type == "param":
        print(bcolors.WARNING + "Warning" +bcolors.ENDC + ": The parameter: \"{}\" will be been depreciated from {}. Please update according to {}".format(name, date, URL))
    if type == "def":
        print(bcolors.WARNING + "Warning" +bcolors.ENDC + ": The function {} will be been depreciated from {}. Please update according to {}".format(name, date, URL))
