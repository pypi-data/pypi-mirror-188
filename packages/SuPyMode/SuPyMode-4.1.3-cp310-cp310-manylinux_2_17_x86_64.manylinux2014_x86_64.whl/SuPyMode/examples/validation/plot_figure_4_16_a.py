...
fiber_0 = SMF28(wavelength=wavelength, position=clad.fiber[0].core)
fiber_1 = SMF28(wavelength=wavelength, position=clad.fiber[1].core)
...
solver.superset.plot(type='field', itr_list=[1.0, 0.05]).show()
...
for data_dir, color in data_directory:
    ...
    artist = Line(x=data[0], y=data[1], line_style="--", color=color, label='experiment')
    ax.add_artist(artist)
