...
class CppSolver : public BaseLaplacian
{
  ...
  CppSolver(ndarray &mesh,
            ...
            bool Debug)
               : BaseLaplacian(mesh)
                {
                 this->finit_difference_matrix= finit_difference_matrix;
                 this->Debug             = Debug;
                 ...
                 this->kInit             = this->k;
                 ScalarType *adress      = (ScalarType*) py_mesh_gradient.request().ptr;
   ...
   SuperMode get_mode(size_t mode_number){ return sorted_supermodes[mode_number]; }
  ...
  void set_boundaries(std::string left_boundary, std::string right_boundary, std::string top_boundary, std::string bottom_boundary)
  {
   ...
   void prepare_supermodes()
   {
     for (SuperMode& mode : SuperModes)
         mode.Init(ITRLength, n_x, n_y, left_boundary, right_boundary, top_boundary, bottom_boundary, n_sorted_mode);
   }
   ...
   void loop_over_itr(std::vector<double> ITRList, size_t order = 1, ScalarType alpha=0){
     ...
     for (size_t slice=0; slice<ITRLength; ++slice)
     {
       ...
       alpha = extrapolate_next(order, all_first_eigen_values, ITRList, next);
     }
   ...
   tuple<ndarray, ndarray> get_slice(size_t Slice)
   {
     ...
     ndarray FieldsPython = eigen_to_ndarray_( OutputFields, { n_sorted_mode, n_x, n_y } ) ;
