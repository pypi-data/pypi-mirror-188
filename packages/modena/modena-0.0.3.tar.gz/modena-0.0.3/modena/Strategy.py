...
class InitialisationStrategy(StrategyBaseClass):
    ...
    def workflow(self, model):
        ...
        if len(p):
            ...
            for i, sm in enumerate(model.substituteModels):
                if i == 0:
                    wf = sm.initialisationStrategy().workflow(sm)
                else:
                    wf.append_wf(
...
class LeaveOneOut(ParameterFittingStrategy):
    ...
    def newPointsFWAction(self, model, **kwargs):
        ...
        * if valid, return empty workflow, else perform DoE.
        ...
        #loo = LeaveOneOut()
        points = [ [ 0.0 for j in range(model.nSamples) ] for i in \
                                 range(model.surrogateFunction.inputs_size())]
...
class Test(ParameterFittingStrategy):
    ...
    def fit(self, model, testIndices):
        ...
        nlfb = nlmrt.nlfb(start=R_par,resfn=R_res,jacfn=rinterface.NULL,trace=False,lower=FloatVector(min_parameters),upper=FloatVector(max_parameters),maskidx=rinterface.NULL)
...
class Initialisation(FireTaskBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'surrogateModel' in kwargs:
            if isinstance(kwargs['surrogateModel'], modena.SurrogateModel.SurrogateModel):
    ...
    def run_task(self, fw_spec):
        ...
        try:
            ...
            model = modena.SurrogateModel.SurrogateModel.load(self['surrogateModelId'])
            return FWAction(
...
class ParameterFitting(FireTaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        try:
            model = modena.SurrogateModel.SurrogateModel.load(self['surrogateModelId'])
            # Seems like this is not needed anymore after the database is updated regularly
            # model.parameters = [i for i in model.parameters]
            print(
                term.cyan
    ...
    def run_task(self, fw_spec):
        ...
        try:
            ...
            print(
                term.yellow
              + 'point = {%s}' % ', '.join(
                    '%s: %g' % (k, v) for (k, v) in p.items()
                )
              + term.normal
            )

            model = modena.SurrogateModel.SurrogateModel.load(self['modelId'])
            oldP = copy.copy(p)
    ...
    def handleReturnCode(self, returnCode):
        ...
        if returnCode == 200:
            try:
                ...
                model = modena.SurrogateModel.SurrogateModel.loadFailing()
            except:
            ...
            try:
                model = modena.SurrogateModel.SurrogateModel.loadFromModule()
            except:
                raise TerminateWorkflow(
            ...
            try:
                model = modena.SurrogateModel.SurrogateModel.loadParametersNotValid()
            except:
                raise TerminateWorkflow(
