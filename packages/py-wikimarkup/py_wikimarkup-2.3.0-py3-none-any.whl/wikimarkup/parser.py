...
from .entities import html_entities as _htmlEntities
...
_h1Pat = re.compile(r'^=(.+)=\s*$', re.MULTILINE)
_h2Pat = re.compile(r'^==(.+)==\s*$', re.MULTILINE)
_h3Pat = re.compile(r'^===(.+)===\s*$', re.MULTILINE)
_h4Pat = re.compile(r'^====(.+)====\s*$', re.MULTILINE)
_h5Pat = re.compile(r'^=====(.+)=====\s*$', re.MULTILINE)
_h6Pat = re.compile(r'^======(.+)======\s*$', re.MULTILINE)
...
class Parser(BaseParser):
    ...
    def bleach(self, text, nofollow, attributes, styles, strip_comments):
        ...
        return bleach.clean(text, tags=self.tags, attributes=attributes,
                            css_sanitizer=css_sanitizer, strip_comments=strip_comments)
