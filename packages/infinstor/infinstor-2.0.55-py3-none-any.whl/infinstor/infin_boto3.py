...
class InfinBotoClient():
    ...
    def retrieve_file_with_retry(self, url, fname):
        ...
        while attempts_left > 0:
            attempts_left -= 1
            try:
                conn = urlopen(url, timeout=60)
            except HTTPError as http_err:
                if http_err.response.status_code == 404: # not found
                    print(f"urlopen({url}) returned error 404 {http_err}. Throwing botocore exception instead")
                    raise ClientError({'Error': {'Code': '404', 'Message': 'Not Found'}}, 'download_file')
            except URLError as e:
                print(f"urlopen({url}) returned error {e}")
                if attempts_left > 0:
            ...
            try:
                ...
                print(f'Exception {ex} in download, retry {attempts_left} more times')
                if attempts_left > 0:
    ...
    def urlopen_with_retry(self, url):
        ...
        while attempts_left > 0:
            ...
            try:
                ...
                print(f'urlopen {url} successful')
                return response
            except Exception as ex:
                print(f'Exception {ex} in urlopen of {url}, retry {attempts_left} more times')
                if attempts_left > 0:
                    ##wait before retrying
                    time.sleep(10 * 2**(max_attempts-attempts_left))
                else:
    ...
    def download_file(self, *args, **kwargs):
        ...
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice' or 'time_spec' in self.timespec_info:
            ...
            if request_bucket in self.timespec_info['infinsnap_buckets']:
                ...
                versionId, presignedUrl = self.get_version_id(request_bucket, request_file, 'download_file')
                if (versionId != None):
    ...
    def get_object_presigned(self, request_bucket, request_file, run_id):
        ...
        try:
            ...
            print('Exception {ex} in request ' + str(ex) + ", for url: " + ps_url)
            raise ex
    ...
    def head_object_presigned(self, request_bucket, request_file, run_id):
    ...
    def get_object(self, **kwargs):
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice' or 'time_spec' in self.timespec_info:
            ...
            if request_bucket in self.timespec_info['infinsnap_buckets']:
                ...
                versionId, presignedUrl = self.get_version_id(request_bucket, request_file, 'get_object')
                if (versionId != None):
                    if self.timespec_info['use_presigned_url_for_infinsnap']:
    ...
    def head_object(self, **kwargs):
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice' or 'time_spec' in self.timespec_info:
            ...
            request_file = kwargs['Key']
            if request_bucket in self.timespec_info['infinsnap_buckets']:
                self.print_greeting("head_object")
                versionId, presignedUrl = self.get_version_id(request_bucket, request_file, 'head_object')
                if (versionId != None):
                    if self.timespec_info['use_presigned_url_for_infinsnap']:
                        print("Using presigned url for head_object", flush=True)
                        res = self.urlopen_with_retry(presignedUrl)
                        clen = res.getheader('Content-Length')
                        rv = {'Body': StreamingBody(res, clen),
                              'ContentLength': int(clen),
                        ...
                        print("Using versionid for head_object", flush=True)
                        kwargs['VersionId'] = versionId
                        return self.defaultClient.head_object(**kwargs)
                else:
                    raise ValueError('head_object: versionId not present. Is InfinSnap enabled?')
            elif request_bucket in self.timespec_info['concurrent_user_buckets']:
                return self.head_object_presigned(request_bucket, request_file, None)
            else:
                run_id = parse_run_id(request_file)
                if run_id:
                    return self.head_object_presigned(request_bucket, request_file, run_id)
                else:
                    return self.defaultClient.head_object(**kwargs)
        else:
            return self.defaultClient.head_object(**kwargs)
