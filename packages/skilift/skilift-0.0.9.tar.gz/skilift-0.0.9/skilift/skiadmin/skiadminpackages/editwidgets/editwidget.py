...
from .... import utils, fromjson, editsection, editpage, editwidget

from ....utils import versions

from .. import adminutils
from skipole import FailPage, ValidateError, ServerError, GoTo, SectionData
...
def retrieve_editfield(skicall):
    ...
    if field_arg in fields_single:
        if field_datalist.cssclass or field_datalist.cssstyle:
            default_value = fromjson.get_widget_default_field_value(project, widgetdescription.modulename, widgetdescription.classname, field_arg)
            if default_value:
...
def _container_domcontents(project, pagenumber, section_name, location_string, container):
    ...
    if 'parts' not in partdict:
        ...
        rows, containers = _domtree(partdict, part_loc, domcontents, part_string_list)
...
def edit_container_dom(skicall):
    ...
    part_tuple = utils.part_info(project, pagenumber, section_name, [widget_name, container, location_integers])
...
def cut_container_dom(skicall):
    ...
    part_tuple = utils.part_info(project, pagenumber, section_name, location)
    if part_tuple is None:
        raise FailPage("Item to remove has not been recognised")
    ...
    # remove the item using functions from utils.editsection and utils.editpage
...
def move_up_in_container_dom(skicall):
    ...
    try:
        ...
        if part_tuple.section_name:
            # move the part in a section, using utils.editsection.move_location(project, section_name, schange, from_location, to_location)
            ...
            # move the part in a page, using utils.editpage.move_location(project, pagenumber, pchange, from_location, to_location)
...
def move_up_right_in_container_dom(skicall):
    ...
    try:
        ...
        new_parent_tuple = utils.part_info(part_tuple.project, part_tuple.pagenumber, part_tuple.section_name, new_parent_location)
        ...
        items_in_new_parent = len(utils.part_contents(part_tuple.project, part_tuple.pagenumber, part_tuple.section_name, new_parent_location))
...
def move_down_in_container_dom(skicall):
    ...
    try:
        ...
        if len(location_integers) == 1:
            ...
            items_in_parent = len(utils.part_contents(part_tuple.project, part_tuple.pagenumber, part_tuple.section_name, parent_location))
...
def move_down_right_in_container_dom(skicall):
    ...
    try:
        ...
        if len(location_integers) == 1:
            ...
            parent_location = (widget_name, container, parent_integers)
        p_contents = utils.part_contents(part_tuple.project, part_tuple.pagenumber, part_tuple.section_name, parent_location)
...
def move_in_container_dom(skicall):
    ...
    if len(location_list) == 2:
        ...
        target_part_tuple = utils.part_info(editedprojname, pagenumber, section_name, target_location)
