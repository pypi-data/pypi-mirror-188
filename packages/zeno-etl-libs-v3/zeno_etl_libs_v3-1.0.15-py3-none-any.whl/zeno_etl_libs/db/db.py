...
class MSSql:
    ...
    def __init__(self, connect_via_tunnel=True, db=None, is_ob=False):
        configobj = Config.get_instance()
        secrets = configobj.get_secrets()
        self.db_secrets = secrets
        if is_ob:
            self.user = self.db_secrets['OB_MSSQL_USER']
            self.password = self.db_secrets['OB_MSSQL_PASSWORD']
            self.host = self.db_secrets['OB_MSSQL_HOST']
            self.port = self.db_secrets['OB_MSSQL_PORT']
            if db is None:
                self.db = self.db_secrets['OB_MSSQL_DATABASE']
            else:
                self.db = db
