...
from endi_celery.locks import acquire_lock, is_locked, release_lock
...
def get_closure_day_and_month() -> Tuple[int, int]:
    # Get data from endi's configuration
...
def get_current_exercice_dates(upload_date):
    """
    Return the dates of current exercice according to the upload_date
    """
    ...
    logger.info(
        "  + Current exercice starts the {} and ends the {}".format(
            current_exercice_begining_date, current_exercice_end_date
        )
    )

    return current_exercice_begining_date, current_exercice_end_date


def collect_operations_by_fiscal_year(
    upload_id: int, data_type: str, upload_date: datetime.date
    ...
    """
    logger.debug("  + Collecting operations for upload : {}".format(upload_id))

    (
        current_exercice_begining_date,
        current_exercice_end_date,
    ) = get_current_exercice_dates(upload_date)

    # Pour la tréso on ne prend pas les écritures dans le futur
    # ATTENTION : Pour l'instant _collect_operations() n'est que pour la tréso
    # mais si elle est utilisé pour d'autres états il faudra filtrer ici
    max_operation_date = min(current_exercice_end_date, date.today())

    current_exercice_operations = (
        AccountingOperation.query()
        .filter(
            AccountingOperation.date.between(
                current_exercice_begining_date, max_operation_date
            )
        )
        .all()
    )
    ...
    logger.debug(
        "    + {} operations collected".format(len(current_exercice_operations))
    )

    # Adding last exercice operations if needed
    # if data_type != AccountingOperationUpload.SYNCHRONIZED_ACCOUNTING:
    #     return current_exercice_operations

    # Has the closure been done for former year ?
    former_closure_year = upload_date.year - 1
    former_year_closure = (
    ...
    logger.info(f"    + {former_closure_year} closure is done ?: {closure_is_done}")
    ...
    if not closure_is_done:
        ...
        logger.info(
            "    + Appending {} operations from last exercice".format(
...
def collect_operations(
    ...
    """
    if compiler_type != "treasury":
        return AccountingOperation.query().filter_by(upload_id=upload.id)
    else:
        return collect_operations_by_fiscal_year(
            upload.id, upload.filetype, upload.date
        )
