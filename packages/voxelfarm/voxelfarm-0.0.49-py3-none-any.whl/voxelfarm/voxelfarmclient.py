...
class rest:
        ...
        if files:
            for file in files:
                ...
                if (not os.path.exists(file_path)):
                    result.success = False
                    result.error_info = 'File not found: ' + file_path
                    return result

                attach_files = {'file': open(file_path, 'rb')}
    ...
    def create_python_process_entity(self, project, name, code, files, fields, inputs, callback = None, update_type = None):
        result = self.creation_result()
    ...
    def create_program_process_entity(self, project, name, program, fields, inputs, callback = None, update_type = None):
        result = self.creation_result()
    ...
    def notify_users(self, project_id, file_entity_id):
        print(f'notify_users for project_id/file_entity_id: {project_id}/{file_entity_id}')
        file_entity = self.get_entity(file_entity_id, project_id)
        if file_entity != None:
            emails_to = os.getenv('vf_notify_emails')
            if emails_to is None:
                print('No email were sent because OS getenv variable vf_notify_emails is None')
                return { "success": True }
            
            emails_to = emails_to.split(",")
            folder_entity = self.get_entity(file_entity['file_folder'], project_id)
            parent_entity = self.get_entity(folder_entity['file_folder'], project_id)
