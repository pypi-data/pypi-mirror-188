...
class workflow_lambda_host:
    ...
    def submit_time_series_events(self, events:dict):
        self.lambda_framework.log(f'Submit Timeseries events...')
        self.lambda_framework.log(f'Creating producer Client...')
        if (len(events) == 0):
            self.lambda_framework.log(f'Events are empty, exiting  function...')
            return { "success": True}
        self.lambda_framework.log(f'Number of Events:{len(events)}')

        vf_ts_conn_str = os.getenv('vf_ts_conn_str')
        vf_ts_hub_name = os.getenv('vf_ts_hub_name')

        self.lambda_framework.log(f'vf_ts_hub_name:{vf_ts_hub_name}')

        client =  EventHubProducerClient.from_connection_string(conn_str=vf_ts_conn_str, eventhub_name=vf_ts_hub_name)
        self.lambda_framework.log('Creating a batch...')  
        event_data_batch = client.create_batch()
        self.lambda_framework.log('Created a batch...')  
        ...
        while (i < len(events)):
            try:
                ...
                #self.lambda_framework.log(f'adding {eventStr}')s
                event_data_batch.add(EventData(eventStr))
            except ValueError:
                can_add = False
            
            if (can_add) :
                #self.lambda_framework.log(f'Added {events[i]} to the batch')
                i += 1
                continue

            if (event_data_batch._count == 0) :
                self.lambda_framework.log('Message was too large and can not be sent until it is smaller')
                i += 1
                continue

            count = event_data_batch._count
            numEventsSent += count
            self.lambda_framework.log(f'Batch is full - sending {count} messages in a single batch. Sent {numEventsSent} events so far')
            ...
            if progressPercent != progessSentToHost:
                ...
                self.lambda_framework.progress(progessSentToHost, 'Publishing events')
        ...
        count = event_data_batch._count
        if ( count > 0) :
            self.lambda_framework.log(f'Sending remaining {count} messages as a single batch.')
            client.send_batch(event_data_batch)
            numEventsSent += count
        
        self.lambda_framework.log(f'Sent {numEventsSent} events done.')

        if (numEventsSent != len(events)) :
            self.lambda_framework.log(f'Not all messages were sent ({numEventsSent}/{len(events)})')

        client.close()
        self.lambda_framework.log('Done submit_ts_events...')

    def handle_dataframe(self, df_model:pd.DataFrame, timestamp:str, timestampId:str, spatialId: str, objectId: str, defaultObjectId:str, propertyIds:tuple, constant_spatial_id:str, constant_object_id:str):
        ...
        if timestampId in df_model or timestamp is not None:
            ...
            for idx, row in df_model.iterrows():
                ...
                #self.lambda_framework.log(f'valid:{valid}')
                if valid:
                    #self.lambda_framework.log(f'item:{object_value}')
                    if timestampId:
                        try:
                    ...
                    #self.lambda_framework.log(f'iso_time:{iso_time}')
                    eventObj = {
                        "time": iso_time,
                    ...
                    for propertyId in propertyIds:
                        if propertyId[0] in df_model:
                            ...
                            eventObj[propertyId[1]] = float(row[propertyId[0]]) if self.is_number(row[propertyId[0]]) else (row[propertyId[0]] if not pd.isnull(row[propertyId[0]]) else '')
            ...
            self.submit_time_series_events(eventsToSend)    
        else:
    ...
    def publish_feed(self, entity_id:str, timestamp:str, timestamp_id:str, spatialId: str, objectId: str, defaultObjectId:str,  propertyIds:tuple, constant_spatial_id:str, constant_object_id:str):
        ...
        entity_path = self.lambda_framework.download_entity_files(entity_id)
        if not os.path.isdir(entity_path):
            self.lambda_framework.log('Error Raw product empty')
            return {'success': False, 'error_info': 'Error Raw product empty'}
        ...
        for file_name in entity_files:
            entity_file = os.path.join(self.lambda_framework.get_scrap_folder(), file_name)
            ...
            if entity_file:
                if file_name.endswith('.ftr'):
                    ...
                    return {'success': False, 'error_info': f'Unsupported file type {file_name}'}
                
                if not df_model.empty:
                    self.handle_dataframe(df_model, timestamp, timestamp_id, spatialId, objectId, defaultObjectId, propertyIds, constant_spatial_id, constant_object_id)
                else:           
                    self.lambda_framework.log(f'file {file_name} is empty.')
                    return {'success': False, 'error_info': f'file {file_name} is empty.'}
