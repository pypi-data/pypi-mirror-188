...
class TestCheckWidth(TestCase):
    ...
    def test_width(self):
        self.assertEqual(check_width(self.width), (self.width, self.left_edge, self.right_edge))
    ...
    def test_width_and_left_edge(self):
        ...
        self.assertEqual(
            check_width(width=width, left_edge=left_edge), (width, left_edge, right_edge)
        )
    ...
    def test_width_and_right_edge(self):
        ...
        self.assertEqual(
            check_width(width=width, right_edge=right_edge), (width, left_edge, right_edge)
        )
    ...
    def test_left_and_right_edge(self):
        ...
        self.assertEqual(
            check_width(left_edge=left_edge, right_edge=right_edge), (width, left_edge, right_edge)
        )
    ...
    def test_left_and_right_edge_swapped_numbers(self):
        ...
        self.assertEqual(
            check_width(left_edge=left_edge, right_edge=right_edge), (width, right_edge, left_edge)
        )
...
class TestRectangle(TestCase):
    ...
    def test_split(self):
        ...
        self.assertListEqual(self.rectangle.split([StrainPosition(1.0, split_point, 'Concrete')]), split_list)

    def test_split_considering_effective_widths_positive(self):
        strain = 1.0
        split_point = StrainPosition(strain, self.top_edge + 0.5 * self.height, 'Concrete')
        rectangle_1 = Rectangle(self.top_edge, split_point.position, 2. * self.bending_width)
        rectangle_2 = Rectangle(split_point.position, self.bottom_edge, 2. * self.bending_width)
        split_list = [rectangle_1, rectangle_2]
        self.assertListEqual(self.rectangle.split([split_point], self.effective_widths), split_list)

    def test_split_considering_effective_widths_negative(self):
        strain = -1.0
        split_point = StrainPosition(strain, self.top_edge + 0.5 * self.height, 'Concrete')
        rectangle_1 = Rectangle(self.top_edge, split_point.position, 2. * self.membran_width)
        rectangle_2 = Rectangle(split_point.position, self.bottom_edge, 2. * self.membran_width)
        split_list = [rectangle_1, rectangle_2]
        self.assertListEqual(self.rectangle.split([split_point], self.effective_widths), split_list)
    ...
    def test_split_considering_effective_widths_zero(self):
        ...
        strain_positions = [
            StrainPosition(-10., self.top_edge, 'Concrete'),
            split_point,
            StrainPosition(10., self.bottom_edge, 'Concrete'),
        ]
        top_rectangle = Rectangle(self.top_edge, split_point.position, 2. * self.membran_width)
        bottom_rectangle = Rectangle(split_point.position, self.bottom_edge, 2. * self.bending_width)
        split_list = [top_rectangle, bottom_rectangle]
        self.assertListEqual(self.rectangle.split(strain_positions, self.effective_widths), split_list)
...
class TestCircle(TestCase):
    ...
    def test_split_inside_effective_width(self):
        ...
        strain_positions = [
            StrainPosition(-10., 0.0, 'Reinforcement'),
            split_point,
            StrainPosition(10., 10.0, 'Reinforcement'),
        ]
        self.assertEqual(circle.split(strain_positions, self.effective_widths), [circle])

    def test_split_outside_effective_width(self):
        """in case reinforcement bar is outside effective width it stays unconsidered"""
        circle = Circle(centroid_y=5.0, diameter=self.diameter, centroid_z=10.)
        split_point = StrainPosition(1.0, 5.0, 'Reinforcement')
        strain_positions = [
            StrainPosition(-10., 0.0, 'Reinforcement'),
            split_point,
...
class TestTrapazoidTriangle(TestCase):
    ...
    def test_split(self):
        ...
        trapazoids = [
            Trapezoid(self.top_edge, point.position, self.top_width, bottom_width=width_at_point),
            Trapezoid(point.position, self.bottom_edge, width_at_point, bottom_width=self.bottom_width),
        ]
...
class TestComposedGeometries(TestCase):
    ...
    def test_upe(self):
        ...
        left_flange = Rectangle(top_edge, top_edge+b_f, left_edge=-0.5*h_w-t_f, right_edge=-0.5*h_w)
        right_flange = Rectangle(top_edge, top_edge+b_f, left_edge=0.5*h_w+t_f, right_edge=0.5*h_w)
    ...
    def test_i_profile(self):
        ...
        top_flange = Rectangle(top_edge, top_edge+t_f, left_edge=-0.5*b_f, right_edge=0.5*b_f)
        web = Rectangle(top_edge+t_f, top_edge+t_f+h_w, width=t_w)
        bottom_flange = Rectangle(top_edge+t_f+h_w, top_edge+t_f+h_w+t_f, width=b_f)
...
class TestRebarLayer(TestCase):
    ...
    def test_rebar_horizontal_distance(self):
        self.assertEqual(self.layer.rebar_horizontal_distance, self.width / self.rebar_number)
