...
class TestSingleSpanSingleLoad(TestCase):
    ...
    def test_position_of_maximum_moment(self):
        self.assertEqual(self.forces.positions_of_maximum_moment(), [self.load_position])

    def test_position_of_maximum_deformation(self):
        self.assertEqual(self.forces.position_of_maximum_deformation(), self.load_position)
...
class TestSingleSpanSingleLoads(TestCase):
    ...
    def setUp(self) -> None:
        self.beam_length = 10.0
        self.load = 10.0
        self.load_positions = [1./3.*self.beam_length, 2./3.*self.beam_length]
        self.single_loads = [SingleLoad(position, 0.5*self.load) for position in self.load_positions]
        self.forces = SingleSpan(length=self.beam_length, loads=self.single_loads)

    def test_transversal_shear_support_left(self):
        self.assertAlmostEqual(self.forces.transversal_shear_support_left, 0.5 * self.load)

    def test_transversal_shear_support_right(self):
        self.assertAlmostEqual(self.forces.transversal_shear_support_right, -0.5 * self.load)
    ...
    def test_positions_of_maximum_moment(self):
        self.assertListEqual(self.forces.positions_of_maximum_moment(), self.load_positions)

    def test_position_of_maximum_deformation(self):
        self.assertEqual(self.forces.position_of_maximum_deformation(), 0.5*self.beam_length)
