...
from m_n_kappa import Rectangle, Concrete, Reinforcement, RebarLayer, Steel, IProfile, MKappaCurve, StrainPosition
...
class TestCompositeBeamPositive(TestCase):
    ...
    def test_cross_section(self):
        self.assertEqual(
            self.m_kappa_curve.cross_section,
            self.cross_section)

    def test_not_successful(self):
        self.assertListEqual(
            self.m_kappa_curve.not_successful, [
                [StrainPosition(strain=-0.0035, position=0.0, material='Concrete'), max_iterations],
                [StrainPosition(strain=-0.0025, position=25, material='Reinforcement'), same_sign],
                [StrainPosition(strain=-0.0025, position=75, material='Reinforcement'), same_sign],
                [StrainPosition(strain=-0.0016904761904761904, position=100.0, material='Steel'), same_sign]
            ])

    def test_m_kappa_points_moments(self):
        self.assertListEqual(
            self.m_kappa_curve.m_kappa_points.moments, [
                0.0, 279065090.70273054, 438755789.00432056, 516818301.88354325, 538683420.997121, 541724537.7407295
            ])
...
class TestCompositeBeamNegative(TestCase):
    ...
    def test_not_successful(self):
        self.assertListEqual(
            self.m_kappa_curve.not_successful, [
                [StrainPosition(strain=-0.0016904761904761904, position=115.0, material='Steel'), same_sign]]
        )

    def test_m_kappa_points_moments(self):
        self.assertListEqual(
            self.m_kappa_curve.m_kappa_points.moments, [
                -442383661.16446424, -409298265.3218652, -405044752.007268, -349978085.7744969,
                -54944467.314156845, -54718951.26913915, 0.0
            ])
