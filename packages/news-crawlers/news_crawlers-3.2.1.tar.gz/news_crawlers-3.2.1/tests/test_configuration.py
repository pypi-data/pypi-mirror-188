...
@pytest.fixture(name="create_tmp_config_file")
def create_tmp_config_file_fixture(change_cwd: Callable):
...
def test_find_config_raises_file_not_found_error_if_provided_path_does_not_exist(
    change_cwd: Callable, create_tmp_config_file: Callable
):
...
def test_find_config_returns_config_on_root_if_not_found_on_default(
    ...
    assert configuration.find_config("config/news_crawlers.yaml") == pathlib.Path("news_crawlers.yaml")


def test_find_config_returns_config_on_default_if_on_root_is_also_present(
    change_cwd: Callable, create_tmp_config_file: Callable, create_tmp_default_config_file: Callable
):
    assert configuration.find_config("config/news_crawlers.yaml") == pathlib.Path("config/news_crawlers.yaml")
