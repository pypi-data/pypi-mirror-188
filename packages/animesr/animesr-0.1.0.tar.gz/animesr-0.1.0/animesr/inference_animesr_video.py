...
@torch.no_grad()
def inference_video(args, video_save_path, device=None, total_workers=1, worker_idx=0, model_name='AnimeSR_v1-PaperModel'):
    # prepare model
    model = get_inference_model(args, device, model_name=model_name)
...
def run(args, model_name='AnimeSR_v1-PaperModel'):
    ...
    if num_process == 1:
        ...
        inference_video(args, video_save_path, device=device, model_name=model_name)
        return
...
def main(source, model_name='AnimeSR_v1-PaperModel'):
    ...
    run(args, model_name=model_name)
