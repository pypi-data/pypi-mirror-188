...
def get_regnet_args(
	...
	"""
	out_dims_per_block_cont = out_dim_slope * jnp.arange(depth) + out_dim_base
	stage_ind = jnp.round(jnp.log(out_dims_per_block_cont / out_dim_base) / jnp.log(log_step))
	out_dims_per_block_quantized = round_to_multiple(out_dim_base * jnp.power(log_step, stage_ind))
	out_dims_per_stage, depths = jnp.unique(out_dims_per_block_quantized, return_counts=True)

	to_round = dim_per_cardinal<out_dims_per_stage
	out_dims_per_stage = out_dims_per_stage.at[to_round].set(round_to_multiple(out_dims_per_stage[to_round], dim_per_cardinal))
