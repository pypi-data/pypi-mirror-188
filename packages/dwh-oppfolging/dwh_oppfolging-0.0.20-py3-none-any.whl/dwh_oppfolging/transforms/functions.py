...
from dwh_oppfolging.transforms.datatypes import Field


def flatten_dict(mapping: dict, separator: str = "_", flatten_lists: bool = False):
    """
    recursively flattens dict with specified separator
    optionally flatten lists in it as well
    >>> flatten_dict({'x': {'y': 3}, 'z': [1, 2, 3]}, "_", True)
    {'x_y': 3, 'z_0': 1, 'z_1': 2, 'z_2': 3}
    """
    def flatten(mapping: dict, parent_key: str = "") -> dict:
        items: list[Any] = []
        for key, value in mapping.items():
            flat_key = key if not parent_key else str(parent_key) + separator + key
...
def get_fields_in_dict(mapping: dict, fields: list[Field]) -> dict[str, Any]:
    """
    returns a dict of the fields in the dict
    >>> get_fields_in_dict({'x': 5, 'y': 3}, [Field('x', int)])
    {'x': 5}
    """
    record = {}
    for field in fields:
        if not field.name in mapping and not field.optional:
            raise KeyError(f"{field.name} missing")
        value = mapping.get(field.name)
        if value is None and not field.nullable:
            raise ValueError(f"{field.name} not nullable")
        if not isinstance(value, field.dtype):
            raise ValueError(f"{field} has {type(value)}, expected {field.dtype}")
