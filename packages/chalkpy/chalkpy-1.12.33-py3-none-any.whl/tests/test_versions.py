...
def test_versioned_feature_names():
    assert str(VChild.evolving) == "v_child.evolving@2"
    assert str(VChild.evolving_v2) == "v_child.evolving@2"
    assert str(VChild.evolving_v1) == "v_child.evolving"
    assert str(VParent.child.evolving) == "v_parent.child.evolving@2"
    assert str(VGrandParent.parent.child.evolving) == "v_grand_parent.parent.child.evolving@2"


def test_applying_versions():
    assert str(VParent.child.evolving @ 2) == "v_parent.child.evolving@2"
    assert str(VGrandParent.parent.child.evolving @ 2) == "v_grand_parent.parent.child.evolving@2"
...
def test_applying_versions_default_1():
    assert str(VParent.child1.evolving @ 2) == "v_parent.child1.evolving@2"
    assert str(VGrandParent.parent.child1.evolving @ 2) == "v_grand_parent.parent.child1.evolving@2"
...
class Funky:
    id: str
    trumpet: str = feature(version=2)
    saxophone: str = feature(version=3, default_version=3)


def test_version_iter():
    assert dict(Funky(trumpet="red", saxophone_v1="purple")) == {
        "funky.trumpet": "red",
        "funky.saxophone": "purple",
    }

    assert dict(Funky(trumpet="red", saxophone="purple")) == {
        "funky.trumpet": "red",
        "funky.saxophone@3": "purple",
    }


def test_len():
    assert len(Funky(trumpet="red", saxophone="purple")) == 2
