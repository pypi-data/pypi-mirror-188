...
class EnumFeature(str, enum.Enum):
    RED = "red"
    BLUE = "blue"
...
class SQLExecuteFeatures:
    ...
    float_feature: float
    datetime_feature: datetime = feature(dtype=pa.timestamp("us", None))
...
def model(sql_source: BaseSQLSourceProtocol):
    ...
    class ExecuteSQLModel(Base):
        ...
        timestamp_feature = Column(TIMESTAMP(True))
...
@pytest.mark.parametrize(
    "force_sqlalchemy_query_execution",
    [pytest.param(True, id="sqlalchemy"), pytest.param(False, id="pyarrow")],
    indirect=True,
)
...
class TestExecute:
    ...
    @pytest.mark.parametrize(
        "graph", [pytest.param(True, id="string_query"), pytest.param(False, id="orm_query")], indirect=True
    )
