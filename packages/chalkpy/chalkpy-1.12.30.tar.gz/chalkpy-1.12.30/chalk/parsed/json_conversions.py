...
from chalk.features import DataFrame, Feature, Filter, serialize_pyarrow_dtype
...
def _convert_type(underlying_type: Type) -> _ConvertedType:
    if isinstance(underlying_type, Windowed):
        return _ConvertedType(name=f"Windowed[{str(underlying_type._kind)}]", bases=["CHALK_WINDOWED"])
...
def convert_type_to_gql(t: Any) -> dataclasses.dataclass:
    ...
    if isinstance(t, Feature):
        ...
        if t.is_has_one:
            ...
            scalar_kind_gql = UpsertScalarKindGQL(
                ...
                version=t.version,
                dtype=serialize_pyarrow_dtype(t.converter.pyarrow_dtype).dict(),
            )
        ...
        return UpsertFeatureGQL(
            ...
            owner=t.owner,
            windowBuckets=t.window_buckets and list(t.window_buckets),
