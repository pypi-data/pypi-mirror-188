...
def _get_field(
    ...
    if isinstance(default, Feature):
        ...
        f = default
        if f.version_info is not None:
            f.version_info.base_name = f._name or annotation_name
            f.name = f.version_info.name_for_version(f.version_info.default)
...
def _process_class(
    ...
    for f in islice(cls_fields, len(cls_fields)):
        if f.version_info is None:
            continue
        alias_from_to[f.attribute_name] = f"{f.attribute_name}_v{f.version_info.default}"

        for i in range(1, f.version_info.maximum + 1):
            f_i = copy.copy(f)
            f_i.name = f.version_info.name_for_version(i)
            f_i.attribute_name = f"{f.attribute_name}_v{i}"
            f.version_info.reference[i] = f_i
            cls_fields.append(f_i)

            # The default feature already exists.
            f_i.no_display = i == f.version_info.default
