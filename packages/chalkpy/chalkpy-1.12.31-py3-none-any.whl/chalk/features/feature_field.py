...
class _VersionInfo:
    ...
    def name_for_version(self, version: int) -> str:
        return self.base_name if version == 1 else f"{self.base_name}:v{version}"
...
class Feature(Generic[_TPrim, _TRich]):
    def __init__(
        ...
        self.version_info: Optional[_VersionInfo] = (
            _VersionInfo(
                maximum=version,
                default=default_version or 1,
                reference={},
            )
...
"""
        ...
        if version not in self.version_info.reference:
            assert self.features_cls is not None
            raise ValueError(
                f"Cannot request version {version} of feature '{re.sub(':.*', '', self.root_fqn)}', because this feature "
                f"has a maximum version of {self.version_info.maximum} < {version}. "
                f"To add versions, write \n"
                f"""    @features
    class {self.features_cls.__name__}:
   -    {self.attribute_name}: ... = feature(version={self.version_info.maximum})
   +    {self.attribute_name}: ... = feature(version={version})
        ...
"""
            )

        versioned_feature = self.version_info.reference[version]
        if len(self.path) == 0:
