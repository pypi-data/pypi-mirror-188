...
class Feature:
    ...
    def is_feature_time(self) -> bool:
        ...
        if self._is_feature_time is None:
            if (
                ...
                return True
            return self.typ.is_feature_time
    ...
    def from_root_fqn(cls, root_fqn: str) -> Feature:
        ...
        if root_ns not in FeatureSetBase.registry:
            raise ValueError(
                f"Could not find feature class named '{root_ns}'. Do you mean one of {list(FeatureSetBase.registry.keys())} "
            )
            ...
            for x in features_cls.features:
                ...
                if x.name == feature_name:
                    ...
                    if len(split_fqn) > 0:
                        ...
                        assert isinstance(x.typ.underlying, type)
    ...
    def join(self) -> Optional[Filter]:
        ...
        if isinstance(foreign_features, Windowed) or not (
            isinstance(foreign_features, type) and issubclass(foreign_features, Features)
        ):
            return None
