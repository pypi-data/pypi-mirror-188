...
class Regtable:
    ...
    def _form(self):
        ...
        obs = pd.DataFrame([int(r.nobs) for r in self.regressions],
                       index=cols,columns=['obs']).T
        rsq = pd.DataFrame(['{:{fmt}}'.format(r.rsquared,fmt=self.sfmt)
                        for r in self.regressions], index=cols,
                       columns=['Rsq']).T
        ...
        if self.orientation == 'vertical':
            ...
            self.out = pd.concat([out,obs,rsq],axis=0).fillna('')

                
        elif self.orientation == 'horizontal':
            out = pd.concat(res,axis=0,ignore_index=True,sort=False).reset_index(drop=True)
            if self.nobs:
                obs = pd.Series([v for s in [[n,''] for n in list(obs.values[0])] for v in s],name='Obs')
                out = pd.concat([out,obs],axis=1)
            if self.rsq:
                rsq = pd.Series([v for s in [[n,''] for n in list(rsq.values[0])] for v in s],name='Rsq')
                out = pd.concat([out,rsq],axis=1)
    ...
    def to_latex(self):
        self._assertions()
        self._form()
        return self.out.style.to_latex()
    ...
    def set_reg_names(self,vars):
        if type(vars) == list:
            ...
            if vars == 'num-brackets':
                ...
                raise ValueError('Invalid input for variable names.')
