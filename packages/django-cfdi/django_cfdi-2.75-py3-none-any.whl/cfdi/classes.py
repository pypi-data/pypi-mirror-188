...
class CfdiPago():
    ...
    def generar_cfdi(self, configuracion, timbrado_prueba=None, pac=None):
            ...
            for imp_p in impuestos_p.values():
                ...
                if imp_p['impuesto_p'] == "002":
                    ...
                    if str(imp_p['tasa_o_cuota_p']) == "0.000000":
                        ...
                        impuesto_iva_0 = to_precision_decimales(imp_p['importe_p'], 6)
                    
                    if str(imp_p['tasa_o_cuota_p']) == "0.080000":
                        base = to_precision_decimales(imp_p['base_p'], 6)
                        base_iva_total_8 = base
                        impuesto = to_precision_decimales(imp_p['importe_p'], 6)
                        impuesto_iva_8 = to_precision_decimales(imp_p['importe_p'], 6)
                    
                    if str(imp_p['tasa_o_cuota_p']) == "0.160000":
                        base = to_precision_decimales(imp_p['base_p'], 6)
                        base_iva_total_16 = base
                        impuesto = to_precision_decimales(imp_p['importe_p'], 6)
                        impuesto_iva_16 = to_precision_decimales(imp_p['importe_p'], 6)
        ...
        if base_iva_total_16:
            cp.TotalTrasladosBaseIVA16 = to_precision_decimales(base_iva_total_16 * tc, 2)
            cp.TotalTrasladosImpuestoIVA16 = to_precision_decimales(impuesto_iva_16 * tc, 2) 

        if base_iva_total_8:
            cp.TotalTrasladosBaseIVA8 = to_precision_decimales(base_iva_total_8 * tc, 2)
            cp.TotalTrasladosImpuestoIVA8 = to_precision_decimales(impuesto_iva_8 * tc, 2) 
        
        if base_iva_total_0:
            cp.TotalTrasladosBaseIVA0 = to_precision_decimales(base_iva_total_0 * tc, 2)
            cp.TotalTrasladosImpuestoIVA0 = to_precision_decimales(impuesto_iva_0 * tc, 2) 

        if base_iva_total_excento:
            cp.TotalTrasladosBaseIVAExento = to_precision_decimales(base_iva_total_excento * tc, 2) 
