    ...
    while 1:   
        ...
        if iter_outer <= simTime:
            ...
            if iter_inner * skip <= simTime:
                ...
                plotter(copy.deepcopy(iPop), file_name, file_localization, copy.copy(iter_outer), plots, select)
        ...
        #         end = True
        # else:
        if (iter_outer % steps == 0 or sum(iPop[0,:]) >= 10**6) and break_type == 0:
            print('all steps')
            end = True
        elif (sum(iPop[0,:]) >= steps or sum(iPop[0,:]) >= 10**6) and break_type == 1 and select != 2:
            print('all cells')
            end = True
