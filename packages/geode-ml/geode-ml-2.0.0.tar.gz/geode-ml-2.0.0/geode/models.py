...
class Unet(SegmentationModel):
    ...
    def __init__(self, n_channels: int = 3,
        ...
        # define all the Unet layers

        # Downsampling-path convolutional layers
        self.inputs = Input(shape=(None, None, n_channels))
        ...
        self.max_pooling = [MaxPooling2D(pool_size=(2, 2),
                                         padding='same') for i in range(4)]

        # Upsampling layers
        self.upsampling = [UpSampling2D(size=(2, 2)) for i in range(4)]
