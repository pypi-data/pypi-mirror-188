...
class Encoder(nn.Module):
    def __init__(self, latent_dims=2,input_shape=50176,dim=1024):
        super(Encoder, self).__init__()
        self.linear1 = nn.Linear(input_shape, dim)
        self.linear2 = nn.Linear(dim, latent_dims)
...
class Decoder(nn.Module):
    def __init__(self, latent_dims=2,input_shape=50176,dim=1024):
        ...
        self.linear1 = nn.Linear(latent_dims, dim)
        self.linear2 = nn.Linear(dim, input_shape)

    def forward(self, z):
        z = F.relu(self.linear1(z))
        z = torch.sigmoid(self.linear2(z))
        return z.reshape((-1, 1, 224, 224))




class Autoencoder(nn.Module):
    def __init__(self, input_shape=50176,latent_dims=200,dim=1024):
        super(Autoencoder, self).__init__()
        self.encoder = Encoder(latent_dims=latent_dims,input_shape=input_shape,dim=dim)
        self.decoder = Decoder(latent_dims=latent_dims,input_shape=input_shape,dim=dim)
