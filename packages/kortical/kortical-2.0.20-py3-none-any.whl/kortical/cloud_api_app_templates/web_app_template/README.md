...
### App Deployment
To deploy the app you need to run the following command from inside the web_app_template directory:
...
From inside the `web_app_template` directory run:
...
`pytest tests/` or `pytest -m api` from inside the `web_app_template` directory on your command line to run the tests.
