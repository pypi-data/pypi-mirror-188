...
NODE_ATTRIBUTE_CALLBACKS = {
    ...
    # 'radical_electrons':        chem_prop('GetNumRadicalElectrons', OneHotEncoder(
    #     [0, 1, 2, 3],
    #     add_unknown=False,
    #     dtype=int,
    # )),
    # 'mass':                     chem_prop('GetMass', list_identity),
...
with Skippable(), (e := Experiment(base_path=BASE_PATH, namespace=NAMESPACE, glob=globals())):
    ...
    def load_data(_e, config):
        ...
        with open(file_path) as file:
            ...
            for row in dict_reader:
                ...
                target = row[_e.p['TARGET_COLUMN_NAME']]
                data = {
                    'smiles': smiles,
    ...
    for d in raw_data_list:
        ...
        for cb in FILTER_CALLBACKS:
            if cb(mol, data):
                e.info(f' ! skipping "{data[SMILES_COLUMN_NAME]}" due to filter "{cb.__name__}"')
                skip = True
                break
        ...
        # ~ Calculating 3D atom positions
        # 22.01.2023: Optionally, if the flag is set, this will apply a conformer on the molecule which will
        ...
        if USE_NODE_COORDINATES:
            try:
                ...
                e.warning(f' ! conformer failed for {data["smiles"]}: {exc}')
                plt.close(fig)
                continue
            ...
            for i, j in g['edge_indices']:
                c_i = g['node_coordinates'][i]
                c_j = g['node_coordinates'][j]
                length = la.norm(c_i - c_j)
                edge_lengths.append([length])

            g['edge_lengths'] = np.array(edge_lengths)
