...
class BinaryWriter:
    ...
    def __init__(
        ...
        Args:
            ...
            skip_timestamps(bool): skip generation of dynamic graph data
        """
        self.folder = folder
        self.suffix = suffix
        ...
        self.node_writer = NodeWriter(folder, suffix, is_temporal)
        self.edge_writer = EdgeWriter(folder, suffix, is_temporal, watermark)
    ...
    def add(
        ...
        args:
            data: Iterable[(int, int, int, float, list, typing.Optional[typing.Tuple[int, int]])] Data for a node first, then all of
                ...
                (node_id/src, -1/dst, type, weight, [ndarray for each feature vector or None in order of feature index], timestamps of creation and deletion of data).
        """
        # Default values for timestamps: node existed since the very beginning and towards the very end.
        node_created_at = 0
        node_removed_at = -1
        for src, dst, typ, weight, features, created_at, removed_at in data:
            ...
            if dst == -1:
                ...
                assert (
                    removed_at is None or removed_at <= node_removed_at
...
class NodeWriter:
    ...
    def __init__(self, folder: str, partition: int, skip_timestamps: bool = True):
        """Initialize writer and create binary files.
...
class NodeFeatureWriter:
    ...
    def __init__(self, folder: str, partition: int, is_temporal: bool = False):
        """Construct writer.
