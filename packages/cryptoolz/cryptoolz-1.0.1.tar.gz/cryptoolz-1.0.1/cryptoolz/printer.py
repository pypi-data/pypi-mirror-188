...
def _write(data: Any, encoded: bool, *args, **kwargs) -> None:
...
class Writer(abc.ABC):
    ...
    one data argument and one switch on whether it is
    printing to stdout / file, as standard modes of sorts.
    Everything further is laissez faire including narrowing and
    expanding arguments in derived classes to enforce behaviour.
    """

    @abc.abstractmethod
    def write(self, data: Any, encoded: bool, *args, **kwargs) -> None:
        pass
...
class PlainWriter(Writer):
    def write(self, data: Any, encoded: bool, *args, **kwargs) -> None:
        _write(data, encoded, *args, **kwargs)
...
class QRCodeWriter(Base64Writer):
    def write(self, data: Union[QRCode, str], encoded: bool, **kwargs) -> None:
        if encoded:
            _write_qr(data, **kwargs)
        else:
            kwargs.pop("po", None)
            _write(data, encoded, **kwargs)
...
class Printer(Queue):
    ...
    def print(
        ...
        **kwargs,
        ...
        while not self.empty():
            data, encoded = self.get()
            self.writer.write(data, encoded, *args, **kwargs)
    ...
    def put(
        ...
        data = (self.writer.encode(item) if encode else item, encode)
        super().put(data, block=block, timeout=timeout)
