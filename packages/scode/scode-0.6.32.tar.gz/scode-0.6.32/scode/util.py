...
def ip_change2():
    ...
    try:
        old_ip = requests.get("https://aaa.e-e.kr/ip", timeout=2).text
    except:
        for _ in range(30):
            try:
                old_ip = requests.get("https://aaa.e-e.kr/ip", timeout=2).text
    ...
    for cnt in range(90):
        ...
        try:
            cur_ip = requests.get("https://aaa.e-e.kr/ip", timeout=2).text
        except:
            pass
        else:
            if old_ip == cur_ip:
...
def err_logging(input_data, program_title=None, path='./error.txt', msg_flag = False):
    ...
    try:
        ip_check = requests.get('https://aaa.e-e.kr/ip')
        ip_address = ip_check.text
        ip_check.close()
    except:
        ip_address = 'None("https://aaa.e-e.kr/ip"와 연결이 되지 않아 ip를 확인 할 수 없습니다.)'
