...
def compute_equal_odds_optimum(
    ...
    # Group-wise ROC points     # TODO: debug this function!
    ...
    # constraints += [
    #     cp.norm_inf(global_roc_point - group_roc_points[i]) <= fairness_tolerance
    #     for i in range(n_groups)
    # ]
    ...
    # _plot_polygons([groupwise_roc_hulls[i] for i in range(n_groups)]) # TODO: delete

    # Run solver
    prob.solve()

    # Log solution
    logging.info(f"cvxpy problem status is {prob.status}.")
