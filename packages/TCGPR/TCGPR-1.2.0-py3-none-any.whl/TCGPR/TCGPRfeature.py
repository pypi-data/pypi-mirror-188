    ...
    if type(sampling_cap) != int:
        ...
        while True: 
            ...
            if len(up_data[0]) < sampling_cap:
                ...
                print('*'*100)
                data_in_pd.to_csv('Featureset selected by TCGPR {year}.{month}.{day}-{hour}.{minute}.{second}.csv'
                        .format(year=namey, month=nameM, day=named, hour=nameh, minute=namem, second=mames),index=False)
                data_in_pd_rest.to_csv('Featureset remained by TCGPR.csv',index=False)
                break
