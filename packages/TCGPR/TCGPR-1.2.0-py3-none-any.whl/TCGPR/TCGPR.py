...
def fit(filePath, Mission = 'DATA', Sequence = 'forward', initial_set_cap=3, sampling_cap=1, ratio=0.1, up_search = 2e2, exploit_coef=2,Self_call = True, exploit_model = False, alpha=1e-10, n_restarts_optimizer=10,
          normalize_y=True,  ):
    ...
    ++ elif Sequence = 'backward':
        tolerance, lower boundary of R is (1+ratio)R[last], default = 0.1, recommend = 0-0.3
    + for Mission = 'FEATURE':
        tolerance, lower boundary of R is (1+ratio)R[last], default = 0.1, recommend = 0-0.3
    ...
    if Mission == 'FEATURE':
        ...
        TCGPRfeature.cal_TCGPR(filePath=filePath, initial_set_cap=initial_set_cap, sampling_cap=sampling_cap, ratio=ratio, up_search = up_search, exploit_coef=exploit_coef,
            alpha=alpha, n_restarts_optimizer=n_restarts_optimizer,normalize_y=normalize_y, exploit_model = exploit_model)
