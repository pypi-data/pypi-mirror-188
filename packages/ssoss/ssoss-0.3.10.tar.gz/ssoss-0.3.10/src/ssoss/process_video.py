...
class ProcessVideo:
    ...
    # THIS IS NO LONGER A NEEDED FUNCTION. DELETE IF CONFIRMED
    def create_pic_list(self, event_df):
        """ from event dataframe, return frames of video to use and corresponding intersection descriptions.

        :param event_df: dataframe of events at sight distance
        :return: list of video frames to extract and list of intersection descriptions
        """
        frames = []
        intersection_desc = []
        current_frame = 0
        prev_frame = 0
        for i in range(0, event_df.last_valid_index()):  # +1??
            current_frame = i
            time_of_sb = event_df.timestamp.iloc[i] + event_df.t_adjust.iloc[i]
            time_of_picture = time_of_sb - self.get_start_timestamp()
            filename_description = event_df.string_desc.iloc[i] + "-" + str(round(time_of_sb, 3))
            if time_of_picture > 0:
                ...
                if current_frame > prev_frame:
                    ...
                    intersection_desc.append(filename_description)
            prev_frame = current_frame
        return frames, intersection_desc
        # TODO: return dict(zip(frames, intrsxn_desc))
    ...
    def extract_frame(self, extract_frames: list, intersection_desc: list):
        ...
        #print(f'Last Frame in list: {len(frames)-1}')
        ...
        while capture.isOpened() and len(extract_frames) > 0 and i < frame_count:
            ...
            for current_frame in tqdm(range(0, extract_frames[-1]), #tqdm(range(0, extract_frames[-1]),
                ...
                if ret is False:
                    ...
                    break
                #print(f'{i}:{current_frame}-{extract_frames[j]}----{j}/{len(extract_frames)}-{extract_frames[-1]}')
    ...
    def get_info(self):
        ...
        print("==================================")
        print('')
        print(f'Filename: {self.video_filename}')
        print(f'Frames per Second (FPS): {self.fps}')
        print(f'Total Number of Frames: {self.get_frame_count()}')
        print(f'Video Duration (HH:MM:SS.Z): {self.get_duration(seconds_output=False)}')
        print('')
        print("==================================")
        print("          VIDEO INFO              ")
        print("==================================")
