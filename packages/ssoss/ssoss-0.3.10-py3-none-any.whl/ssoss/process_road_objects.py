...
class ProcessRoadObjects:
    ...
    def __init__(self, in_dir_path=PurePath("./in/"),
    ...
    def intersection_sight_distance_timestamps(self):
        gpx_df = self.gpx_listDF
                ...
                for item in range(len(list(intersection_id))):
                    ...
                    if p.h_prev_and_current_before_next(approach_intersection, b_index) and \
                       p.h_next_less_than_current(approach_intersection, b_index):
                        ...
                        #t_simple = p.t_to_approach_simple(approach_intersection, b_index)
                        t_acc = p.t_to_approach_acc(approach_intersection, b_index)

                        print(
                            f"{sro_id}.{b_index}-SD:{approach_intersection_sight_distance}/{d_current}-------t:{p.get_timestamp()}-----t_countdown:{t_acc}---")
    ...
    def gpx_summary(self):
        ...
        {symbol * width}
        # GPX File:: {self.in_dir_path}/{self.gpx_file}
