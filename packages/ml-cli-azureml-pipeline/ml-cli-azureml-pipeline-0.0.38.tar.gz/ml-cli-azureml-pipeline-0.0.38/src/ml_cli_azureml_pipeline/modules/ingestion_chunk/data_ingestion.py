...
for dataset in datasets:
    ...
    #if number_chunk == 1:
    chunk_name = "chunk0"
    if dataset_directory is None or dataset_directory == "":
        dir_path = Path(str(Path(raw_data_dir) / chunk_name))
    else:
        dir_path = Path(str(Path(raw_data_dir) / chunk_name / dataset_directory))
    dir_path.mkdir(parents=True, exist_ok=True)
    dataset.download(target_path=dir_path, overwrite=False)
#else:
