...
class LoggingAppBase:
    ...
    def __init__(self, level=logging.INFO, logfile=None):
        ...
        self.args = {}
        self.initial_level = libsrg.level2str(level)
        try:
            ...
            self.parser.add_argument('--libsrg', help='Print version of libsrg and exit', action='version',
                                     version=f"libsrg {version('libsrg')}")
    ...
    def perform_parse(self):
        ...
        level = libsrg.level2int(self.args.logging)
        logging.getLogger().setLevel(level)
        if 'logfile' in self.args and self.args.logfile is not None:
            LoggingCounter.add_logfile(self.args.logfile, mode='w')
