...
class Cfg:
    ...
    def _check(self):
        ...
        if not os.path.isdir(self.outdir):
            errs += 1
            services.logger.err('outdir is not a valid dir; does it exist?')
        ...
        if self.storage_type not in valid:
            ...
            if not os.path.isdir(self.storage_shared_dir):
                errs += 1
                services.logger.err('storage_shared_dir is not a valid dir; does it exist?')
        ...
        if self.use_utc is None:
            errs += 1
            services.logger.err('use_utc is None; must be either True or False')
