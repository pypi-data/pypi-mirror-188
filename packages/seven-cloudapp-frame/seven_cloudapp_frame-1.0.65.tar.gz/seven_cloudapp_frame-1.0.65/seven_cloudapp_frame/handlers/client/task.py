...
@LastEditTime: 2023-01-30 14:53:28
...
class InviteNewUserHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        invoke_result_data = task_base_model.process_invite_user(app_id, act_id, module_id, user_id, login_token, from_user_id, self.__class__.__name__, check_user_nick, check_new_user, continue_request_expire, close_invite_limit, check_act_info_release=check_act_info_release, check_act_module_release=check_act_module_release)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        if is_stat == True:
...
class InviteJoinMemberHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        invoke_result_data = task_base_model.process_invite_join_member(app_id, act_id, module_id, user_id, login_token, mix_nick, from_user_id, self.__class__.__name__, access_token, app_key, app_secret, check_user_nick, continue_request_expire, close_invite_limit)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        ref_params = {}
        invoke_result_data = self.business_process_executed(invoke_result_data, ref_params)
