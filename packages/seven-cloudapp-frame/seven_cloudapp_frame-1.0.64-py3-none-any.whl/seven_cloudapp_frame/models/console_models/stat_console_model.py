...
@LastEditTime: 2022-12-13 10:38:22
...
class StatConsoleModel():
    ...
    def console_stat_queue(self, mod_count=1):
        ...
        """
        stat_config = config.get_value("stat_config", {})
                ...
                if len(stat_queue_list) > 0:
                    for stat_queue in stat_queue_list:
                        try:
                            ...
                            stat_config = config.get_value("stat_config", {})
                            stat_log_model = StatLogModel(db_transaction=db_transaction, sub_table=SevenHelper.get_sub_table(stat_orm.id, stat_config.get("statlog_subtable_num", 0)))
                            is_add = True
    ...
    def _process_redis_stat_user_queue(self, mod_value):
        ...
        while True:
            try:
                ...
                try:
                    stat_queue_dict = SevenHelper.json_loads(stat_queue_json)
                    stat_user_log_model = StatUserLogModel(db_transaction=db_transaction)
                    ...
                    stat_report_total = stat_user_report_model.get_total(stat_report_condition, params=stat_report_param)
                    if stat_report_total == 0:
...
class StatUserDataWork(TimingWork):
    ...
    def clear_report(self):
        ...
        while True:
            stat_user_report_list = stat_user_report_model.get_list(where="create_day<%s", order_by="id asc", limit="100", params=[SevenHelper.get_now_day_int(-24 * space_day)])
