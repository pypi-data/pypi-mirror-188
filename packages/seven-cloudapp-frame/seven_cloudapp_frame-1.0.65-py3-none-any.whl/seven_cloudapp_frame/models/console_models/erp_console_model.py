...
@LastEditTime: 2022-11-15 15:02:38
...
class ErpConsoleModel():
    ...
    def console_erp(self, mod_count=1, prize_roster_sub_table=None):
        """
        :description: 控制台推送订单和回写物流
        :param mod_count: 单表队列数
        :param prize_roster_sub_table: 中奖记录扩展表名
        :return: 
        :last_editors: HuangJianYi
        """
        for i in range(mod_count):

            t = threading.Thread(target=self.process_order_push, args=[i, mod_count, True, prize_roster_sub_table])
            t.start()

            j = threading.Thread(target=self.process_order_pull, args=[i, mod_count])
            j.start()

    def process_order_push(self, mod_value, mod_count, is_loop=True, prize_roster_sub_table=""):
        """
                ...
                if len(erp_relation_list) > 0:
                    for erp_relation_dict in erp_relation_list:
                        try:
                            ...
                            prize_roster_model = PrizeRosterModel(sub_table=prize_roster_sub_table)
