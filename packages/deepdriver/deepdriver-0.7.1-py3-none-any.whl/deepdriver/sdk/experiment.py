...
def prepare_code(code: dict = None) -> Tuple[bool, str, Callable]:
    ...
    if arti.entry_list == []:
        ...
        file_entry_list = next((entry.local_path for entry in arti.entry_list if entry.path.endswith(code['file'])),
                               None)
        if not file_entry_list:
            raise Exception(f'cannot find file({code["file"]}) in artifcat')

        # dynamically function load from file
        with open(file_entry_list, "r") as file:
        ...
        return True, path, eval(func_name)
