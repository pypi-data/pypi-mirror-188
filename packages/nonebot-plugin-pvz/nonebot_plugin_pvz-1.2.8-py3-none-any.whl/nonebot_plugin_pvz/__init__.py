                ...
                if plant in my_plants:
                    ...
                    if flag:
                        ...
                        res = MessageSegment.text(
                            f"放置成功，您的草坪现在为：\n{','.join(now_pos)}") + img + MessageSegment.text("可以重新进行阵容评估哦")
...
async def _(event: MessageEvent):
    ...
    if user_id in users_id:
        ...
        await asyncio.sleep(1)
        await lawn_evaluation.finish(f"您当前的阵容定级为{level}, 超越了{exceed}%的当前用户, 排名为{summary_z}/{summary_m}", at_sender=True)
    else:
        await asyncio.sleep(1)
        await play_with_computer_zombie.finish("您暂未开启草坪,请通过'查看草坪'来开启", at_sender=True)
