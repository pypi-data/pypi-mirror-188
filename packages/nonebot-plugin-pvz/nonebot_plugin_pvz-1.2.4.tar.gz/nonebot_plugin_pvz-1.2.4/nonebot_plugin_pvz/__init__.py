...
def one_by_one(team: List[str], plants: List[str]) -> (List[MessageSegment], int):
    if not os.path.exists(PVZ_OUTPUT_PATH):
        os.makedirs(PVZ_OUTPUT_PATH)
    # 或去当前全部植物的对象
    ...
    for zombie in team:
        ...
        log.append(MessageSegment.text("当前双方阵容")+MessageSegment.image(f"file:///{PVZ_OUTPUT_PATH}/output_new{cnt}.png"))
        # 图片计数序号递增
        cnt += 1
        ...
        while zb.hp > all_damage:
            ...
            if min(distance) > 0.5 or (lawn_plants[distance.index(
                ...
                if fight_cond == 0:
                    ...
                    lawn_pic([all_plants[x] for x in plants if x != "0"], cnt)
                    zombie_pic(all_zombie[zombie], dist, cnt)
                    log.append(MessageSegment.image(f"file:///{PVZ_OUTPUT_PATH}/output_new{cnt}.png"))
                    cnt += 1
                else:
                    fight_time += 0.5
                # 判断僵尸是否可以跳过植物
                if zb.jump:
                    if lawn_plants[tar].unjumpable == 1:
                        zb.jump = 0
                        zb.v = zb.v/2
                        log.append(MessageSegment.text(f"时间{time},{zombie}被高坚果阻挡后无法起跳，撑杆掉落，速度减半"))
                    else:
                        # 距离递减
                        dist -= 1
                        # 生成图片
                        ...
                        if zb.v == 0.4:
                            zb.jump = 0
                            zb.v = 0.2
                            log.append(MessageSegment.text(f"时间{time},{zombie}被植物阻挡后起跳，撑杆掉落，速度减半")+MessageSegment.image(f"file:///{PVZ_OUTPUT_PATH}/output_new{cnt}.png"))
                        else:
                            # 如果是跳跳僵尸，则可以继续跳过，同时失去攻击机会
                            log.append(MessageSegment.text(f"时间{time},{zombie}被植物阻挡后起跳,同时企图继续跳过其他植物")+MessageSegment.image(f"file:///{PVZ_OUTPUT_PATH}/output_new{cnt}.png"))
                        cnt += 1
                    # 判断当前目标
                    if tar > 0:
                        lawn_plants[tar - 1].hp -= zb.damage
                        log.append(MessageSegment.text(f"时间{time},{zombie}对{plants[tar]}造成{zb.damage}点伤害"))
                        if lawn_plants[tar - 1].hp < 0:
                            log.append(MessageSegment.text(f"时间{time},{plants[tar - 1]}被击败移出"))
                ...
                if fight_time / \
                    ...
                    if lawn_plants[tar].only_hurt_by_boss == 0 or (
                        ...
                        log.append(MessageSegment.text(f"时间{time},{zombie}对{plants[tar]}造成{zb.damage if zb.jump == 0 else 0}点伤害。两次攻击间植物合计对{zombie}造成{all_damage}点伤害,{zombie}剩余血量为{zb.hp - all_damage}"))
                        # 如果当前目标植物的耐久度小于0，则移除该植物
                        if lawn_plants[tar].hp < 0:
                            log.append(MessageSegment.text(f"时间{time},{plants[tar]}被击败移出"))
                            # 移除植物
                            lawn_plants.pop(tar)
                            plant_pos.pop(tar)
                            plants.pop(tar)
        ...
        if iswin != 1 and zcnt == len(team):
            iswin = 2
        if iswin == 1:
            log.append(MessageSegment.text(f"入侵成功！成功吃掉对方脑子！")+MessageSegment.image(f"file:///{PVZ_ORI_PATH}/end.jpg"))
            break
        elif iswin == 2:
