from src.rec_system.data.survey import update_survey_answers_file, convert_to_explicit_dataset
import pytest
import pandas as pd
...
class TestSurvey:
    ...
    def test_convert_to_explicit_dataset_check_columns(self, test_input, expected):
        df = convert_to_explicit_dataset(df=test_input)
        assert df.columns.to_list() == expected

    @pytest.mark.parametrize("expected",
                             [['Adres e-mail','pytanie','ocena']])
    def test_convert_to_explicit_dataset_no_parameters_shape(self, expected):
        df = convert_to_explicit_dataset()
        assert df.columns.to_list() == expected
    ...
    def test_convert_to_explicit_dataset_with_empty_csv(self, test_input, expected):
        with pytest.raises(expected, match='No columns to parse from file') as exc_info:
            convert_to_explicit_dataset(survey_path=test_input)

    @pytest.mark.parametrize("test_input,expected", [(pd.read_csv("./tests/unit/data/survey.csv"), False)])
    def test_convert_to_explicit_dataset_with_df(self, test_input, expected):
        df = convert_to_explicit_dataset(df=test_input)
        assert df.empty == expected
