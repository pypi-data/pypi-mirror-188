...
class Zerodha(Broker):
    ...
    def orders(self, tag=None):
        ...
        if len(orders) == 0:
            orders = pd.DataFrame(columns=['user_id', 'tradingsymbol', 'tag', 'quantity_traded', 'average_price',
                                           'transaction_type', 'exchange', 'product', 'status'])
        ...
        orders = orders[['user_id', 'tradingsymbol', 'tag', 'quantity_traded', 'average_price', 'transaction_type',
                         'ltp', 'exchange', 'product', 'status']]
        orders.loc[:, 'quantity_traded'] = np.where(orders.status.isin(["REJECTED", "CANCELLED"]),
                                                    0,
                                                    orders.quantity_traded)
