...
def multipart_upload(
    ...
    part_size_in_bytes = 10 * 1024**2  # minimum part size on aws is 5MB

    # we have to use low-level API to be able to support resumable uploads - https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpu-upload-object.html
    ...
    if mpu_id != None:
        ...
        parts_already_uploaded = {
            ...
            for part in get_uploaded_parts(
                s3_client, s3_bucket_name, s3_file_url, mpu_id
            )
        ...
        mpu = s3_client.create_multipart_upload(
            Bucket=s3_bucket_name, Key=s3_file_url
        )
        mpu_id = mpu["UploadId"]
    ...
    part_numbers_to_upload = set(
        range(1, total_bytes // part_size_in_bytes + 2)
    )  # TODO: why +2? check corner cases
    thread_map(
