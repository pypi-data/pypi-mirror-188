...
from autumn8.cli.pending_uploads import remove_upload
...
def upload_model(
    ...
    **kwargs,
    ...
    s3_file_url = generate_s3_file_url(
        organization_id,
        run_id,
        kwargs,
        model_file_name,
    ...
    if input_file_path != None and len(input_file_path) > 0:
        ...
        s3_input_file_url = generate_s3_input_file_url(
            organization_id,
            run_id,
            kwargs,
            s3_bucket_name,
...
def generate_s3_input_file_url(
    organization_id, run_id, kwargs, s3_bucket_name, filename
):
    if "s3_input_file_url" not in kwargs:
        return f"{s3_bucket_name}/inputs/{organization_id}-{run_id}-{filename}"
    return kwargs["s3_input_file_url"]
...
def generate_s3_file_url(
    ...
    if "s3_file_url" not in kwargs:
        ...
        return f"{s3_bucket_name}/models/{organization_id}-{run_id}-{model_file_name}{additional_extension}"
    return kwargs["s3_file_url"]
