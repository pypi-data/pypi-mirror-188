...
def temp_train_test_split(params):
    ...
    return f"data = huble.sklearn.train_test_split(data=data,parameters={parameters})"


def temp_train_model(params):
    return f"data = huble.sklearn.train_model(data=data, model=model, column={params['target_column']})"
