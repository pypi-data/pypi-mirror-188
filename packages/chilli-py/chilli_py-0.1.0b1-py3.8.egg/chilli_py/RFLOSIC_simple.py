...
from chilli_py.BFGS import State, GD, BFGS, rflo 
...
class SCF_FLOSIC1: 
    ...
    def __init__(self,flo,fods,U0,opt_method="CG",gtol=5e-4,maxiter=300,opt_verbose=False): 
        ...
        self.fods = fods 
        self.U0 = U0 
        state = State(flo.atoms,"FODs",fods=fods,mf=copy(flo.mf),U=copy(U0))
        ...
        self.fodopt = BFGS(state,rflo,alpha=70.,Nmax=100,maxstep=0.2,Etol=1e-6,fmax=0.005)
        # opt = GD(state,rflo,alpha=0.1) #,alpha=70.,Nmax=100,maxstep=0.2,Etol=1e-6,fmax=0.005)
    ...
    def _kernel(self,flo,fods,U,use_avg=True,maxiter=300,verbose=False,Etol=1e-6): 
        ...
        res.fods = fods
        res.force = force 
        res.fmax = get_fmax(force) #self.fodopt._get_fmax(force) 
        return res
...
def SCF_FLOSIC2(flo,fods,U0,use_avg=True,maxiter=300,verbose=False,Etol=1e-6,opt_method="CG",gtol=5e-4,opt_verbose=False):
    ...
    if use_avg:
        avg = DIIS()
    state = State(flo.atoms,"FODs",fods=fods,mf=copy(flo.mf),U=copy(U))
    ...
    for iiter in range(maxiter):
        ...
        rflo.verbose = 4 
        # Begin: Inner loop 
        opt = BFGS(state,rflo,alpha=70.,Nmax=100,maxstep=0.2,Etol=1e-6,fmax=0.005)
        # opt = GD(state,rflo,alpha=0.1) #,alpha=70.,Nmax=100,maxstep=0.2,Etol=1e-6,fmax=0.005)
...
def main():
    ...
    for s in SYSTEM: 
        ...
        mflo.verbose = 4
        res,t  = mflo.kernel()
        s_dict = {s: {"Etot": res.Etot, "fmax" : res.fmax}}
        bench.update(s_dict) 
    pprint(bench) 
