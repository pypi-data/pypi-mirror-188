...
def bruhcolored(text, color=None, on_color=None, attrs=None, type="limited"):
    """
    ...
    if type == "full":
        for i in range(0, len(COLORS_256), 16):
            for j in range(i, i + 16):
                c = bruhcolored((' ' * (4 - len(str(j)))) + (str(j)), color=j, on_color=231, type="full")
                print(f"{c}", end="")
            print()
        for i in range(0, len(COLORS_256), 16):
            for j in range(i, i + 16):
                c = bruhcolored((' ' * (4 - len(str(j)))) + (str(j)), color=j, on_color=232, type="full")
                print(f"{c}", end="")
            print()
    else:
