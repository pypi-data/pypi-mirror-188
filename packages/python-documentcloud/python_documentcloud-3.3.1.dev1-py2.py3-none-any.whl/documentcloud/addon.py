...
class SoftTimeOutAddOn(AddOn):
    ...
    def rerun_addon(self, last_document, remaining_documents):
        ...
        if self.documents:
            ...
            # If documents were passed in by query, get the timestamp from the last
            # document processed, and query for documents created after that one.
            # If we have already added a created_at clause, replace it instead of
            # appending to it again
            print(self.query)
            start_timestamp = last_document.created_at.replace(tzinfo=None) + "Z"
            query_endings = self.query.split()[-3:]
            if (
                query_endings[0].startswith("created_at:{")
                and query_endings[1] == "TO"
                and query_endings[2] == "*}"
            ):
                query = self.query.split()[:-3]
            else:
                query = self.query

            options["query"] = f"{query} created_at:{{{start_timestamp} TO *}}"
            print(options["query"])
    ...
    def get_documents(self):
        ...
        if self.documents:
            ...
            documents = self.client.documents.search(self.query, sort="-created_at")
